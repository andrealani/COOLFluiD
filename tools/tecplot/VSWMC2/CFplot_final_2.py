...
def readFileList(): 
   ...
   for line in myfile:
      ...
      fname = line.replace('\n','')
      if ("_" in ctime): ctime = ctime.replace("_",'.')  
      fileList.append((fname, float(ctime)))
...
def runTecplotScript(fileList, surfList, cmd_changedir ):
   """
   ...
   for file in fileList:
      ...
      cmd_convert1 = cmd_changedir + " ; ln -sf " + file[0] + " output.plt ; preplot output.plt output.plt.bin"
      #print(cmd_convert1)
      system(cmd_convert1)

      # generate output.surf.plt.bin needed by TECPLOT script
      cmd_convert2 = cmd_changedir + " ; ln -sf " + surfList[counter][0] + " output.surf.plt ; preplot output.surf.plt output.surf.plt.bin"
      #print(cmd_convert2)
      counter+=1
      system(cmd_convert2)

      # generate the pics
      cmd_macro = "tec360 -b -p CFplot_final.mcr"
      print cmd_macro
      system(cmd_macro)

      for fvar in state:
       cmd_pics = cmd_changedir + " ; mv " + fvar + "_final.png" + " " + fvar + "_time_" + unicode(file[1]) + ".png"
       print cmd_pics
       system(cmd_pics)
