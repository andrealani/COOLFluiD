...
Environment::ObjectProvider<CellVertexSparsity,
                            GlobalJacobianSparsity,
                            FrameworkLib>
aCellVertexSparsityProvider("CellVertex");

//////////////////////////////////////////////////////////////////////////////

CellVertexSparsity::CellVertexSparsity()
{
  CFAUTOTRACE;
}
...
void CellVertexSparsity::initPeriodicData(std::vector<int>& crossNodes)
{
...
void CellVertexSparsity::computeNNz( std::valarray<CFint>& nnz, std::valarray<CFint>& ghostNnz)
{
  CFAUTOTRACE;
...
//     MeshDataStack::getActive()->getConnectivity("cellStates");
...
//  std::cout << "NNZ SUM:      " << nnz.sum() << "\n" << std::flush;
//  std::cout << "NNZGHOST SUM: " << ghostNnz.sum() << "\n" << std::flush;
//  sleep(2);
...
void CellVertexSparsity::computeMatrixPattern( std::valarray<CFint>& nnz,
                                               ...
                                               vector< vector<CFuint> >& matrixPattern)
{
  ...
  cf_assert(nnz.size() == ghostNnz.size());
  cf_assert( nnz.size() == matrixPattern.size() );
  ...
  for (CFuint iState = 0; iState < nbStates; ++iState)
  {
    ...
    for (CFuint iCell = 0; iCell < nbNeighborCells; ++iCell) {
      const CFuint cellID = neighborCells[iState][iCell];
      const CFuint nbStatesInCell = cellStates->nbCols(cellID);
      for (CFuint iState = 0; iState < nbStatesInCell; ++iState) {
        // store all the neighbor states + the considered state itself
        State *const currState = states[(*cellStates)(cellID,iState)];
        neighborStates->insert(SetOfNeighbors::value_type(currState));
        if (!currState->isParUpdatable()) {
          // if the state is not updatable store it in ghostNeighStates
          ghostNeighStates->insert(SetOfNeighbors::value_type(currState));
    ...
    matrixPattern[iState].reserve(nbNeighStates);
    SetOfNeighbors::const_iterator itr;
    for (itr = neighborStates->begin(); itr != neighborStates->end(); ++itr)
      matrixPattern[iState].push_back( (*itr)->getGlobalID() );
