...
#ifndef COOLFluiD_ShapeFunctions_LagrangeShapeFunctionLineP3_hh
#define COOLFluiD_ShapeFunctions_LagrangeShapeFunctionLineP3_hh
...
class ShapeFunctions_API LagrangeShapeFunctionLineP3 : public ShapeFunctions::LagrangeShapeFunction {
public:
  ...
  static void getStatesMappedCoordinates(std::vector<RealVector>& mappedCoords)
  {
    ...
    mappedCoords[1][KSI] = 0.  ;
    mappedCoords[2][KSI] = -1.0/3.0  ;
    mappedCoords[3][KSI] = 1.0/3.0  ;
  }
        ...
        const RealVector& mappedCoord, RealVector& shapeFunc)
  {
    ...
    CFreal onesixteenth = 1.0/16.0;
    shapeFunc[0] = -onesixteenth*(1.0 - mappedCoord[0])*(1. - 9.*mappedCoord[0]*mappedCoord[0]);
    shapeFunc[1] = -onesixteenth*(1.0 + mappedCoord[0])*(1.0 - 9.*mappedCoord[0]*mappedCoord[0]);
    shapeFunc[2] = 9.0*onesixteenth*(1.0 - mappedCoord[0]*mappedCoord[0])*(1.0 - 3.*mappedCoord[0]);
    shapeFunc[3] = 9.0*onesixteenth*(1.0 - mappedCoord[0]*mappedCoord[0])*(1.0 + 3.*mappedCoord[0]);
  }
...
//CFout << "x1: " << x1 << "\n";
//CFout << "coord: " << coord << "\n";
    RealVector v(x0.size());
    RealVector w(x0.size());

    v = x1-x0;
    w = coord-x0;

    CFreal normV = sqrt(v[XX]*v[XX] + v[YY]*v[YY]);
    CFreal normW = sqrt(w[XX]*w[XX] + w[YY]*w[YY]);
//CFout << "x1-x0: " << v << "\n";
//CFout << "coord-x0: " << w << "\n";
//CFout << "NormFace: " <<normV << "\n";
//CFout << "NormProj: " <<normW << "\n";
    // w[XX]/v[XX] = w[XX]/v[XX] = ksi
    mappedCoord[XX] = normW/normV;  ////@todo here what to do ??
...
}; // end of class LagrangeShapeFunctionLineP3
...
#endif // COOLFluiD_ShapeFunctions_LagrangeShapeFunctionLineP3_hh
