...
Environment::ObjectProvider<SpectralFDBuilder,
               MeshDataBuilder,
               SpectralFDModule,
               1>
...
SpectralFDBuilder::SpectralFDBuilder(const std::string& name) :
...
SpectralFDBuilder::~SpectralFDBuilder()
{
}

//////////////////////////////////////////////////////////////////////////////

void SpectralFDBuilder::configure ( Config::ConfigArgs& args )
{
...
void SpectralFDBuilder::releaseMemory()
{
...
void SpectralFDBuilder::setMapGeoToTrs()
{
}

//////////////////////////////////////////////////////////////////////////////

void SpectralFDBuilder::createTopologicalRegionSets()
{
  CFAUTOTRACE;
...
void SpectralFDBuilder::computeGeoTypeInfo()
{
  CFAUTOTRACE;

  // set polynomial type
  getCFmeshData().setSolutionPolyType(CFPolyForm::SPECTRALFD);
...
void SpectralFDBuilder::createCellFaces()
{
  CFAUTOTRACE;
...
void SpectralFDBuilder::createInnerFacesTRS()
{
  CFAUTOTRACE;
...
void SpectralFDBuilder::reorderInnerFacesTRS()
{
  CFAUTOTRACE;

  // Pointer to list containing the GeoType of the face ==> class variable of SpectralFDBuilder, we can access this variable directly through m_inGeoTypes
  ...
  // Get the number of different face types in mesh (their should only be one type for SpectralFD for now!!)
...
vector< vector < vector < CFuint > > > SpectralFDBuilder::getNodeConnPerOrientation(const CFGeoShape::Type shape)
{
  CFAUTOTRACE;

  SpectralFDElementData* sdElemData;
  switch (shape)
  {
    ...
    {
      sdElemData = new QuadSpectralFDElementData(CFPolyOrder::ORDER0);
    } break;
    case CFGeoShape::HEXA:
    {
      sdElemData = new HexaSpectralFDElementData(CFPolyOrder::ORDER0);
    } break;
    default:
    {
...
vector < vector < CFuint > > SpectralFDBuilder::getFaceConnPerOrientation(const CFGeoShape::Type shape)
{
  CFAUTOTRACE;
  ...
  // get vector containing the nodes connectivities for each orientation
  vector < vector < CFuint > > faceConnPerOrientation = *sdElemData->getFaceConnPerOrient();
  delete sdElemData;
...
void SpectralFDBuilder::setMaxNbStatesInCell()
{
...
void SpectralFDBuilder::setMaxNbNodesInCell()
{
  SafePtr<vector<ElementTypeData> > elementType = getCFmeshData().getElementTypeData();
...
void SpectralFDBuilder::setMaxNbFacesInCell()
{
  SafePtr<vector<ElementTypeData> > elementType = getCFmeshData().getElementTypeData();
...
void SpectralFDBuilder::createBoundaryFacesTRS()
{
  CFAUTOTRACE;
...
SpectralFDBuilder::createTopologicalRegionSet
 ...
 const CFuint iTRS)
{
  ...
  CFLogDebugMin( "SpectralFDBuilder::setTopologicalRegions() : nameTRS "
                 << name
...
void SpectralFDBuilder::createPartitionBoundaryFacesTRS()
{
  CFAUTOTRACE;
...
vector< vector < CFuint > > SpectralFDBuilder::getBFaceOrientations(const CFGeoShape::Type shape)
{
  CFAUTOTRACE;
  ...
  // get vector containing the nodes connectivities for each orientation
  vector < vector < CFuint > > bFaceOrientations = *sdElemData->getFaceNodeConn();
  delete sdElemData;
...
void SpectralFDBuilder::reorderBoundaryFacesTRS()
{
  CFAUTOTRACE;
