...
MethodCommandProvider< StdSetup,SpectralFDMethodData,SpectralFDModule >
...
void StdSetup::execute()
{
  ...
  if (getMethodData().hasDiffTerm() && getMethodData().separateConvDiffComs())
  {
  ...
  // EXPLICIT INSTANCES OF THE SPECTRALFD BASIS FUNCTIONS
  // get SpectralFDElementData
  vector< SpectralFDElementData* >& sdLocalData = getMethodData().getSDLocalData();
  const CFPolyOrder::Type polyOrder = sdLocalData[0]->getPolyOrder();
  ...
  if (dim == 2)
  {
    switch (polyOrder)
    {
      ...
      {
      //  SpectralFDBaseFunctionQuadP4::SpectralFDBaseFunctionQuadP4();
      } break;
...
void StdSetup::computeStatesVolumes()
{
  ...
  cf_assert(elemType->size() == sdLocalData.size());
  CFuint totNbrStates = 0;
  const CFuint nbrElemTypes = elemType->size();
  for (CFuint iElemType = 0; iElemType < nbrElemTypes; ++iElemType)
  {
    totNbrStates += (*elemType)[iElemType].getNbElems()*sdLocalData[iElemType]->getNbrOfSolPnts();
  }
  ...
  for (CFuint iElemType = 0; iElemType < nbrElemTypes; ++iElemType)
  {
    ...
    SafePtr< vector< RealVector > > solPntsLocalCoords
        = sdLocalData[iElemType]->getSolPntsLocalCoords();
...
void StdSetup::computeFaceJacobianVectorSizes()
{
  ...
  vector< SpectralFDElementData* >& sdLocalData = getMethodData().getSDLocalData();
  SafePtr< vector< RealVector > >                 faceFlxPntsFaceLocalCoords = sdLocalData[0]->getFaceFlxPntsFaceLocalCoords();
