#include "FluxReconstructionTurb/NavierStokesGReKO3DSourceTerm_Lang.hh"
...
#include "GReKO/NavierStokes3DGReKLogOPuvt.hh"
...
MethodCommandProvider<NavierStokesGReKO3DSourceTerm_Lang, FluxReconstructionSolverData, FluxReconstructionKOmegaModule>
NavierStokesGReKO3DSourceTerm_LangFRProvider("NavierStokesGReKO3DSourceTerm_Lang");

///////////////////////////////////////////////////////////////////////////////////////////////////

void NavierStokesGReKO3DSourceTerm_Lang::defineConfigOptions(Config::OptionList& options)
{
...
NavierStokesGReKO3DSourceTerm_Lang::NavierStokesGReKO3DSourceTerm_Lang(const std::string& name) :
...
NavierStokesGReKO3DSourceTerm_Lang::~NavierStokesGReKO3DSourceTerm_Lang()
{
}

//////////////////////////////////////////////////////////////////////////////////////////////////

void NavierStokesGReKO3DSourceTerm_Lang::setup()
{
...
void NavierStokesGReKO3DSourceTerm_Lang::unsetup()
{
...
std::vector< Common::SafePtr< BaseDataSocketSource > >
  NavierStokesGReKO3DSourceTerm_Lang::providesSockets()
{
...
NavierStokesGReKO3DSourceTerm_Lang::needsSockets()
{
...
void  NavierStokesGReKO3DSourceTerm_Lang::getSToStateJacobian(const CFuint iState)
{
  // reset the jacobian
  ...
  const CFreal T = max(0.0,(*((*m_cellStates)[iState]))[4]);
  ...
  const CFreal avGa    = min(max((*((*m_cellStates)[iState]))[7],0.0),1.0);
  const CFreal avRe    = max((*((*m_cellStates)[iState]))[8],20.0);
...
void  NavierStokesGReKO3DSourceTerm_Lang::getStrain(const CFreal VoverRadius, const CFuint iState)
{
...
void  NavierStokesGReKO3DSourceTerm_Lang::getVorticity(const CFuint iState)
{
  const CFreal gradU_Y = (*(m_cellGrads[iState][1]))[YY];
  const CFreal gradU_Z = (*(m_cellGrads[iState][1]))[ZZ];
  const CFreal gradV_X = (*(m_cellGrads[iState][2]))[XX];
...
void NavierStokesGReKO3DSourceTerm_Lang::addSourceTerm(RealVector& resUpdates)
{       
  ...
  SafePtr< NavierStokes3DGReKLogOPuvt > navierStokesVarSet = m_diffVarSet.d_castTo< NavierStokes3DGReKLogOPuvt >();
...
//  }
    ...
    const CFuint uID = 1;//getStateVelocityIDs()[XX];
    const CFuint vID = 2;//getStateVelocityIDs()[YY];
    const CFuint wID = 3;//getStateVelocityIDs()[YY];
    ...
    const CFreal dwx = (*(m_cellGrads[iSol][wID]))[XX]; 
    const CFreal dwy = (*(m_cellGrads[iSol][wID]))[YY]; 
    const CFreal dwz = (*(m_cellGrads[iSol][wID]))[ZZ];
...
void NavierStokesGReKO3DSourceTerm_Lang::getRethetac(const CFreal Retheta)
{	
/// errors here in FV!
//  if (Retheta <= 1860)
//  {
...
void NavierStokesGReKO3DSourceTerm_Lang::getFlength(const CFreal Retheta)
{
  if (Retheta < 400)
  {  
...
CFreal NavierStokesGReKO3DSourceTerm_Lang::getFlengthDeriv(const CFreal Retheta)
{
  CFreal deriv = 0.0;
...
void NavierStokesGReKO3DSourceTerm_Lang::getRethetat(const CFreal Tu) 
{
  cf_assert(Tu >= 0.0);   
...
CFreal NavierStokesGReKO3DSourceTerm_Lang::getRethetatDeriv(const CFreal Tu) 
{
  cf_assert(Tu >= 0.0);   
...
void NavierStokesGReKO3DSourceTerm_Lang::getLambda(CFreal& lambda, const CFreal theta, const CFreal viscosity, const CFuint iState)
{
  SafePtr< NavierStokes3DGReKLogOPuvt > navierStokesVarSet = m_diffVarSet.d_castTo< NavierStokes3DGReKLogOPuvt >();
  ...
  const CFreal avw = m_solPhysData[EulerTerm::VZ];   
  const CFreal overU2 = 1./(avV*avV);//1./avV;
  
  // error in FV!!
  const CFreal dUdsTerm = avu*avu*(*(m_cellGrads[iState][1]))[XX] + avv*avv*(*(m_cellGrads[iState][2]))[YY] + avw*avw*(*(m_cellGrads[iState][3]))[ZZ] + avu*avv*((*(m_cellGrads[iState][1]))[YY] + (*(m_cellGrads[iState][2]))[XX]) + avu*avw*((*(m_cellGrads[iState][1]))[ZZ] + (*(m_cellGrads[iState][3]))[XX]) + avw*avv*((*(m_cellGrads[iState][3]))[YY] + (*(m_cellGrads[iState][2]))[ZZ]);
...
void NavierStokesGReKO3DSourceTerm_Lang::getFlambda(const CFreal lambda, const CFreal Tu, CFreal& Flambda, const CFreal theta, bool Prime )
{
  // error in FV!!
...
void NavierStokesGReKO3DSourceTerm_Lang::getRethetatwithPressureGradient(const CFreal viscosity, const CFreal Tu, const CFuint iState)
{
  SafePtr< NavierStokes3DGReKLogOPuvt > navierStokesVarSet = m_diffVarSet.d_castTo< NavierStokes3DGReKLogOPuvt >();
