...
#include "UFEM/TetraP1P1Cell/LES.hh"
...
COOLFluiD::Common::SafePtr< COOLFluiD::UFEM::TetraP1P1Cell::LES > LESInstance = CFNULL;
...
Environment::ObjectProvider < LES,
                              ...
                              UFEMTerm::NARGS >
aTetraP1P1Cell_LES_Provider ( "TetraP1P1Cell_LES" );
...
void LES::grad_variable ( unsigned int nbvars, double* variable_array_in, double* variable_array_out )
{
  ...
  for (unsigned int ivar=0; ivar<nbvars; ivar++){
    ...
    variable_array_out[2]=0.;
    for (unsigned int inod=0; inod<4; inod++){
      ...
      variable_array_out[2]+=variable_array_in[inod*nbvars+ivar]*(double)nz[inod];
    }
    variable_array_out[0]*=invvol;
    variable_array_out[1]*=invvol;
    variable_array_out[2]*=invvol;
    variable_array_out=&variable_array_out[3];
  }
}
