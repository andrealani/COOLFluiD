...
#include "UFEM/TriagP1P1Cell/StandardKEpsilon.hh"
...
Environment::ObjectProvider < StandardKEpsilon,
                              ...
                              UFEMTerm::NARGS >
aTriagP1P1Cell_StandardKEpsilon_Provider ( "TriagP1P1Cell_StandardKEpsilon" );
...
Node& node1 = *nodes[1];
Node& node2 = *nodes[2];
CFLog(INFO,"y coord=" << (node0[1]+node1[1]+node2[1])/3. << " dist=" << wallNearestDistance[iCell] << " wallgrad=" << wallNearestVelocityGradient[i] << "\n");
*/
...
#define kronecker(i,j) ((i)==(j)?1.:0.)
  ...
  CFreal dudx=0.,dudy=0.,dvdx=0.,dvdy=0.,dKdx=0.,dKdy=0.,dEPSdx=0.,dEPSdy=0.;
  ...
  for (CFuint iState=0; iState<3; ++iState) {
    ...
    const CFreal mut=RhoElm*m_Cmu*interState[3]*interState[3]/interState[4];

    if ((mut<bottomlimit)||(interState[3]<m_BottomPrec)||(interState[4]<m_BottomPrec)) {
      if (interState[3]<m_BottomPrec) interState[3]=m_BottomPrec;
      if (interState[4]<m_BottomPrec) interState[4]=m_BottomPrec;
      const CFreal klime=sqrt(bottomlimit*interState[4]/m_Cmu);
      if (interState[3]<klime) {
        // no change interState[3]
        interState[4]=m_Cmu*interState[3]*interState[3]/bottomlimit;
        //cout << "k ";
    ...
    if ((mut>toplimit)||(interState[3]>m_TopPrec)||(interState[4]>m_TopPrec)){
      if (interState[3]>m_TopPrec) interState[3]=m_TopPrec;
      if (interState[4]>m_TopPrec) interState[4]=m_TopPrec;
      const CFreal klime=sqrt(toplimit*interState[4]/m_Cmu);
      if (interState[3]<klime) {
        // no change interState[3]
        interState[4]=m_Cmu*interState[3]*interState[3]/bottomlimit;
        //cout << "K ";
    ...
    dudx      +=interState[1]*(double)nx[iState];
    dudy      +=interState[1]*(double)ny[iState];
    dvdx      +=interState[2]*(double)nx[iState];
    dvdy      +=interState[2]*(double)ny[iState];
    dKdx      +=interState[3]*(double)nx[iState];
    dKdy      +=interState[3]*(double)ny[iState];
    dEPSdx    +=interState[4]*(double)nx[iState];
    dEPSdy    +=interState[4]*(double)ny[iState];
    for (CFuint iEq=0; iEq<nbEqs; ++iEq) {
      estate[iEq] += interState[iEq];
    }
  }
...
Node& node1 = *nodes[1];
Node& node2 = *nodes[2];
if (((node0[0]+node1[0]+node2[0])/3.>1.954)&&((node0[0]+node1[0]+node2[0])/3.<1.955)){
...
CFreal upluslog=log(yplus)/0.41+5.1;
  ...
  CFuint Pi,Ui,Vi,Ki,EPSi,Pj,Uj,Vj,Kj,EPSj;
  CFreal val;
  CFreal S2 = 2.*((dudx*dudx)+(dvdy*dvdy))+((dudy+dvdx)*(dudy+dvdx));

  // SUPG
  CFreal umag=sqrt(estate[1]*estate[1]+estate[2]*estate[2]);
  CFreal tauSUPGK[2]={0.,0.},tauSUPGEPS[2]={0.,0.};
  ...
  // Nonlinear SUPG 
  CFreal uparK[2]={0.,0.};
  CFreal normgradK[2]={0.,0.};
  CFreal absgradK=sqrt(dKdx*dKdx+dKdy*dKdy);
  if (fabs(absgradK)>1.e-10){
    normgradK[0]=dKdx/absgradK;
    normgradK[1]=dKdy/absgradK;
    uparK[0]=(estate[1]*normgradK[0]+estate[2]*normgradK[1])*normgradK[0];
    uparK[1]=(estate[1]*normgradK[0]+estate[2]*normgradK[1])*normgradK[1];
  }
  CFreal uparKmag=sqrt(uparK[0]*uparK[0]+uparK[1]*uparK[1]);

  CFreal uparEPS[2]={0.,0.};
  CFreal normgradEPS[2]={0.,0.};
  CFreal absgradEPS=sqrt(dEPSdx*dEPSdx+dEPSdy*dEPSdy);
  if (fabs(absgradEPS)>1.e-10){
    normgradEPS[0]=dEPSdx/absgradEPS;
    normgradEPS[1]=dEPSdy/absgradEPS;
    uparEPS[0]=(estate[1]*normgradEPS[0]+estate[2]*normgradEPS[1])*normgradEPS[0];
    uparEPS[1]=(estate[1]*normgradEPS[0]+estate[2]*normgradEPS[1])*normgradEPS[1];
  }
  CFreal uparEPSmag=sqrt(uparEPS[0]*uparEPS[0]+uparEPS[1]*uparEPS[1]);
...
//  tauSUPG*=0.;
  ...
  Pi=0; Ui=1; Vi=2; Ki=3; EPSi=4;
  for(CFuint i=0; i<3; ++i) {

    //CFreal u_ni   = estate[1]*celldata.nx[i]+estate[2]*celldata.ny[i];
    CFreal tauSUPGK_u_ni = tauSUPGK[0]*celldata.nx[i]+tauSUPGK[1]*celldata.ny[i];
    CFreal tauSUPGEPS_u_ni = tauSUPGEPS[0]*celldata.nx[i]+tauSUPGEPS[1]*celldata.ny[i];

    Pj=0; Uj=1; Vj=2; Kj=3; EPSj=4;
    for(CFuint j=0; j<3; ++j) {
      ...
      for(CFuint k=0; k<3; ++k){
        ...
        // Convection K (Standard + SUPG)
        val  = 1./24.*uknj*(1.+kronecker(i,k));
        val += tauSUPGK_u_ni/(12.*celldata.vol)*uknj;
        adata.A(Ki,Kj) += val;

        // Convection E (Standard + SUPG)
        val  = 1./24.*uknj*(1.+kronecker(i,k));
        val += tauSUPGEPS_u_ni/(12.*celldata.vol)*uknj;
        adata.A(EPSi,EPSj) += val;

      }

      //diffusion K (Standard)
      val = (nuk)/(4.*celldata.vol)*(celldata.nx[i]*celldata.nx[j]+celldata.ny[i]*celldata.ny[j]);
      adata.A(Ki,Kj) += val;

      //diffusion E (Standard)
      val = (nueps)/(4.*celldata.vol)*(celldata.nx[i]*celldata.nx[j]+celldata.ny[i]*celldata.ny[j]);
      adata.A(EPSi,EPSj) += val;
      ...
      Pj+=nbEqs; Uj+=nbEqs; Vj+=nbEqs; Kj+=nbEqs; EPSj+=nbEqs;
    }
    Pi+=nbEqs; Ui+=nbEqs; Vi+=nbEqs; Ki+=nbEqs; EPSi+=nbEqs;
  }
