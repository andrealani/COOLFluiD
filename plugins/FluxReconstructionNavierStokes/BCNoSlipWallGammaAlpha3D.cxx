...
#include "FluxReconstructionNavierStokes/BCNoSlipWallGammaAlpha3D.hh"
...
Framework::MethodStrategyProvider<
    BCNoSlipWallGammaAlpha3D,FluxReconstructionSolverData,BCStateComputer,FluxReconstructionNavierStokesModule >
  BCNoSlipWallGammaAlpha3DProvider("BCNoSlipWallGammaAlpha3D");

//////////////////////////////////////////////////////////////////////////////

void BCNoSlipWallGammaAlpha3D::defineConfigOptions(Config::OptionList& options)
{
...
BCNoSlipWallGammaAlpha3D::BCNoSlipWallGammaAlpha3D(const std::string& name) :
...
BCNoSlipWallGammaAlpha3D::~BCNoSlipWallGammaAlpha3D()
{
  CFAUTOTRACE;
}
...
void BCNoSlipWallGammaAlpha3D::computeGhostStates(const vector< State* >& intStates,
...
//  }
      ...
      //CFLog(INFO, "is turb: " << transitionCriterion(iState) << ", transition occurred: " << m_transitionOccurred << "\n");
      if ((transitionCriterion(iState) || (m_intSolPhysData[iK+2] > m_gammaThreshold)) && !m_enforceLaminar && coords[iState][XX]>m_leadingEdgeLaminarEnforce)// && coords[iState][XX]>0.1)// && m_transitionOccurred))
      {
        // gamma
...
void BCNoSlipWallGammaAlpha3D::computeGhostGradients
...
void BCNoSlipWallGammaAlpha3D::computeBndGradVars(const std::vector< RealVector* >& gradVarsFlxPnt,
                                                  ...
                                                  std::vector< RealVector* >& bndGradVars)
{
  ...
  // wall, and KWall, the wall value of log-omega and gamma the ghost state carries
  for (CFuint iState = 0; iState < nbrStates; ++iState)
  {
    ...
    // gamma: the ghost value, 0.01 for a laminar wall, the interior value once transition occurred
    if (nbTurbVars == 4)
    {
      bndGradVarsState[iK+2] = ghostState[iK+2];
    }
  }
}
...
void BCNoSlipWallGammaAlpha3D::computeBndStates(const std::vector< Framework::State* >& intStates,
                                                const std::vector< Framework::State* >& ghostStates,
                                                const std::vector< RealVector >& unitNormals,
                                                const std::vector< RealVector >& flxPntCoords,
...
void BCNoSlipWallGammaAlpha3D::computeBndGrads(const std::vector< std::vector< RealVector* > >& intGrads,
...
void BCNoSlipWallGammaAlpha3D::constrainBndGrads(const RealVector& bndState,
...
void BCNoSlipWallGammaAlpha3D::setup()
{
  CFAUTOTRACE;
