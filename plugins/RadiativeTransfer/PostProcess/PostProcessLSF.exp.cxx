...
Environment::ObjectProvider<PostProcessLSFexp, PostProcess, RadiativeTransferModule, 1 >
PostProcessLSFProvider("PostProcessLSFexp");
...
void PostProcessLSFexp::configure ( Config::ConfigArgs& args )
{
  ConfigObject::configure(args);
}

PostProcessLSFexp::PostProcessLSFexp(const string &name):
  PostProcess(name)
{
...
PostProcessLSFexp::~PostProcessLSFexp()
{
}

void PostProcessLSFexp::runPostProcess(vector<CFreal> dataVector){
  ...
  for(CFuint c=0; c<nCells; ++c){
    ...
    if (cell->getState(0)->isParUpdatable()){
      ...
      if(!isRegected){
        ...
        cellPos.push_back( std::abs(distCell)  );
