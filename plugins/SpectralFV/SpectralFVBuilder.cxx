...
Environment::ObjectProvider<SpectralFVBuilder,
               MeshDataBuilder,
               SpectralFVModule,
               1>
StdBuilderProvider("StdBuilder");

//////////////////////////////////////////////////////////////////////////////

SpectralFVBuilder::SpectralFVBuilder(const std::string& name) :
...
SpectralFVBuilder::~SpectralFVBuilder()
{
}

//////////////////////////////////////////////////////////////////////////////

void SpectralFVBuilder::configure ( Config::ConfigArgs& args )
{
...
void SpectralFVBuilder::releaseMemory()
{
...
void SpectralFVBuilder::setMapGeoToTrs()
{
  /// @todo implement if you want to have implicit because the face information
  ///       is needed to compute the sparsity of the matrix
}

//////////////////////////////////////////////////////////////////////////////

void SpectralFVBuilder::createTopologicalRegionSets()
{
  CFAUTOTRACE;
...
void SpectralFVBuilder::computeGeoTypeInfo()
{
  CFAUTOTRACE;

  // set the polynomial type
  getCFmeshData().setSolutionPolyType(CFPolyForm::SPECTRALFV);
...
void SpectralFVBuilder::createCellFaces()
{
  CFAUTOTRACE;
...
void SpectralFVBuilder::createInnerFacesTRS()
{
  CFAUTOTRACE;
  ...
  CFLog(NOTICE,"Created SFVM TRS InnerFaces with "
        << innerFaceTRS->getLocalNbGeoEnts() << " local faces\n");
}

//////////////////////////////////////////////////////////////////////////////

void SpectralFVBuilder::reorderInnerFacesTRS()
{
  CFAUTOTRACE;

  // Pointer to list containing the GeoType of the face ==> class variable of SpectralFVBuilder, we can access this variable directly through m_inGeoTypes
  ...
  // Get the number of different face types in mesh (their should only be one type for SpectralFV for now!!)
  ...
  // Check if only one face type present (check for cell is done before)
  if (faceGeoTypes.size() != 1) throw Common::NotImplementedException (FromHere(),"For now, there can be only one type of face in the mesh, since only triangles (2D) and tetrahedrons (3D) are supported");

  // Check if cells have one node more than faces
  // This should always be the case for now, only linear, triangular or tetrahedral cells are used
  cf_assert(innerFaceNodes->nbCols(0)+1 == cellNodes->nbCols(0));

  // Get vector containing the nodes connectivities for each orientation
  const CFuint nbrNodesPerFace = innerFaceNodes->nbCols(0);
  vector< vector < vector < CFuint > > > nodeConnPerOrientation
                                              = getNodeConnPerOrientation(nbrNodesPerFace);
  ...
  for (CFuint iOrient = 0; iOrient < nbrOrient; ++iOrient)
  {
    ...
    for (CFuint iFace = faceIdx; iFace < nbInnerFaces; ++iFace)
    {
      ...
      CFuint leftCellID  = (*innerFaceCells)(iFace,0);
      CFuint rightCellID = (*innerFaceCells)(iFace,1);
      ...
      {
        const CFuint leftCellNodeLocalID  = nodeConnPerOrientation[iOrient][0][iNode];
        const CFuint rightCellNodeLocalID = nodeConnPerOrientation[iOrient][1][iNode];
      ...
      if (hasThisOrient)
      {
        ...
        for (CFuint iNode = 0; iNode < nbrNodesPerFace; ++iNode)
        {
          (*innerFaceNodes)(faceIdx,iNode) = leftCellFaceNodes[iNode];
        }
...
vector< vector < vector < CFuint > > > SpectralFVBuilder::getNodeConnPerOrientation(const CFuint dim)
{
  CFAUTOTRACE;

  // Create vector containing the nodes connectivities for each orientation
  vector< vector < vector < CFuint > > > nodeConnPerOrientation;

  switch (dim)
  {
    case DIM_1D: // 1D problem, faces are nodes
    {
      SpectralFVElementData* svElemData =
                                    new LineSpectralFVElementData(CFPolyOrder::ORDER0); // the order does not matter here
      nodeConnPerOrientation = *svElemData->getSVFaceNodeConnPerOrientation();
      delete svElemData;
    } break;
    case DIM_2D: // 2D problem, faces are lines
    {
      SpectralFVElementData* svElemData =
                                    new TriagSpectralFVElementData(CFPolyOrder::ORDER0); // the order does not matter here
      nodeConnPerOrientation = *svElemData->getSVFaceNodeConnPerOrientation();
      delete svElemData;
    } break;
    case DIM_3D: // 3D problem, faces are triangles
    {
      SpectralFVElementData* svElemData =
                                    new TetraSpectralFVElementData(CFPolyOrder::ORDER0); // the order does not matter here
      nodeConnPerOrientation = *svElemData->getSVFaceNodeConnPerOrientation();
    } break;
    default:
    {
      std::string message = "Faces have " + StringOps::to_str(dim) + " nodes. Only faces with 1 (for 1D), 2 (for 2D) and 3 (for 3D, triangular faces) nodes are supported...";
      throw Common::ShouldNotBeHereException (FromHere(),message);
    }
  }
...
void SpectralFVBuilder::setMaxNbStatesInCell()
{
...
void SpectralFVBuilder::setMaxNbNodesInCell()
{
  SafePtr<vector<ElementTypeData> > elementType = getCFmeshData().getElementTypeData();
...
void SpectralFVBuilder::setMaxNbFacesInCell()
{
  SafePtr<vector<ElementTypeData> > elementType = getCFmeshData().getElementTypeData();
...
void SpectralFVBuilder::createBoundaryFacesTRS()
{
  CFAUTOTRACE;
...
SpectralFVBuilder::createTopologicalRegionSet
 ...
 const CFuint iTRS)
{
  ...
  for (CFuint iTR = 0; iTR < nbTRs; ++iTR)
  {
    ...
    if (nbFaces > 0)
    {
      ...
      for (CFuint iFace = 0; iFace < nbFaces; ++iFace, ++nbProcessedFaces)
      {
        ...
        const std::string faceGeoTypeName = makeGeomEntName(getGeoShape(CFGeoEnt::FACE, dim, nbFaceNodes),
                                                         getGeometricPolyType(),
                                                         getGeometricPolyOrder(),
                                                         getSolutionPolyType(),
                                                         getSolutionPolyOrder());
  ...
  CFLogDebugMin( "SpectralFVBuilder::setTopologicalRegions() : nameTRS "
                 << name
...
void SpectralFVBuilder::createPartitionBoundaryFacesTRS()
{
  CFAUTOTRACE;
...
vector< vector < CFuint > > SpectralFVBuilder::getBFaceOrientations(CFuint dim)
{
  CFAUTOTRACE;
  ...
  switch (dim)
  {
    case DIM_1D: // 1D problem, faces are nodes
    {
      SpectralFVElementData* svElemData =
                                    new LineSpectralFVElementData(CFPolyOrder::ORDER0); // the order does not matter here
      bFaceOrientations = *svElemData->getSVFaceNodeConn();
      delete svElemData;
    } break;
    case DIM_2D: // 2D problem, faces are lines
    {
...
void SpectralFVBuilder::reorderBoundaryFacesTRS() // this routine will have to be altered for 3D
{
  CFAUTOTRACE;
  ...
  // get the number of faces to each element
  SafePtr<vector<ElementTypeData> > elementType = getCFmeshData().getElementTypeData();
  cf_assert(elementType->size() == 1); // there should be only one element type
  const CFuint nbCellFaces = (*elementType)[0].getNbFaces();

  // get number of face nodes (since only simplices are used, this is the number of faces to each cell minus 1)
  const CFuint nbFaceNodes = nbCellFaces-1;

  // get possible face orientations
  vector< vector< CFuint > > bFaceOrientations = getBFaceOrientations(nbFaceNodes);
        ...
        for (CFuint iBFace = faceIdx; iBFace < stopFaceIdx; ++iBFace)
        {
          ...
          if (hasThisOrient)
          {
            ...
            for (CFuint iNode = 0; iNode < nbFaceNodes; ++iNode)
            {
              ...
              (*bFaceNodeConn)(faceIdx,iNode) = faceNodes[iNode];
            }
