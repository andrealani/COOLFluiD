...
class RunRoeFluxBlitz : public BaseRunLib
{
  public:
  ...
  RunRoeFluxBlitz () : tmpv(SIZE), rEV(SIZE,SIZE), lEV(SIZE,SIZE), eV(SIZE), reV(SIZE), leV(SIZE), abseV(SIZE), absJ(SIZE,SIZE), rflux(SIZE), lflux(SIZE)
  {
  };
    ...
    void compute()
  {
    ...
    using namespace blitz;
      ...
      for(int i = 0; i < nbelems; ++i)
      {
        ...
        absJ = sum(rEV(i,k) * lEV(k,j), k);

        rflux = 10.0;
        lflux = 20.0;

        tmpv = rstate - lstate;
        flux = 0.5 * ( (rflux + lflux)  - sum(absJ(i,j)*tmpv(j),j) );
