...
class RunRoeFluxCFLET : public BaseRunLib
{
  public:

  typedef RunRoeFluxCFLET<TYPE,SIZE> SELF;
  typedef COOLFluiD::MathTools::CFMat<TYPE>   GEMatrix;
  typedef COOLFluiD::MathTools::CFVec<TYPE>   DEVector;

  RunRoeFluxCFLET () : trstate(SIZE), tlstate(SIZE), Tvec(SIZE), TTvec(SIZE)
  {
  };
    ...
    void compute()
  {
      for(int i = 0; i < nbelems; ++i)
      {
        ...
        for (int j = 0; j<SIZE; ++j) { rflux[j] = 10.0; }
        for (int j = 0; j<SIZE; ++j) { lflux[j] = 20.0; }

//        trstate.setPtr(*R[i]); tlstate.setPtr(*L[i]);
        Tvec = rstate - lstate;
        TTvec.setPtr(Tvec);

        flux = (rflux + lflux) - ( absJ * TTvec );
        flux *= 0.5;
// TTvec.release();
