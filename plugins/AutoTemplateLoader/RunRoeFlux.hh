...
class RunRoeFlux : public BaseRunLib
{
  public:
  ...
  RunRoeFlux () : rEV(SIZE,SIZE), lEV(SIZE,SIZE), eV(SIZE), reV(SIZE), leV(SIZE), abseV(SIZE), absJ(SIZE,SIZE), rflux(SIZE), lflux(SIZE)
  {
  };
    ...
    void compute()
  {
      for(unsigned int i = 0; i < nbelems; ++i)
      {
        ...
        flux = 0.5 * ( (rflux + lflux)  - absJ * (rstate - lstate));
