...
MHD3DProjectionVarSet::MHD3DProjectionVarSet(Common::SafePtr<Framework::BaseTerm> term) :
...
MHD3DProjectionVarSet::~MHD3DProjectionVarSet()
{
}

//////////////////////////////////////////////////////////////////////////////

void MHD3DProjectionVarSet::computeFlux (const RealVector& data,
...
void MHD3DProjectionVarSet::computeStateFlux (const RealVector& data)
{
  const CFreal rho = data[MHDTerm::RHO];
  const CFreal u = data[MHDTerm::VX];
  const CFreal v = data[MHDTerm::VY];
...
void MHD3DProjectionVarSet::setTransformationMatrices(const RealVector& coords,
...
RealVector& MHD3DProjectionVarSet::computeTanakaFluxPowell99Formulation(const RealVector& data,
									const RealVector& normals)
{
  ...
  if (potentialBType == "Dipole") {
    computeMagneticDipole(data[MHDProjectionTerm::XP],data[MHDProjectionTerm::YP],data[MHDProjectionTerm::ZP]);
    Bx0 = _BDipole[0];
    By0 = _BDipole[1];
    Bz0 = _BDipole[2];
  }
...
void MHD3DProjectionVarSet::computePFSSMagneticField(const RealVector& stateCoordsSpherical,
...
CFreal MHD3DProjectionVarSet::getMaxEigenValue(const RealVector& data,
					       const RealVector& normal)
{
  ...
  if (potentialBType == "PFSS") {
    computePFSSPotentialField(Bx0, By0, Bz0);
  }  
...
CFreal MHD3DProjectionVarSet::getMaxAbsEigenValue(const RealVector& data,
						  const RealVector& normal)
{
   // AL: included B0 contribution which was missing
  const std::string& potentialBType = getModel()->getPotentialBType();
...
void MHD3DProjectionVarSet::computeMagneticDipole(CFreal xCoord, CFreal yCoord, CFreal zCoord)
{
  ...
  _BDipole[XX] = rCubeInv*(coeff*xCoord - mX);
  _BDipole[YY] = rCubeInv*(coeff*yCoord - mY);
  _BDipole[ZZ] = rCubeInv*(coeff*zCoord - mZ);
}

//////////////////////////////////////////////////////////////////////////////

void MHD3DProjectionVarSet::computeEigenValues(const RealVector& data,
					       const RealVector& normal,
					       RealVector& result)
{
...
void MHD3DProjectionVarSet::computePFSSPotentialField(CFreal& Bx0, CFreal& By0, CFreal& Bz0)
{
  std::string nsp = MeshDataStack::getActive()->getPrimaryNamespace();
  std::string datahandleName2 = nsp + "_BPFSS";
