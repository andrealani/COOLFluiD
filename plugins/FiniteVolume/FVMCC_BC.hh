 ...
 *
 */
class FVMCC_BC : public CellCenterFVMCom {
...
public: // functions
  ...
  { 
    ...
    const std::vector<std::string>& trsNames = getMethodData().getTRSsWithGhostsOnFace(); 
    for (CFuint i = 0; i < trsNames.size(); ++i) {
      if (trsNames[i] == getCurrentTRS()->getName()) {
...
}; // end of class FVMCC_BC

//////////////////////////////////////////////////////////////////////////////

template <typename T1, typename T2> 
void FVMCC_BC::repositionNode(const T1& innerValue, T1& ghostValue, 
                              const T2& bValue, const T2& minValue) 
{ 
  using std::cout; 
  using std::endl; 
  ...
  if (applyFix) { 
    ...
    if (check < 0.) {
      cout << "### FVMCC_BC::repositionNode() failed => n*(G-M) = " << check << " < 0" << endl;
