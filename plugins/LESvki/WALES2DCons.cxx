...
Environment::ObjectProvider<WALES2DCons, DiffusiveVarSet,
	       LESvkiModule, 2>
WALEScons2DConsProvider("WALES2DCons");

//////////////////////////////////////////////////////////////////////////////

WALES2DCons::WALES2DCons
(const std::string& name, Common::SafePtr<Framework::PhysicalModelImpl> model) :
  WALES2DVarSet(name, model),
...
void WALES2DCons::setGradientVars(const vector<RealVector*>& states,
					 RealMatrix& values,
...
void WALES2DCons::setGradientVarGradients(const vector<RealVector*>& states,
...
void WALES2DCons::setStateGradients(const vector<RealVector*>& states,
...
CFreal WALES2DCons::getDynViscosity(const RealVector& state,
...
CFreal WALES2DCons::getDensity(const RealVector& state)
{
  return state[0];
}
...
void WALES2DCons::setGradientState(const RealVector& state)
{
...
////////////////////////////////////////////////////////////////////////////////
CFreal WALES2DCons::getTurbDynViscosityFromGradientVars(const RealVector& state, const vector<RealVector*>& gradients)
{
  const CFreal dim = 3.0;
 const RealVector& gradU = *gradients[1];
  ...
  const CFreal SdXX = (gradU[XX]*gradU[XX]+gradU[YY]*gradV[XX])+(gradU[XX]*gradU[XX]+2.0*gradV[XX]*gradU[YY]+gradV[YY]*gradV[YY])/dim;
  const CFreal SdXY = 0.5 * (gradU[XX]*gradU[YY]+gradU[YY]*gradV[YY]+gradV[XX]*gradU[XX]+gradV[YY]*gradV[XX]);
  const CFreal SdYY = (gradV[YY]*gradV[YY]+gradV[XX]*gradU[YY])+(gradU[XX]*gradU[XX]+2.0*gradV[XX]*gradU[YY]+gradV[YY]*gradV[YY])/dim;

  const CFreal mut = pow(Cw,2)*pow(SdXX*SdXX+2*SdXY*SdXY+SdYY*SdYY,3./2.)/(pow(SXX*SXX+2.*SXY*SXY+SYY*SYY,5./2.)+pow(SdXX*SdXX+2*SdXY*SdXY+SdYY*SdYY,5./4.));
