...
void Smagorinsky2DVarSet::setup()
{
...
RealVector& Smagorinsky2DVarSet::getFlux(const RealVector& state,
                                          ...
                                          const CFreal& radius)
{
  ...
  const CFreal tauSGSdYY = -2.*fltRho*nuSGS*(gradV[YY]-(gradU[XX]+gradV[YY])/dim);
...
RealMatrix& Smagorinsky2DVarSet::getFlux(const RealVector& state,
                                          const vector<RealVector*>& gradients,
                                          const CFreal& radius)
{
  setGradientState(state);
...
void Smagorinsky2DVarSet::getAxiSourceTerm(const RealVector& physicalData,
...
CFreal Smagorinsky2DVarSet::getHeatFlux(const RealVector& state,
					 const vector<RealVector*>& gradients,
					 const RealVector& normal)
{
