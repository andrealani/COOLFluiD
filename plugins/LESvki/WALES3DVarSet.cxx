...
RealVector& WALES3DVarSet::getFlux(const RealVector& state,
...
RealMatrix& WALES3DVarSet::getFlux(const RealVector& state,
                                          const vector<RealVector*>& gradients,
                                          const CFreal& radius)
{
  setGradientState(state);
...
void WALES3DVarSet::getAxiSourceTerm(const RealVector& physicalData,
...
CFreal WALES3DVarSet::getHeatFlux(const RealVector& state,
					 const vector<RealVector*>& gradients,
					 const RealVector& normal)
{
