...
RealVector& Gradient2DVarSet::getFlux(const RealVector& state,
                                          ...
                                          const CFreal& radius)
{
  ...
  const CFreal a22 = 1/12 * pow(D,2) * (gradV[XX]*gradV[XX]+gradV[YY]*gradV[YY]); 
   ...
   const CFreal tauSGSdXX = fltRho * a11;
  const CFreal tauSGSdXY = fltRho * a12;
  const CFreal tauSGSdYY = fltRho * a22;
...
RealMatrix& Gradient2DVarSet::getFlux(const RealVector& state,
                                          const vector<RealVector*>& gradients,
                                          const CFreal& radius)
{
  setGradientState(state);
...
void Gradient2DVarSet::getAxiSourceTerm(const RealVector& physicalData,
...
CFreal Gradient2DVarSet::getHeatFlux(const RealVector& state,
					 const vector<RealVector*>& gradients,
					 const RealVector& normal)
{
