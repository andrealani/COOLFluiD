...
Environment::ObjectProvider<NavierStokes2DGReKOBSLPuvt, DiffusiveVarSet,
	       GReKOModule, 2>
ns2DGReKOBSLPuvtProvider("NavierStokes2DGReKOBSLPuvt");

//////////////////////////////////////////////////////////////////////////////

NavierStokes2DGReKOBSLPuvt::NavierStokes2DGReKOBSLPuvt
(const std::string& name, SafePtr<PhysicalModelImpl> model) :
  NavierStokes2DGReKOPuvt(name, model)
{
}
...
void NavierStokes2DGReKOBSLPuvt::computeBlendingCoefFromGradientVars(const RealVector& state, const RealVector& gradK, const RealVector& gradOmega)
{

  const CFreal rho = getDensity(state);
  const CFreal K = max(0., state[4]);
  ...
  const CFreal CD_kw = std::max(2.* rho * _sigmaOmega2 * overOmega * (gradK[XX]*gradOmega[XX] + gradK[YY]*gradOmega[YY]), 1.e-20);
