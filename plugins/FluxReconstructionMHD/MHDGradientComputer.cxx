...
#include "FluxReconstructionMHD/MHDGradientComputer.hh"
...
MethodCommandProvider< MHDGradientComputer,
		       FluxReconstructionSolverData,
		       FluxReconstructionMHDModule >
mhdGradientComputerProvider("ConvRHSMHD");
  
//////////////////////////////////////////////////////////////////////////////
  
MHDGradientComputer::MHDGradientComputer(const std::string& name) :
...
void MHDGradientComputer::computeGradients()
{ 
  ...
  { 
    ...
    for (CFuint iSolPnt = 0; iSolPnt < m_nbrSolPnts; ++iSolPnt)
    {
      const RealVector transformedState = static_cast<RealVector&>(*m_updateToSolutionVecTrans->transform((*m_cellStates)[iSolPnt]));
...
void MHDGradientComputer::computeGradientFaceCorrections()
{
  ...
  else if ((getMethodData().getUpdateVarStr() == "Puvt" || getMethodData().getUpdateVarStr() == "RhoivtTv") && getMethodData().hasArtificialViscosity())
  {
	...
	m_projectedCorrL = (avgSol-transformededStateL[iEq])*m_faceJacobVecSizeFlxPnts[iFlx][LEFT]*m_unitNormalFlxPnts[iFlx];
	m_projectedCorrR = (avgSol-transformededStateR[iEq])*m_faceJacobVecSizeFlxPnts[iFlx][RIGHT]*m_unitNormalFlxPnts[iFlx];
...
void MHDGradientComputer::setup()
{
  CFAUTOTRACE;
