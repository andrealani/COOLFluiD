...
#include "LinEuler/CreateDampingCoefficient.hh"
...
MethodCommandProvider<CreateDampingCoefficient,
                      DataProcessingData,
                      LinearizedEulerModule>
aCreateDampingCoefficientProvider("CreateDampingCoefficient");

//////////////////////////////////////////////////////////////////////////////

void CreateDampingCoefficient::defineConfigOptions(Config::OptionList& options)
{
...
CreateDampingCoefficient::CreateDampingCoefficient(const std::string& name) :
...
CreateDampingCoefficient::~CreateDampingCoefficient()
{
}
...
CreateDampingCoefficient::needsSockets()
{
...
CreateDampingCoefficient::providesSockets()
{
...
void CreateDampingCoefficient::setup()
{
  DataHandle < Framework::State*, Framework::GLOBAL > states = socket_states.getDataHandle();
  DataHandle<CFreal> dampingCoeff = socket_dampingCoeff.getDataHandle();
  dampingCoeff.resize(states.size());

  // Initialize the damping coefficient with zeros (This has to be done exactly once, at the beginning during setup)
...
void CreateDampingCoefficient::executeOnTrs()
{

  SafePtr<TopologicalRegionSet> trs = getCurrentTRS(); //modify this later
  CFLogDebugMin( "CreateDampingCoefficient::executeOnTrs() called for TRS: " << trs->getName() << "\n");
  ...
  for(CFuint iBoundaryTRS = 0; iBoundaryTRS < m_BoundaryTRS.size(); ++iBoundaryTRS)
  {
    ...
    for (CFuint iBoundaryState = 0; iBoundaryState < boundaryTrs->getNbStatesInTrs(); ++iBoundaryState)
    {
      ...
      for (CFuint iState = 0; iState < states.size(); iState++) //is it rather ++iState?
      {
        // Obtain coordinates for the given node
...
void CreateDampingCoefficient::unsetup()
{
  DataHandle<CFreal> dampingCoeff = socket_dampingCoeff.getDataHandle();
