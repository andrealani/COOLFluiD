...
MethodCommandProvider<SuperInletPhi, CellCenterFVMData, FiniteVolumeMultiFluidMHDModule> superInletPhiFVMCCProvider("SuperInletPhiFVMCC");
                                        
//////////////////////////////////////////////////////////////////////////////

void SuperInletPhi::defineConfigOptions(Config::OptionList& options)
{
...
void SuperInletPhi::configure ( Config::ConfigArgs& args )
{
  using namespace COOLFluiD::Framework;
...
void SuperInletPhi::setGhostState(GeometricEntity *const face)
{
  const CFuint nbSpecies = _updateVarSet->getModel()->getNbScalarVars(0);
 ...
 if(!is2DHalf) {
   for (CFuint i = 0 ; i < nbSpecies; i++){
     (*ghostState)[endEM + nbSpecies + 2*i]     = 2.*_variables[endEM + nbSpecies + 2*i] - (*innerState)[endEM + nbSpecies + 2*i];
     (*ghostState)[endEM + nbSpecies + 2*i + 1] = 2.*_variables[endEM + nbSpecies + 2*i + 1] - (*innerState)[endEM + nbSpecies + 2*i + 1];
   } 
   //set the Temperatures
   for (CFuint i = 0 ; i < nbSpecies; i++){
     (*ghostState)[endEM + nbSpecies + 2*nbSpecies + i] = 2.*_variables[endEM + nbSpecies + 2*nbSpecies + i] - (*innerState)[endEM + nbSpecies + 2*nbSpecies + i];
 ...
 if(_isSubsonic){ //Electrons assumed to be the first fluid
   if(!is2DHalf) {
     ...
     //set the Temperatures
     (*ghostState)[endEM + nbSpecies + 2*nbSpecies ] = 2.*_variables[endEM + nbSpecies + 2*nbSpecies ] - (*innerState)[endEM + nbSpecies + 2*nbSpecies ];
   }
   else {
     (*ghostState)[endEM] = (*innerState)[endEM];
     (*ghostState)[endEM + nbSpecies ]     = 2.*_variables[endEM + nbSpecies ] - (*innerState)[endEM + nbSpecies ];
