...
//YM: PhysicalModelStack::getActive()->getImplementor()->is2DHalf()
 ...
 if (nbEqs == totalNbEqs || iEqSS == 0) {
  ...
  if (nbEqs == totalNbEqs || iEqSS == 1) {
	...
	for (CFuint i = 0; i < nbSpecies; ++i) {
	  ...
	  _flux[m_uID[i]] = m_tau[i](XX,XX)*normal[XX] + m_tau[i](XX,YY)*normal[YY] + m_tau[i](XX,ZZ)*normalzz;
	  _flux[m_vID[i]] = m_tau[i](YY,XX)*normal[XX] + m_tau[i](YY,YY)*normal[YY] + m_tau[i](YY,ZZ)*normalzz;
	  _flux[m_wID[i]] = m_tau[i](ZZ,XX)*normal[XX] + m_tau[i](YY,ZZ)*normal[YY] + m_tau[i](ZZ,ZZ)*normalzz;
	  const CFuint endVisc = 2;  
	...
	//cout <<"DiffMFMHD2DHalfVarSet::getFlux--> species = " << i <<"\n";
	//cout <<"DiffMFMHD2DHalfVarSet::getFlux--> lambda  = " << lambda <<"\n";
