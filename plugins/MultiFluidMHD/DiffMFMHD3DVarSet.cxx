  ...
  if (nbEqs == totalNbEqs || iEqSS == 1) {
    ...
    if(getModel().isBraginskii()) {
      if(nbSpecies == 2){		// Model with plasma + neutrals
	    ...
	    // AL: the following needs to be extended properly to 3D
            CFreal qiX = -((DiffMFMHDData[endVisc + 0] + DiffMFMHDData[endVisc + 3])*Tx + (DiffMFMHDData[endVisc + 1] + DiffMFMHDData[endVisc + 4])*Ty);
	    // CFreal qiX = -((DiffMFMHDData[endVisc + 0] + DiffMFMHDData[endVisc + 3] + DiffMFMHDData[endVisc + 6])*Tx +
	    //		   (DiffMFMHDData[endVisc + 1] + DiffMFMHDData[endVisc + 4] + DiffMFMHDData[endVisc + 7])*Ty +
	    //		   (DiffMFMHDData[endVisc + 2] + DiffMFMHDData[endVisc + 5] + DiffMFMHDData[endVisc + 8])*Tz);
            CFreal qiY = -((DiffMFMHDData[endVisc + 1] + DiffMFMHDData[endVisc + 4])*Tx + (DiffMFMHDData[endVisc + 2] + DiffMFMHDData[endVisc + 5])*Ty);
	    CFreal qiZ = 0.; // missing implementation
            _qFlux[0] = qiX*normal[XX] + qiY*normal[YY] + qiZ*normal[ZZ];
          }
          else {		//neutrals
    ...
    else {		// Default case with constant viscosity and conductivity
      for (CFuint i = 0; i < nbSpecies; ++i) {
        const RealVector& DiffMFMHDData = getModel().getPhysicalData();
        const CFreal lambda = DiffMFMHDData[nbSpecies + i];		//WATCH OUT: only reads one value of the thermal conductivity

	_qFlux[i] = -lambda*MathFunctions::innerProd(*gradients[m_TID[i]], normal);
