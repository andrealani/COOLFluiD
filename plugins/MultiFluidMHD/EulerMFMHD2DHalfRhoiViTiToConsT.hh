#ifndef COOLFluiD_Physics_MultiFluidMHD_EulerMFMHD2DHalfRhoiViTiToConsT_hh
#define COOLFluiD_Physics_MultiFluidMHD_EulerMFMHD2DHalfRhoiViTiToConsT_hh
 ...
 */
template <>
class VarSetTransformerT<Physics::MultiFluidMHD::EulerMFMHD2DHalfRhoiViTiT, 
			 Physics::MultiFluidMHD::EulerMFMHD2DHalfConsT, 
			 NOTYPE> {
...
public:
  ...
  HOST_DEVICE void transform(const CFreal *const state, CFreal *const result)
  {
    ...
    for (CFuint ie = 0; ie < nbSpecies; ++ie) { 
      ...
      const CFuint uID   = endEM + nbSpecies + dim*ie;
...
//     std::cout << "EulerMFMHD2DHalfRhoiViTiToCons::transform() => result = "<< result <<"\n";
    ...
    //   Set RhoiEi = Rhoi*(Cv*Ti + 0.5*V²)
      ...
      for (CFuint ie = 0; ie < nbSpecies; ++ie) {
        ...
        const CFreal V2 = state[endEM + nbSpecies + dim*ie]*state[endEM + nbSpecies + dim*ie] + state[endEM + nbSpecies + dim*ie + 1]*
              state[endEM + nbSpecies + dim*ie + 1] + state[endEM + nbSpecies + dim*ie + 2]*state[endEM + nbSpecies + dim*ie + 2];
  ...
  HOST_DEVICE void transformFromRef(const CFreal *const data, CFreal *const result)
  {
    ...
    const CFreal rho = data[EulerMFMHDTerm::RHO];
...
//     std::cout << "EulerMFMHD2DHalfRhoiViTiToCons::transformFromRef() => result = "<< result <<"\n"; 
    }  
...
}; // end of class EulerMFMHD2DHalfRhoiViTiToConsT
...
#endif // COOLFluiD_Physics_MultiFluidMHD_EulerMFMHD2DHalfRhoiViTiToConsT_hh
