...
#include "SubSystemCoupler/FVMCCMeshMatcherWrite.hh"
...
MethodCommandProvider<FVMCCMeshMatcherWrite, SubSysCouplerData, SubSystemCouplerModule> FVMCCMeshMatcherWriteProvider("FVMCCMeshMatcherWrite");

//////////////////////////////////////////////////////////////////////////////

void FVMCCMeshMatcherWrite::defineConfigOptions(Config::OptionList& options)
{
...
FVMCCMeshMatcherWrite::FVMCCMeshMatcherWrite(const std::string& name) :
  CouplerCom(name),
  _sockets(),
...
FVMCCMeshMatcherWrite::~FVMCCMeshMatcherWrite()
{
}
...
FVMCCMeshMatcherWrite::needsSockets()
{
...
void FVMCCMeshMatcherWrite::configure ( Config::ConfigArgs& args )
{
...
void FVMCCMeshMatcherWrite::executeWrite(const CFuint iProc)
{
  CFAUTOTRACE;
...
CFLog(VERBOSE,"Try to match: " << socketCoordNames[iType] << "\n");
...
void FVMCCMeshMatcherWrite::nodeToElementPairing(const RealVector& coord, CFint& stateID, RealVector& coord_Proj)
{
  CFAUTOTRACE;
...
if(dim == 2){
      ...
      if (isOnFace)
      {
        ...
        if ((distanceToFace < _minimumDistanceOnFace) &&(distanceToFace < _minimumDistanceOffFace) ){
          ...
          _shapeFunctionAtCoord = currFace.computeGeoShapeFunctionAtMappedCoord(mappedCoord);
...
//         if(!projectedOnAFace){
          ...
          bool compute(false);
          for(CFuint iNode=0; iNode < currFace.nbNodes(); ++iNode)
          {
            v = coord - (*(currFace.getNode(iNode)));
...
/*            if(distance2Node < _minimumDistanceOffFace){
            ...
            if((distance2Node < _minimumDistanceOffFace) && (distance2Node < _minimumDistanceOnFace)) {
              ...
              stateID = iNode;
              _minimumDistanceOffFace = distance2Node;
              coord_Proj = (*(currFace.getNode(iNode)));
            }
          }
          if(compute){
            _matchingFace.first = (*iTRS);
...
//           }
        ...
        if(dim == 3){
          assert(currFace.getNodes()->size() == 3);
...
void FVMCCMeshMatcherWrite::writeIsAcceptedFile(const std::string dataHandleName)
{
  CFAUTOTRACE;
