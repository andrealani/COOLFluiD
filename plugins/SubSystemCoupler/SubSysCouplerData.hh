#ifndef COOLFluiD_Numerics_SubSystemCoupler_SubSysCouplerData_hh
#define COOLFluiD_Numerics_SubSystemCoupler_SubSysCouplerData_hh
...
#include "SubSystemCoupler/PostVariableTransformer.hh"
#include "SubSystemCoupler/PreVariableTransformer.hh"
...
//////////////////////////////////////////////////////////////////////////////
   ...
   * CouplerCom 's that compose the Coupler.
   *
   * @see CouplerCom
   *
   * @author Thomas Wuilbaut
   */
class SubSysCouplerData : public Framework::CouplerData {
...
typedef std::vector< std::vector<CoupledInterface> > CoupledInterfaces;
   ...
   */
  SubSysCouplerData(Common::SafePtr<Framework::Method> owner);
   ...
   */
  bool isTransferFiles()
  {
    return _isTransferFiles;
  }
   ...
   */
  void setCoupledSubSystemsNames(const std::vector<std::string>& subSysNames, const std::vector<std::string>& nameSpacesNames)
  {
   ...
   */
  void setCoupledSubSystemCoordTypes(const std::string& interfaceName, const std::vector<std::string>& coordType)
  {
    const CFuint idx = _coupledInterfacesMap.find(interfaceName);
   ...
   */
  Common::SafePtr<PreVariableTransformer> getPreVariableTransformer(const std::string& interfaceName)
  {
    CFuint idx = _coupledInterfacesMap.find(interfaceName);
    return _preTransformers[idx].getPtr();
  }

  /**
   * Get the Variable Transformers
   */
  std::vector < Common::SelfRegistPtr<PreVariableTransformer > > getPreVariableTransformers()
  {
    return _preTransformers;
  }

  /**
   * Get the Variable Transformer
   */
  Common::SafePtr<PostVariableTransformer > getPostVariableTransformer(const std::string& interfaceName)
  {
    CFuint idx = _coupledInterfacesMap.find(interfaceName);
    return _postTransformers[idx].getPtr();
  }

  /**
   * Get the Variable Transformers
   */
  std::vector < Common::SelfRegistPtr<PostVariableTransformer > > getPostVariableTransformers()
  {
    return _postTransformers;
  }
...
private:
  ...
  ///Configuration String for the variable transformer
  std::vector<std::string> _preTransformersStr;

  ///Configuration String for the variable transformer
  std::vector<std::string> _postTransformersStr;

  ///Variable transformer (before Data Transfer)
  std::vector< Common::SelfRegistPtr<PreVariableTransformer> > _preTransformers;

  ///Variable transformer (after Data Transfer)
  std::vector< Common::SelfRegistPtr<PostVariableTransformer> > _postTransformers;
...
}; // end of class SubSysCouplerData

//////////////////////////////////////////////////////////////////////////////

/// Definition of a command for Coupler
typedef Framework::MethodCommand<SubSysCouplerData> CouplerCom;

/// Definition of a command provider for Coupler
typedef Framework::MethodCommand<SubSysCouplerData>::PROVIDER CouplerComProvider;
...
#endif // COOLFluiD_Numerics_SubSystemCoupler_SubSysCouplerData_hh
