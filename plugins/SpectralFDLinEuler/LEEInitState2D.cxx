...
#include "LinEuler/LinEuler2DLinearCons.hh"

#include "SpectralFDLinEuler/LEEInitState2D.hh"
...
MethodCommandProvider<LEEInitState2D, SpectralFDMethodData, SpectralFDModule>
LEEInitState2DProvider("LEEInitState2D");

////////////////////////////////////////////////////////////////////////////
//

std::vector<Common::SafePtr<BaseDataSocketSink> >
LEEInitState2D::needsSockets()
{
...
void LEEInitState2D::defineConfigOptions(Config::OptionList& options)
{
...
LEEInitState2D::LEEInitState2D(const std::string& name) :
...
LEEInitState2D::~LEEInitState2D()
{
}

////////////////////////////////////////////////////////////////////////////

void LEEInitState2D::setup()
{
  ...
  getMethodData().getUpdateVar().d_castTo<LinEuler2DVarSet>();
...
void LEEInitState2D::configure ( Config::ConfigArgs& args )
{
  CFAUTOTRACE;
...
******************************/
  ...
  _varSet.reset((Environment::Factory<ConvectiveVarSet>::getInstance().getProvider(varSetName)->create(physModel->getImplementor()->getConvectiveTerm())).d_castTo<Physics::LinearizedEuler::LinEuler2DCons>());
...
void LEEInitState2D::executeOnTrs()
{
  CFAUTOTRACE;
  ...
  CFLogDebugMin("LEEInitState2D::executeOnTrs() called for TRS: " << trs->getName() << "\n");
...
// 0-1 xy
// 2   localid
// 3   isnodal, 1 if states is node, -1 if only quadrature point
// 4-7 meanflow

std::vector< std::vector<CFreal> > dumpvec(trs->getNbStatesInTrs());

//std::cout<< trs->getNbStatesInTrs() << std::endl;

for(CFuint i=0; i<3*trs->getNbStatesInTrs(); i++) {

  dumpvec[i].resize(8,-1.);
  dumpvec[i][4]=meanflow[i][0];
  dumpvec[i][5]=meanflow[i][1];
  dumpvec[i][6]=meanflow[i][2];
  dumpvec[i][7]=meanflow[i][3];
}
...
fos << "VARIABLES= x y lid isnodal mA mB mC Md\n";
