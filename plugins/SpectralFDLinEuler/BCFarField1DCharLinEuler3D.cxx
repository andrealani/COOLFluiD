...
#include "SpectralFDLinEuler/BCFarField1DCharLinEuler3D.hh"
...
Framework::MethodStrategyProvider<
    BCFarField1DCharLinEuler3D,SpectralFDMethodData,BCStateComputer,SpectralFDLinEulerModule >
    BCFarField1DCharLinEuler3DProvider("FarField1DCharLinEuler3D");

//////////////////////////////////////////////////////////////////////////////

std::vector<Common::SafePtr<BaseDataSocketSink> >
BCFarField1DCharLinEuler3D::needsSockets()
{
...
BCFarField1DCharLinEuler3D::BCFarField1DCharLinEuler3D(const std::string& name) :
...
BCFarField1DCharLinEuler3D::~BCFarField1DCharLinEuler3D()
{
  CFAUTOTRACE;
}
...
void BCFarField1DCharLinEuler3D::computeGhostStates(const vector< State* >& intStates,
                                                    ...
                                                    const std::vector< RealVector >& coords)
{
  ...
  for (CFuint iState = 0; iState < nbrStates; ++iState)
  {
    ...
    const CFreal W0     = m_intSolPhysData[LinEulerTerm::W0   ];
    const CFreal C0     = m_intSolPhysData[LinEulerTerm::c    ];
    const CFreal C0Sq   = C0*C0;
    const CFreal V0n    = U0*normal[XX] + V0*normal[YY] + W0*normal[YY];
    ...
    if (V0n < -C0) // supersonic inflow (normal points outward)
    {
      ...
      m_ghostSolPhysData[LinEulerTerm::w]   = -m_intSolPhysData[LinEulerTerm::w  ];
    ...
    else if (V0n < 0.) // subsonic inflow
    {
      ...
      // tangential velocity component (from 3rd and 4th characteristic var == 0) (is forced to zero -> this causes reflections)
//       m_tanVel[XX] = 0.0;
//       m_tanVel[YY] = 0.0;
//       m_tanVel[ZZ] = 0.0;

      // normal velocity component (from 5th characteristic var == 0)
      ...
      const CFreal w = vn*normal[ZZ]; // + m_tanVel[ZZ];
      ...
      m_ghostSolPhysData[LinEulerTerm::w  ] = 2.0*w   - m_intSolPhysData[LinEulerTerm::w  ];
      m_ghostSolPhysData[LinEulerTerm::p  ] = p;
    }
    else if (V0n < C0) // subsonic outflow
    {
      // density and velocity --> from internal solution (2nd, 3rd, 4th and 5th characteristic from internal)
      ...
      const CFreal w   = m_intSolPhysData[LinEulerTerm::w  ];
      ...
      m_ghostSolPhysData[LinEulerTerm::w  ] = w  ;
    ...
    {
      throw Common::ShouldNotBeHereException(FromHere(),"BCFarField1DCharLinEuler3D::computeGhostStates(): could not determine flow condition");
    }
...
void BCFarField1DCharLinEuler3D::computeGhostGradients(const std::vector< std::vector< RealVector* > >& intGrads,
                                               std::vector< std::vector< RealVector* > >& ghostGrads,
                                               const std::vector< RealVector >& normals,
                                               const std::vector< RealVector >& coords)
{
  throw Common::NotImplementedException(FromHere(),"BCFarField1DCharLinEuler3D::computeGhostGradients(): this should not be needed for linearized Euler!!!");
}

//////////////////////////////////////////////////////////////////////////////

void BCFarField1DCharLinEuler3D::setup()
{
  CFAUTOTRACE;
  ...
  {
    throw Common::ShouldNotBeHereException(FromHere(),"Update variable set is not LinEuler3DVarSet in BCFarField1DCharLinEuler3D!");
  }
