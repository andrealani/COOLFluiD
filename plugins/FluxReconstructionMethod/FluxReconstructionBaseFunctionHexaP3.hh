#ifndef COOLFluiD_FluxReconstructionMethod_FluxReconstructionBaseFunctionHexaP3_hh
#define COOLFluiD_FluxReconstructionMethod_FluxReconstructionBaseFunctionHexaP3_hh
 ...
 * representation of the solution in a P3 hexahedral element.
 *
 * @author Kris Van den Abeele
 *
 */
class FluxReconstructionBaseFunctionHexaP3 : public ShapeFunctions::LagrangeShapeFunction {
public:
  ...
  */
        ...
        const RealVector& mappedCoord, RealVector& shapeFunc)
  {
    ...
    // ksi factors
    for (CFuint iSol = 0; iSol < 4; ++iSol)
    {
      const CFreal ksiSol = m_solPnts1D[iSol];
      m_ksiFac[iSol] = 1.;
      for (CFuint iFac = 0; iFac < 4; ++iFac)
      {
        if (iFac != iSol)
        {
    ...
    CFuint iFunc = 0;
    for (CFuint iKsi = 0; iKsi < 4; ++iKsi)
    {
      const CFreal ksiFac = m_ksiFac[iKsi];
      for (CFuint iEta = 0; iEta < 4; ++iEta)
      {
        const CFreal etaFac = m_etaFac[iEta];
        for (CFuint iZta = 0; iZta < 4; ++iZta, ++iFunc)
        {
  ...
  {
    return "FluxReconstructionHexaP2";
  }
...
}; // end of class FluxReconstructionBaseFunctionHexaP3
...
#endif // COOLFluiD_FluxReconstructionMethod_FluxReconstructionBaseFunctionHexaP3_hh
