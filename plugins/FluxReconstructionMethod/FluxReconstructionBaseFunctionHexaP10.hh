#ifndef COOLFluiD_FluxReconstructionMethod_FluxReconstructionBaseFunctionHexaP10_hh
#define COOLFluiD_FluxReconstructionMethod_FluxReconstructionBaseFunctionHexaP10_hh
 ...
 * representation of the solution in a P10 hexahedral element.
 *
 * @author Ray Vandenhoeck
 * @author Alexander Papen
 *
 */
class FluxReconstructionBaseFunctionHexaP10 : public ShapeFunctions::LagrangeShapeFunction {
public:
  ...
  */
        ...
        const RealVector& mappedCoord, RealVector& shapeFunc)
  {
    ...
    // ksi factors
    for (CFuint iSol = 0; iSol < 11; ++iSol)
    {
      const CFreal ksiSol = m_solPnts1D[iSol];
      m_ksiFac[iSol] = 1.;
      for (CFuint iFac = 0; iFac < 11; ++iFac)
      {
        if (iFac != iSol)
        {
    ...
    CFuint iFunc = 0;
    for (CFuint iKsi = 0; iKsi < 11; ++iKsi)
    {
      const CFreal ksiFac = m_ksiFac[iKsi];
      for (CFuint iEta = 0; iEta < 11; ++iEta)
      {
        const CFreal etaFac = m_etaFac[iEta];
        for (CFuint iZta = 0; iZta < 11; ++iZta, ++iFunc)
        {
  ...
  {
    return "FluxReconstructionHexaP10";
  }
...
}; // end of class FluxReconstructionBaseFunctionHexaP10
...
#endif // COOLFluiD_FluxReconstructionMethod_FluxReconstructionBaseFunctionHexaP10_hh
