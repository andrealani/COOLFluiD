...
/// @author Rayan Dhib
class ConvDiffLLAVJacobFluxReconstruction : public CombinedJacobFluxReconstruction {
...
protected: //functions
   ...
   * residual mode, then redistributed over the solution points.
   * @pre m_cells[LEFT] and m_states[LEFT] hold the cell
   */
  virtual void prepareIsolatedCellAV(const CFuint cellID);

  /**
   * Redistribute the artificial viscosity over the solution points of one cell
   * with the weight 1 - relax + relax*|grad rho|/mean(|grad rho|), or keep it
   * uniform when that mean is zero.
   */
  void redistributeCellViscosity(const CFuint side);
