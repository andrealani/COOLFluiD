#ifndef COOLFluiD_FluxReconstructionMethod_FluxReconstructionBaseFunctionHexaP9_hh
#define COOLFluiD_FluxReconstructionMethod_FluxReconstructionBaseFunctionHexaP9_hh
 ...
 * representation of the solution in a P9 hexahedral element.
 *
 * @author Ray Vandenhoeck
 * @author Alexander Papen
 *
 */
class FluxReconstructionBaseFunctionHexaP9 : public ShapeFunctions::LagrangeShapeFunction {
public:
  ...
  */
        ...
        const RealVector& mappedCoord, RealVector& shapeFunc)
  {
    ...
    // ksi factors
    for (CFuint iSol = 0; iSol < 10; ++iSol)
    {
      const CFreal ksiSol = m_solPnts1D[iSol];
      m_ksiFac[iSol] = 1.;
      for (CFuint iFac = 0; iFac < 10; ++iFac)
      {
        if (iFac != iSol)
        {
    ...
    CFuint iFunc = 0;
    for (CFuint iKsi = 0; iKsi < 10; ++iKsi)
    {
      const CFreal ksiFac = m_ksiFac[iKsi];
      for (CFuint iEta = 0; iEta < 10; ++iEta)
      {
        const CFreal etaFac = m_etaFac[iEta];
        for (CFuint iZta = 0; iZta < 10; ++iZta, ++iFunc)
        {
  ...
  {
    return "FluxReconstructionHexaP9";
  }
...
}; // end of class FluxReconstructionBaseFunctionHexaP9
...
#endif // COOLFluiD_FluxReconstructionMethod_FluxReconstructionBaseFunctionHexaP9_hh
