#ifndef COOLFluiD_FluxReconstructionMethod_SimpleEdgeFRQ2_hh
#define COOLFluiD_FluxReconstructionMethod_SimpleEdgeFRQ2_hh
...
class SimpleEdgeFRQ2{
public:
  SimpleEdgeFRQ2( Framework::Node* node1 , Framework::Node* node2):
    firstNode(node1),
    secondNode(node2)
  { 
  }
...
class SimpleEdgeGraphFRQ2
{

public: 
  
  SimpleEdgeGraphFRQ2():socket_nodes("socket_nodes"){;}
  
  void setNodeDataSocketFR( Framework::DataSocketSink < Framework::Node* , 
			  Framework::GLOBAL > nodes );
  
  void computeConnectivityFR();
  
  class iterator{
  public:
    
    iterator(const SimpleEdgeGraphFRQ2 &edgeGraph) :
	...
	m_edgeGraph->socket_nodes.getDataHandle();
      ...
      SimpleEdgeFRQ2 tmp( nodes[firstNodeID], nodes[secondNodeID]);
      return tmp;
    }
  ...
  private:
    SimpleEdgeGraphFRQ2 const *m_edgeGraph;
    CFuint m_row;
    CFuint m_col;
  };
  ...
  const std::vector<Framework::Node*>& getNeighborNodesOfNode(const Framework::Node* firstNode) {
    ...
    const std::vector<CFuint> getNbOfNeighborCells() {
    return m_nbOfNeighborCellsToaNode;
  }
