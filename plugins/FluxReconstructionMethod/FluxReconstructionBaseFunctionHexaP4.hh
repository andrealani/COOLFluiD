#ifndef COOLFluiD_FluxReconstructionMethod_FluxReconstructionBaseFunctionHexaP4_hh
#define COOLFluiD_FluxReconstructionMethod_FluxReconstructionBaseFunctionHexaP4_hh
 ...
 * representation of the solution in a P4 hexahedral element.
 *
 * @author Ray Vandenhoeck
 * @author Alexander Papen
 *
 */
class FluxReconstructionBaseFunctionHexaP4 : public ShapeFunctions::LagrangeShapeFunction {
public:
  ...
  */
        ...
        const RealVector& mappedCoord, RealVector& shapeFunc)
  {
    ...
    // ksi factors
    for (CFuint iSol = 0; iSol < 5; ++iSol)
    {
      const CFreal ksiSol = m_solPnts1D[iSol];
      m_ksiFac[iSol] = 1.;
      for (CFuint iFac = 0; iFac < 5; ++iFac)
      {
        if (iFac != iSol)
        {
    ...
    CFuint iFunc = 0;
    for (CFuint iKsi = 0; iKsi < 5; ++iKsi)
    {
      const CFreal ksiFac = m_ksiFac[iKsi];
      for (CFuint iEta = 0; iEta < 5; ++iEta)
      {
        const CFreal etaFac = m_etaFac[iEta];
        for (CFuint iZta = 0; iZta < 5; ++iZta, ++iFunc)
        {
  ...
  {
    return "FluxReconstructionHexaP4";
  }
...
}; // end of class FluxReconstructionBaseFunctionHexaP4
...
#endif // COOLFluiD_FluxReconstructionMethod_FluxReconstructionBaseFunctionHexaP4_hh
