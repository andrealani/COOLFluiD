...
void CellToFaceGEBuilder::setup()
{
  ...
  // get internal faces-nodes connectivity
  m_intFaceToNodes = MeshDataStack::getActive()->getConnectivity("InnerFacesNodes");
...
void CellToFaceGEBuilder::assembleNeighbourFace(const CFuint cellGeoType, const CFuint faceLocalID)
{
  ...
  else
  {
    ...
    m_builtGeoCells[faceLocalID] = cell;

    // set a large value in m_bndFaceBCType, there is no BC in this face
    m_faceBCIdx[faceLocalID] = 10000;

    // set the nodes in the face
    const CFuint nbrFaceNodes = m_intFaceToNodes->nbCols(intFaceIdx);
    for (CFuint iNode = 0; iNode < nbrFaceNodes; ++iNode)
    {
      const CFuint nodeID = (*m_intFaceToNodes)(intFaceIdx,iNode);
      face->setNode(iNode,m_nodes[nodeID]);
    }
  }
