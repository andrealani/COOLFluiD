#ifndef COOLFluiD_FluxReconstructionMethod_FluxReconstructionBaseFunctionHexaP8_hh
#define COOLFluiD_FluxReconstructionMethod_FluxReconstructionBaseFunctionHexaP8_hh
 ...
 * representation of the solution in a P8 hexahedral element.
 *
 * @author Ray Vandenhoeck
 * @author Alexander Papen
 *
 */
class FluxReconstructionBaseFunctionHexaP8 : public ShapeFunctions::LagrangeShapeFunction {
public:
  ...
  */
        ...
        const RealVector& mappedCoord, RealVector& shapeFunc)
  {
    ...
    // ksi factors
    for (CFuint iSol = 0; iSol < 9; ++iSol)
    {
      const CFreal ksiSol = m_solPnts1D[iSol];
      m_ksiFac[iSol] = 1.;
      for (CFuint iFac = 0; iFac < 9; ++iFac)
      {
        if (iFac != iSol)
        {
    ...
    CFuint iFunc = 0;
    for (CFuint iKsi = 0; iKsi < 9; ++iKsi)
    {
      const CFreal ksiFac = m_ksiFac[iKsi];
      for (CFuint iEta = 0; iEta < 9; ++iEta)
      {
        const CFreal etaFac = m_etaFac[iEta];
        for (CFuint iZta = 0; iZta < 9; ++iZta, ++iFunc)
        {
  ...
  {
    return "FluxReconstructionHexaP8";
  }
...
}; // end of class FluxReconstructionBaseFunctionHexaP8
...
#endif // COOLFluiD_FluxReconstructionMethod_FluxReconstructionBaseFunctionHexaP8_hh
