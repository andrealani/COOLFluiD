...
#include "SpectralFVNavierStokes/BCCurvedWallEuler2D.hh"
...
void BCCurvedWallEuler2D::computeNormalsInFaceFluxPointsFromUserFunction()
{
  ...
  for (CFuint iBCTRS = 0; iBCTRS < nbrBCTRSs; ++iBCTRS)
  {
    ...
    for (CFuint iFace = 0; iFace < nbrFaces; ++iFace)
    {
      ...
      // get face nodes
      vector< RealVector > nodeCoords(2,RealVector(2));
...
void BCCurvedWallEuler2D::computeNormalsInFaceFluxPointsFromDomainModel()
{
  ...
  RealVector avgNormal(2);
  vector< RealVector > mapCoordFaceNode(2,RealVector(1));
  mapCoordFaceNode[0][KSI] = 0.5;
  mapCoordFaceNode[1][KSI] = 0.5;
  RealVector mapCoordFlxPnt(1);
  vector< RealVector > geoDerivs(1,RealVector(2));
  ...
  for (CFuint iBCTRS = 0; iBCTRS < nbrBCTRSs; ++iBCTRS)
  {
    ...
    const CFuint nbTRs = m_bcTRSs[iBCTRS]->getNbTRs();

    // loop over the TRs
    for (DomainModel::TRidx iTR = 0; iTR < nbTRs; ++iTR)
    {
      // get the TR
      SafePtr< TopologicalRegion > bcTR = m_bcTRSs[iBCTRS]->getTopologicalRegion(iTR);
      ...
      for (CFuint iFace = 0; iFace < nbrFaces; ++iFace)
      {
        ...
        avgNormal = face->computeAvgCellNormal();

        // get face nodes
        vector< Node* >* nodes = face->getNodes();

        // get the mapped coordinates of the face nodes
        mapCoordFaceNode[0] = 0.5; // copy for a good guess
        domModel->computeParamCoord(trGlobalIdx,*(*nodes)[0],mapCoordFaceNode[0]);
        mapCoordFaceNode[1] = 0.5; // copy for a good guess
        domModel->computeParamCoord(trGlobalIdx,*(*nodes)[1],mapCoordFaceNode[1]);
        ...
        {
          // coordinates
          mapCoordFlxPnt[KSI] = (*m_flxPntWheightCoordsSVFaces)[iFlx][0]*mapCoordFaceNode[0][KSI] +
                                (*m_flxPntWheightCoordsSVFaces)[iFlx][1]*mapCoordFaceNode[1][KSI];

          // get the first derivative of the model geometry
          domModel->compute1stDeriv(trGlobalIdx,mapCoordFlxPnt,geoDerivs);

          // compute unit normal
          flxPntNormals[iFlx].resize(2);
          const CFreal factor = 1.0/sqrt(geoDerivs[0][XX]*geoDerivs[0][XX] +
                                         geoDerivs[0][YY]*geoDerivs[0][YY]);
          flxPntNormals[iFlx][XX] = +geoDerivs[0][YY]*factor;
          flxPntNormals[iFlx][YY] = -geoDerivs[0][XX]*factor;

          // check orientation
          if (MathFunctions::innerProd(flxPntNormals[iFlx],avgNormal) < 0.0)
          {
            flxPntNormals[iFlx] *= -1.0;
          }
        }
