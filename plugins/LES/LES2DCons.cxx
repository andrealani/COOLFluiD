...
Environment::ObjectProvider< LES2DCons, DiffusiveVarSet, LESModule, 2>
  aLES2DConsProvider("LES2DCons");
...
void LES2DCons::setGradientVars(const vector<RealVector*>& states,
                                RealMatrix& values,
                                const CFuint stateSize)
{
  ...
  for (CFuint i = 0; i < nbStates; ++i)
  {
    ...
    values(3,i) = (state[3] - 0.5*state[0]*(
...
void LES2DCons::setGradientVarGradients(const vector<RealVector*>& states,
...
void LES2DCons::setStateGradients(const vector<RealVector*>& states,
...
CFreal LES2DCons::getDynViscosity(const RealVector& state,
...
CFreal LES2DCons::getDensity(const RealVector& state)
{
  return state[0];
}
...
void LES2DCons::setGradientState(const RealVector& state)
{
