#include "NEQ/NEQ.hh"
#include "Euler2DNEQSymmToConsInRef.hh"
...
Environment::ObjectProvider<Euler2DNEQSymmToConsInRef, 
			    VarSetMatrixTransformer, 
			    NEQModule, 1> 
euler2DNEQSymmToConsInRefProvider("Euler2DNEQSymmToConsInRef");

//////////////////////////////////////////////////////////////////////////////

Euler2DNEQSymmToConsInRef::Euler2DNEQSymmToConsInRef
(Common::SafePtr<Framework::PhysicalModelImpl> model) :
  VarSetMatrixTransformer(model),
  _model(model->getConvectiveTerm().d_castTo<NEQTerm>()), _ys(), _alpha(), _RiGas(), _mmasses()
{
}
...
void Euler2DNEQSymmToConsInRef::setMatrixFromRef()
{
  ...
  for (CFuint is = 0; is < nbSpecies; ++is) {
    ...
    _transMatrix(eID,is) = ova2*_alpha[is]/beta-ova2*V2;
    _transMatrix(evID,is) = 0.0;
  }
...
for (CFuint is = 0; is < nbSpecies; ++is) {
  ...
  _transMatrix(uID,eID) = ova*rho*u;
  _transMatrix(uID,evID) = 0.0;
  
  _transMatrix(vID,uID) = 0.0;
  _transMatrix(vID,vID) = rho;
  _transMatrix(vID,eID) = ova*rho*v;
  _transMatrix(vID,evID) = 0.0;

  _transMatrix(eID,uID)  = rho*u;
  _transMatrix(eID,vID)  = rho*v;
  _transMatrix(eID,eID)  = ova*rho*H;
  _transMatrix(eID,evID) = ova2*phi*rho/beta;
  
  _transMatrix(evID,uID) = 0.0;
  _transMatrix(evID,vID) = 0.0;
  _transMatrix(evID,eID) = ova*rho*ev;
  _transMatrix(evID,evID) = -ova2*rho;//_transMatrix checked on 2009/4/2. Error in position (eID,eID) fixed.
}
      
//////////////////////////////////////////////////////////////////////////////

void Euler2DNEQSymmToConsInRef::computeInverse(RealMatrix& mat)
{  
...
void Euler2DNEQSymmToConsInRef::computeDirect(RealMatrix& mat)
{
