...
//////////////////////////////////////////////////////////////////////////////////////////////////
void NavierStokesGReKO2DSourceTerm_Lang::SetConvandDiffVarset()
{
...
////////////////////////////////////////////////////////////////////////////////////////////////////////
void NavierStokesGReKO2DSourceTerm_Lang::computeProductionTerm(const CFreal& twothirdRhoK,
                                                           const CFreal& MUT,
                                                           const CFreal& vOverRadius,
                                                           const CFreal& KProdTerm)
{
 CFreal ux = (*(_gradients[1]))[XX]; 
 CFreal uy = (*(_gradients[1]))[YY]; 
 CFreal vx = (*(_gradients[2]))[XX]; 
 CFreal vy = (*(_gradients[2]))[YY]; 
   
  const CFreal coeffTauMu = _diffVarSet->getModel().getCoeffTau();
  _KProdTerm = coeffTauMu*(MUT*((4./3.)*((ux-vy)*(ux-vy)+(ux*vy)
                         -(ux+vy-vOverRadius)*vOverRadius)
                         +(uy+vx)*(uy+vx)))
                -twothirdRhoK*(ux+vy+vOverRadius);
}
//////////////////////////////////////////////////////////////////////////////////////////////////
...
//  CFreal tauYY = coeffTauMu * (mut * (2.*(*(_gradients[2]))[YY] - twoThirdDivV)) - twoThirdRhoK;
 ...
 // CFreal prodTerm_k1 = Gaeff*(tauXX*(*(_gradients[1]))[XX] + tauYY*(*(_gradients[2]))[YY] + tauXY * ((*(_gradients[1]))[YY] + (*(_gradients[2]))[XX]));
  ...
  CFreal prodTerm_Omega = (_diffVarSet->getGammaCoef()*rho/mut)*_KProdTerm;
  ...
  ///CoeffDk This coefficient is used in the destruciton term related to k: 
    ...
    CFreal destructionTerm_k = Radius*CoeffDk*((-1.) * rho * avOmega * avK * _diffVarSet->getBetaStar(_avState));
  // CFreal destructionTerm_k = ((-1.) * rho * avOmega * avK * _diffVarSet->getBetaStar(_avState));

  ///Destruction term: Omega
  CFreal destructionTerm_Omega = Radius* (-1.) * rho * avOmega * avOmega * _diffVarSet->getBeta(_avState);
