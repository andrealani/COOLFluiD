#include "AeroCoef/AeroCoefSpectralFV.hh"
#include "AeroCoef/EulerConsComputeAeroSpectralFV.hh"
...
using namespace COOLFluiD::Common;
using namespace COOLFluiD::SpectralFV;
...
MethodCommandProvider<EulerConsComputeAeroSpectralFV, DataProcessingData, AeroCoefSpectralFVModule>
    EulerConsComputeAeroSpectralFVProvider("EulerConsComputeAeroSpectralFV");

//////////////////////////////////////////////////////////////////////////////

void EulerConsComputeAeroSpectralFV::defineConfigOptions(Config::OptionList& options)
{
...
EulerConsComputeAeroSpectralFV::EulerConsComputeAeroSpectralFV(const std::string& name) :
...
EulerConsComputeAeroSpectralFV::~EulerConsComputeAeroSpectralFV()
{
}
...
EulerConsComputeAeroSpectralFV::needsSockets()
{
...
EulerConsComputeAeroSpectralFV::providesSockets()
{
...
void EulerConsComputeAeroSpectralFV::configure ( Config::ConfigArgs& args )
{
  CFAUTOTRACE;
...
void EulerConsComputeAeroSpectralFV::setFunction()
{
  // some sanity checks
  std::vector<std::string> m_functionDef = Common::StringOps::getWords(m_function);
  if(m_functionDef.size() != 1)
   throw BadValueException (FromHere(),"EulerConsComputeAeroSpectralFV::setFuntion(): alpha function wrongly defined.");
...
void EulerConsComputeAeroSpectralFV::executeOnTrs()
{
  CFAUTOTRACE;
...
void EulerConsComputeAeroSpectralFV::computeFaceData()
{
...
void EulerConsComputeAeroSpectralFV::computeWall()
{
  CFAUTOTRACE;
  ...
  // get SpectralFVMethodData
  SafePtr<SpaceMethod> spaceMethod = getMethodData().getCollaborator<SpaceMethod>();
  SafePtr<SpectralFVMethod> spectralFVMethod = spaceMethod.d_castTo<SpectralFVMethod>();
  cf_assert(spectralFVMethod.isNotNull());
  SafePtr<SpectralFVMethodData> spectralFVData = spectralFVMethod->getData();

  // get bndFacesStartIdxs from SpectralFVMethodData
  map< std::string , vector< vector< CFuint > > >&
    bndFacesStartIdxsPerTRS = spectralFVData->getBndFacesStartIdxs();
  ...
  vector< SpectralFVElementData* >& svLocalData = spectralFVData->getSVLocalData();
  SafePtr< vector< RealVector > > outputPntsMappedCoord = svLocalData[0]->getFaceOutputPntFaceMappedCoords();
...
void EulerConsComputeAeroSpectralFV::computeAero()
{
/*  CFAUTOTRACE;
  ...
  // GET FACE MAPPED COORDINATES OF OUTPUT POINTS
  vector< SpectralFVElementData* >& svLocalData = spectralFVData->getSVLocalData();
  SafePtr< vector< vector< RealVector > > > faceNodeMappedCoords = svLocalData[0]->getFaceNodeCoords();
  ...
  const CFuint solPolyOrder = svLocalData[0]->getPolyOrder();
...
void EulerConsComputeAeroSpectralFV::setup()
{
  CFAUTOTRACE;
  ...
  // get face builder
  m_faceBuilder = spectralFVData->getFaceBuilder();

  // get boundary face term computer
  m_bndFaceTermComputer = spectralFVData->getBndFaceTermComputer();
  ...
  SafePtr< vector< SafePtr< BCStateComputer > > >
      bcStateComputers = spectralFVData->getBCStateComputers();
...
void EulerConsComputeAeroSpectralFV::unsetup()
{
}

//////////////////////////////////////////////////////////////////////////////

void EulerConsComputeAeroSpectralFV::prepareOutputFileWall()
{
  CFAUTOTRACE;
...
void EulerConsComputeAeroSpectralFV::prepareOutputFileAero()
{
  CFAUTOTRACE;
...
void EulerConsComputeAeroSpectralFV::updateOutputFileAero()
{
  CFAUTOTRACE;
