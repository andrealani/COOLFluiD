...
#include "AeroCoef/NavierStokesComputeAeroDG.hh"
...
MethodCommandProvider<NavierStokesComputeAeroDG, DataProcessingData, AeroCoefDGModule> navierStokesComputeAeroDG("NavierStokesComputeAeroDG");

//////////////////////////////////////////////////////////////////////////////

void NavierStokesComputeAeroDG::defineConfigOptions(Config::OptionList& options)
{
...
NavierStokesComputeAeroDG::NavierStokesComputeAeroDG(const std::string& name) :
...
NavierStokesComputeAeroDG::~NavierStokesComputeAeroDG()
{
}
...
NavierStokesComputeAeroDG::needsSockets()
{
...
NavierStokesComputeAeroDG::providesSockets()
{
...
void NavierStokesComputeAeroDG::setup()
{
 m_lift = 0.;
 m_drag = 0.;
...
void NavierStokesComputeAeroDG::configure ( Config::ConfigArgs& args )
{
  CFAUTOTRACE;
...
void NavierStokesComputeAeroDG::setFunction()
{
  // some sanity checks
  std::vector<std::string> m_functionDef = Common::StringOps::getWords(m_function);
  if(m_functionDef.size() != 1)
   throw BadValueException (FromHere(),"NavierStokesComputeAeroDG::setFuntion(): alpha function wrongly defined.");
...
void NavierStokesComputeAeroDG::executeOnTrs()
{
  CFAUTOTRACE;
...
void NavierStokesComputeAeroDG::computeWall()
{
//   CFAUTOTRACE;
//
...
void NavierStokesComputeAeroDG::computeAero()
{
  CFAUTOTRACE;
...
//   DataHandle< RealVector> nstates = socket_nstates.getDataHandle();
 ...
 m_Re = DGData->getReynolds();
  ...
  m_drag = .0;
  State tempState;
  RealMatrix Mgrad(nbEqs,nbDim);
  RealMatrix tensor(nbDim,nbDim);
      ...
      for(CFuint kvadrature_point = 0; kvadrature_point < m_nbKvadrPoint; kvadrature_point++ )
      {
        ...
        for (CFuint iState = 0; iState < nbStatesInCellLeft; ++iState) //loop over states in cell
        {
          ...
          for (CFuint iEq = 0; iEq < nbEqs; ++iEq) //loop over members of state
          {
            ...
            {
              Mgrad(iEq,k)+=leftGradient[kvadrature_point](iState,k)*state[iEq];
            }
          }
        }
        ...
        for(CFuint i=0; i<nbDim; ++i)
          for(CFuint j=0; j<nbDim; ++j)
            tensor(i,j)= Mgrad(i+1,j)*tempState[0] - Mgrad(0,j)*tempState[i+1];
        tensor /= tempState[0]*tempState[0];
        CFreal divergence = 0;
        for(CFuint i=0; i<nbDim; ++i)
          divergence+=tensor(i,i);

        for(CFuint i=0; i<nbDim; ++i)
          for(CFuint j=i; j<nbDim; ++j)
        ...
        // for j > i is not needed : ij = ji
        for(CFuint i=0; i<nbDim; ++i)
          for(CFuint j=0; j<nbDim; ++j)
            if(i==j)
            {
              aero(i,i) += (p-tensor(i,i))*leftWeight[0][kvadrature_point]*detJacobi;
            }
            else
            {
              aero(i,j) += -tensor(i,j)*leftWeight[0][kvadrature_point]*detJacobi;
            }
      }

      //Compute Cp (scaled by m, depending on ownership of face to processor)
      //aero-pInf are switched to take into account the minus sign in normal
      Cptemp = 0.;
      for(CFuint iDim=0; iDim<nbDim; ++iDim)
      {
        for(CFuint iDim2=0; iDim2<nbDim; ++iDim2)
          Cp[iDim] += aero(iDim,iDim2) * normal[iDim2];
      }
      // release the face
...
void NavierStokesComputeAeroDG::unsetup()
{
}

//////////////////////////////////////////////////////////////////////////////

void NavierStokesComputeAeroDG::prepareOutputFileWall()
{
  CFAUTOTRACE;
...
void NavierStokesComputeAeroDG::prepareOutputFileAero()
{
  CFAUTOTRACE;
...
void NavierStokesComputeAeroDG::updateOutputFileAero()
{
  CFAUTOTRACE;
