#include "Mutation2.0I/MutationLibrary2.hh"
#include "Mutation2.0I/Mutation2.hh"
...
extern "C" {
 ...
 /**
  ...
  void FORTRAN_NAME(arrhenius)(FDOUBLE, FINT, FDOUBLE, FINT, FDOUBLE, FINT, 
			       ...
			       FINT, FINT);
  /**
   * Importing of the FORTRAN subroutine SOURCE
   */
  void FORTRAN_NAME(source)(FDOUBLE, FINT, FDOUBLE, FINT, FDOUBLE, FINT, 
                               FDOUBLE, FINT, FINT, FINT,
                               FDOUBLE, FDOUBLE, FDOUBLE, FDOUBLE, FDOUBLE,
                               FDOUBLE, FDOUBLE, FDOUBLE, FDOUBLE, FDOUBLE, 
                               FDOUBLE, FDOUBLE, FDOUBLE, FDOUBLE, FDOUBLE,
  ...
  /**
   * Importing of the FORTRAN subroutine SMSUT
   */
  void FORTRAN_NAME(smsut)(FDOUBLE, FINT, FDOUBLE, FINT, FINT, FINT,FDOUBLE, FDOUBLE,
			   FDOUBLE, FDOUBLE, FDOUBLE,FDOUBLE, FDOUBLE, FDOUBLE);
  
  /**
   * Importing of the FORTRAN subroutine SMCG
   */
  void FORTRAN_NAME(smcg)(FDOUBLE, FINT, FDOUBLE, FINT, FINT, FINT,FDOUBLE, FDOUBLE,
			  FDOUBLE, FDOUBLE, FDOUBLE,FDOUBLE, FDOUBLE, FDOUBLE);
  ...
  /**
   * Importing of the FORTRAN subroutine INITIALIZELOOKUP
   */
  void FORTRAN_NAME(initializelookup)(FDOUBLE, FINT, FINT, FINT);
}

//////////////////////////////////////////////////////////////////////////////

Environment::ObjectProvider<MutationLibrary2,
          PhysicalPropertyLibrary,
          Mutation2Module,
          1>
mutation2LibraryProvider("Mutation2");

//////////////////////////////////////////////////////////////////////////////

void MutationLibrary2::defineConfigOptions(Config::OptionList& options)
{
...
MutationLibrary2::MutationLibrary2(const std::string& name)
...
MutationLibrary2::~MutationLibrary2()
{
}

//////////////////////////////////////////////////////////////////////////////

void MutationLibrary2::configure ( Config::ConfigArgs& args )
{
...
void MutationLibrary2::copyDataFiles()
{
  ...
  if (m_libPath == "") {
    ...
    m_libPath = baseDir + "/plugins/Mutation2.0I/";
  }
...
void MutationLibrary2::deleteDataFiles()
 {
...
void MutationLibrary2::setLibrarySequentially()
{
 ...
 /* _extraData.enthalpyTt.resize(_NS);
  ...
  FORTRAN_NAME(mutinitcf) (&_LWR1,_WR1,&_LWR2,_WR2,&_LWR3,_WR3,&_LWR4,_WR4,&_LWI,_WI,&_LWC,_WC,
         &_imod,&_IVIBTEMPI,&_IVIBSPEI,&_INVIBSPEI,&_IVIBI,&_IATOMI, &_ITV, &_IDIS);
  ...
  // set the ID identifying the electron temperature
  if (_electrEnergyID < 0 && _nbTvib > 0 && getNbTe() == 0) {
    _electrEnergyID = 0;
  }
  ...
  _hasElectrons = (_NE == 1) ? true : false;
   ...
   // calculates interpolation coefficients and store values into global 
   // arrays to be reused during look up
   FORTRAN_NAME(initializelookup)(_WR1, &_LWR1, _WI, &_LWI);
}

//////////////////////////////////////////////////////////////////////////////

void MutationLibrary2::unsetup()
{
  if(isSetup()) {
...
CFdouble MutationLibrary2::lambdaD(CFdouble& temperature,
				   CFdouble& pressure)
{
  ...
  if (temp < 100.) {
    //cout << " MutationLibrary2::lambdaD() => Watch Out !! T <<<<< 100. " << temp << endl;
    temp = 100.;
  }
...
CFdouble MutationLibrary2::etaD(CFdouble& temperature,
				CFdouble& pressure,
				CFreal* tVec)
{
  //composition must be called before!
  ...
  CFdouble temp = temperature;
  if (temp < 100.) {
    //cout << " MutationLibrary2::etaD() => Watch Out !! T <<<<< 100. " << temp << endl;
    temp = 100.;
  }
...
CFdouble MutationLibrary2::lambdaCG(CFdouble& temperature,
                                   CFdouble& pressure)
{
  //composition must be called before!
  ...
  CFdouble temp = temperature;
  if (temp < 100.) {
    //cout << " MutationLibrary2::lambdaCG() => Watch Out !! T <<<<< 100. " << temp << endl;
    temp = 100.;
  }
...
CFdouble MutationLibrary2::lambdaNEQ(CFdouble& temperature,
				     CFdouble& pressure)
{
  //composition must be called before!
  CFdouble lambdaTot = 0.0;
  ...
  if (temp < 100.) {
    //cout << " MutationLibrary2::lambdaNEQ() => Watch Out !! T <<<<< 100. " << temp << endl;
    temp = 100.;
  }
...
void MutationLibrary2::lambdaVibNEQ(CFreal& temperature,
            RealVector& tVec,
            CFdouble& pressure,
            CFreal& lambdaTrRo,
            RealVector& lambdaInt)
{
  ...
  CFreal Te = (_nbTe == 0)? std::min(tVec[0],_maxTe) : std::min(tVec[TeID],_maxTe);
...
CFdouble MutationLibrary2::etaCG(CFdouble& temperature,
				 CFdouble& pressure,
				 CFreal* tVec)
{
  //composition must be called before!
  ...
  CFdouble temp = temperature;
  if (temp < 100.) {
    //cout << " MutationLibrary2::etaCG() => Watch Out !! T <<<<< 100. " << temp << endl;
    temp = 100.;
  }
...
CFdouble MutationLibrary2::sigma(CFdouble& temp,   //electrical conductivity
         CFdouble& pressure,
         CFreal* tVec)
{
//composition must be called before!
...
void MutationLibrary2::gammaAndSoundSpeed(CFdouble& temp,
           CFdouble& pressure,
           CFdouble& rho,
           CFdouble& gamma,
           CFdouble& soundSpeed)
...
void MutationLibrary2::frozenGammaAndSoundSpeed(CFdouble& temp,
						CFdouble& pressure,
						CFdouble& rho,
						CFdouble& gamma,
						CFdouble& soundSpeed,
...
CFdouble MutationLibrary2::soundSpeed(CFdouble& temp,
                                      CFdouble& pressure)
{
...
void MutationLibrary2::setComposition(CFdouble& temp,
             CFdouble& pressure,
             RealVector* x)
{
...
void MutationLibrary2::setDensityEnthalpyEnergy(CFdouble& temp,
						CFdouble& pressure,
						RealVector& dhe)
{
						...
						bool storeExtraData)
{
  ...
  if (_nbTvib != 0) {
    ...
    const CFuint TeID = tVec.size()-1;
    Te = (_nbTe == 0)? std::min(tVec[0],_maxTe) : std::min(tVec[TeID],_maxTe);
    setTVarray(tVec);
  } 
  else {
    Te = temp;
...
CFdouble MutationLibrary2::density(CFdouble& temp,
				   CFdouble& pressure,
				   CFreal* tVec)
{
  CFdouble ND = 0.0;
...
CFdouble MutationLibrary2::pressure(CFdouble& rho,
            CFdouble& temp,
            CFreal* tVec)
{
  CFdouble p = 0.0;
...
CFdouble MutationLibrary2::electronPressure(CFreal rhoE,
              CFreal tempE)
{
...
CFdouble MutationLibrary2::energy(CFdouble& temp,
          CFdouble& pressure)
{
  CFdouble ND = 0.0;
  CFdouble rho = 0.0;
...
CFdouble MutationLibrary2::enthalpy(CFdouble& temp,
                                   CFdouble& pressure)
{
  setDefaultTVarray(temp);
...
void MutationLibrary2::setElementXFromSpeciesY(const RealVector& ys)
{
...
void MutationLibrary2::setElectronFraction(RealVector& ys)
{
...
void MutationLibrary2::setSpeciesFractions(const RealVector& ys)
{
  if (presenceElectron()) {
...
void MutationLibrary2::getSpeciesMolarFractions
...
void MutationLibrary2::getSpeciesMassFractions
...
void MutationLibrary2::getTransportCoefs(CFdouble& temp,
          CFdouble& pressure,
          CFdouble& lambda,
          CFdouble& lambdacor,
          RealVector& lambdael,
...
void MutationLibrary2::getMassProductionTerm(CFdouble& temperature,
              RealVector& tVec,
              CFdouble& pressure,
              CFdouble& rho,
              const RealVector& ys,
...
void MutationLibrary2::getSource(CFdouble& temperature,
				 RealVector& tVec,
				 CFdouble& pressure,
				 CFdouble& rho,
				 const RealVector& ys,
...
// Vibrational energies at T, Tv's and Te
      FORTRAN_NAME(enthalpy)(_WR1, &_LWR1, _WI, &_LWI, &temp, &Te, &temp, _TVARRAY, &pressure,
                             _HTOTAL, _HTRANS, _HELECT, _HROT, _HVIBR, _HFORM);
  ...
  FORTRAN_NAME(source) (_WR1, &_LWR1, _WR2, &_LWR2, _WR3, &_LWR3, _WR4, &_LWR4, 
                       ...
                       _OMEGAVE, _OMEGACV, &omegaet, &_OMEGAI, _OMEGAJACOB, 
                       &iimolF77, &flagJ);
...
//      _OMEGAI = 0.0;  // to remove
  ...
  if (flagJac) {
      ...
      const CFuint nbTvTe = _nbTvib+_nbTe; 
      for (CFuint is = 0; is < nns; ++is ) {
          jacobian(startT,is) = _OMEGAJACOB[nns*sizeJ+is];
      }
      for (CFuint js = startT; js < nbEqs; ++js) {
          const CFuint idx = js - dim;
          jacobian(startT,js) = _OMEGAJACOB[nns*sizeJ+idx];
      }

      for (CFuint v = 0; v < nbTvTe; ++v) {
        for (CFuint js = 0; js < nns; ++js) {
          jacobian(nns+dim+v+1,js) = _OMEGAJACOB[(nns+v+1)*sizeJ+js]; 
        }
        for (CFuint js = startT; js < nbEqs; ++js) {
          const CFuint idx = js - dim;
          jacobian(nns+dim+v+1,js) = _OMEGAJACOB[(nns+v+1)*sizeJ+idx]; 
        }
      }
  }  
}
...
void MutationLibrary2::getDij_fick(RealVector& dx,
				   CFdouble& pressure,
				   CFdouble& temperature,
				   RealMatrix& Dij,
				   RealVector& rhoUdiff)
{
  throw NotImplementedException(FromHere(), "MutationLibrary2::getDij_fick()");
}

//////////////////////////////////////////////////////////////////////////////

void MutationLibrary2::getGammaN(CFreal& m_GN)
{
  throw NotImplementedException(FromHere(), "MutationLibrary2::getGammaN()");
}

//////////////////////////////////////////////////////////////////////////////

void MutationLibrary2::getGammaO(CFreal& m_GO)
{
  throw NotImplementedException(FromHere(), "MutationLibrary2::getGammaO()");
}
      
//////////////////////////////////////////////////////////////////////////////

void MutationLibrary2::setSpeciesMolarFractions(const RealVector& xs)
{
  throw NotImplementedException(FromHere(), "MutationLibrary2::setSpeciesMolarFractions()");
}

//////////////////////////////////////////////////////////////////////////////

void MutationLibrary2::getSpeciesTotEnthalpies(CFdouble& temp,
					       RealVector& tVec,
					       CFdouble& pressure,
					       RealVector& hsTot,
					       RealVector* hsVib,
					       RealVector* hsEl)
{
 if (tVec.size()== 0) {
  ...
  if (presenceElectron()) {
    for(CFuint i = 0; i < _boltzmannIDs.size(); ++i) {
      const CFuint elEnergyID = _boltzmannIDs[i];
      (*hsEl)[elEnergyID] = _HELECTUM[elEnergyID];
    }
    (*hsEl)[0] = _HTRANSUM[0];
  }
}
...
void MutationLibrary2::getSourceTermVT(CFdouble& temperature,
              RealVector& tVec,
              CFdouble& pressure,
              CFdouble& rho,
              RealVector& omegav,
...
/*
  The original value from Johnston was 
  ...
  ****************************************************************
  ...
  M_N      = kg/mol  e.g. 0.014 for N
  M_N{hat} = kg/kg-mol e.g. 14  for N

  I_N [J/kg-mol] = 1/M_N{hat} I_Nhat [J/kg] = 1/(M_N*1000.0) I_Nhat

  ****************************************************************

 */
    _OMEGAI = -_OMEGA[8]*4.05e5/_MOLARMASSP[8] - _OMEGA[10]*4.30e5/_MOLARMASSP[10];
  }
/*
 I COMMENT THIS FOR CR TO USE THE SELF CONSISTENT MODEL !!!!
//     _OMEGAI = -_OMEGA[10]*4.05e5/_MOLARMASSP[10] - _OMEGA[12]*4.30e5/_MOLARMASSP[12];
      _OMEGAI = -_OMEGA[15]*4.05e5/_MOLARMASSP[15] - _OMEGA[17]*4.30e5/_MOLARMASSP[17];
*/
//  }
...
void MutationLibrary2::getMolarMasses(RealVector& mm)
{
...
void MutationLibrary2::cvGnoffo()
{
...
void MutationLibrary2::testAndWriteProperties()
{
  CFdouble lambdaTot = 0.0;
  CFdouble lambdaInth = 0.0;
  CFdouble lambdaTh = 0.0;
...
void MutationLibrary2::testAndWriteSourceTerms()
{
  // Limit the mass composition of species !!!
...
void MutationLibrary2::transportCoeffNEQ(CFreal& temperature, 
					 CFdouble& pressure,
					 CFreal* tVec, 
					 RealVector& normConcGradients,
					 CFreal& eta,
...
void MutationLibrary2::getSourceEE(CFdouble& temperature,
				   RealVector& tVec,
				   CFdouble& pressure,
				   CFdouble& rho,
				   const RealVector& ys,
				   ...
				   CFdouble& omegaEE)
{
  ...
  */
  ...
  if (_nbTe == 0) {
    ...
    // Park's model
    if (_includeElectronicEnergy && presenceElectron()) {
      omegaEE += omegaet + _OMEGAI;
      for(CFuint i = 0; i < _boltzmannIDs.size(); ++i) {
        const CFuint elEnergyID = _boltzmannIDs[i];
	omegaEE += _OMEGA[elEnergyID]*_HELECT[elEnergyID]/_MOLARMASSP[elEnergyID];
      }
    }
  }
  else {
    ...
    //      _OMEGAI = 0.0;  // to remove
    omegaEE = omegaet - sumOmegaVE + _OMEGAI;
