...
void ParCFmeshBinaryFileReader::readNbNodes(MPI_File* fh)
{
  ...
  readScalar(fh, m_totNbNodes);
  CFint nbNonUpdatableNodes = 0;
  readScalar(fh, nbNonUpdatableNodes);
...
void ParCFmeshBinaryFileReader::readNbStates(MPI_File* fh)
{
  ...
  readScalar(fh, m_totNbStates);
  CFint nbNonUpdatableStates = 0;
  readScalar(fh, nbNonUpdatableStates);
...
void ParCFmeshBinaryFileReader::readNbElementTypes(MPI_File* fh)
{
  ...
  readScalar(fh, m_totNbElemTypes);
...
/*void ParCFmeshBinaryFileReader::readNodeList(MPI_File* fh)
{
  ...
  // do a MPI_All_to_allv to gather gather all the local node data
  
  CFuint nodeSize = dim;
  if (m_hasPastNodes)  {nodeSize += tmpPastNode.size();}
  if (m_hasInterNodes) {nodeSize += tmpInterNode.size();}
  if (nbExtraVars > 0) {nodeSize += extraVars.size();}
  
  // read "\n" character
  char c; readScalar(fh, c); 
  
  const CFuint nbNodesToReadAtOnce = m_totNbNodes/m_nbProc;
  vector<CFreal> buf(nbNodesToReadAtOnce*nodeSize);
  
  CFuint countLocals = 0;
  CFuint iNode = 0;
  while (iNode < m_totNbNodes) {
    const CFuint sizeToRead    = std::min(nbNodesToReadAtOnce, m_totNbNodes - iNode);
    const CFuint sizeToReadBuf = sizeToRead*nodeSize;
    
    // read part of the node list
    readArray(fh, &buf[0], sizeToReadBuf);
    
    CFuint countBuf = 0;
    for (CFuint n = 0; n < sizeToRead; ++n, ++iNode) {
      // read nodes
      for (CFuint d = 0; d < dim; ++d) {tmpNode[d] = buf[countBuf++];}
      // read past nodes
      if (m_hasPastNodes) {
	for (CFuint d = 0; d < dim; ++d) {tmpPastNode[d] = buf[countBuf++];}
      }
      // read inter nodes
      if (m_hasInterNodes) {
      	for (CFuint d = 0; d < dim; ++d) {tmpInterNode[d] = buf[countBuf++];}
      }
      // read extra variables
      if (nbExtraVars > 0) {
	for (CFuint d = 0; d < extraVars.size(); ++d) {extraVars[d] = buf[countBuf++];}
      }
...
/*void ParCFmeshBinaryFileReader::readStateList(MPI_File* fh)
{
  ...
  CFuint stateSize = readState.size();
  if (m_hasPastStates) {
    stateSize += tmpPastState.size();
  }
  if (m_hasInterStates) {
    stateSize += tmpInterState.size();
  }
  if (nbExtraVars > 0) {
    stateSize += extraVars.size();
  }
  
  const CFuint nbStatesToReadAtOnce = m_totNbStates/m_nbProc;
  vector<CFreal> buf(nbStatesToReadAtOnce*stateSize);
  
  CFuint countLocals = 0;
  CFuint iState = 0;
  while (iState < m_totNbStates) {
    // read the state
    const CFuint sizeToRead    = std::min(nbStatesToReadAtOnce, m_totNbStates - iState);
    const CFuint sizeToReadBuf = sizeToRead*stateSize;
    
    if (isWithSolution)  { 
      // read part of the state list
      readArray(fh, &buf[0], sizeToReadBuf);
    }
    
    CFuint countBuf = 0;
    for (CFuint n = 0; n < sizeToRead; ++n, ++iState) {
      if (isWithSolution)  { 
	// no init values were used
	if (m_useInitValues.size() == 0) {
	  // read states
	  for (CFuint d = 0; d < readState.size(); ++d) {
	    readState[d] = buf[countBuf++];
	  }
	  	  
	  // read past states
	  if (m_hasPastStates) {
	    for (CFuint d = 0; d < tmpPastState.size(); ++d) {
	      tmpPastState[d] = buf[countBuf++];
	    }
	  }
	...
	cout << "state[" << iState << "] = " << *newState << endl;
...
void ParCFmeshBinaryFileReader::readGeomEntList(MPI_File* fh)
{
  ...
  readArray(fh, &nbNodesStatesInTRGeo[0], nbNodesStatesInTRGeo.size());
  char c; readScalar(fh,c);  // reads a "\n"
...
void ParCFmeshBinaryFileReader::readNbExtraNodalVars(MPI_File* fh)
{
  ...
  readScalar(fh, nbExtraNodalVars);
...
void ParCFmeshBinaryFileReader::readNbExtraStateVars(MPI_File* fh)
{
  ...
  readScalar(fh, nbExtraStateVars);
...
void ParCFmeshBinaryFileReader::readExtraVarStrides(MPI_File* fh)
{
  ...
  readArray(fh, &extraVarStrides[0], nbExtraVars);
...
void ParCFmeshBinaryFileReader::readExtraStateVarStrides(MPI_File* fh)
{
  ...
  readArray(fh, &extraStateVarStrides[0], nbExtraStateVars);
...
void ParCFmeshBinaryFileReader::readExtraNodalVarStrides(MPI_File* fh)
{
  ...
  readArray(fh, &extraNodalVarStrides[0], nbExtraNodalVars);
...
void ParCFmeshBinaryFileReader::readExtraVars(MPI_File* fh)
{
  ...
  if (nbExtraVars > 0) {
    ...
    readArray(fh, &extraVars[0], sizeExtraVars);
