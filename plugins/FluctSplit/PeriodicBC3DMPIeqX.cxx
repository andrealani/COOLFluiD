...
#include "FluctSplit/PeriodicBC3DMPIeqX.hh"
...
MethodCommandProvider<PeriodicBC3DMPIeqX,
                      FluctuationSplitData,
                      FluctSplitModule>
periodicBC3dmpieqXProvider("PeriodicBC3DMPIeqX");

//////////////////////////////////////////////////////////////////////////////

void PeriodicBC3DMPIeqX::defineConfigOptions(Config::OptionList& options)
{
...
PeriodicBC3DMPIeqX::PeriodicBC3DMPIeqX(const std::string& name) :
...
PeriodicBC3DMPIeqX::~PeriodicBC3DMPIeqX()
{
}
...
PeriodicBC3DMPIeqX::needsSockets()
{
...
void PeriodicBC3DMPIeqX::setup()
{
  ...
  NodeCoordinate.reserve(3*nb_st_trs);
  for (CFuint iState = 0; iState< nb_st_trs; iState++){
   ...
   const CFuint stateID = (*applied_trs_statesIdx)[iState];
    ...
    NodeCoordinate.push_back((*state).getCoordinates()[ZZ]);
  ...
  CFuint nC = 4; //total number of data of the state
  ...
  for(CFuint p=0; p<nP; ++p){
    ...
    for(CFuint iState = 0; iState<nb_st_trs; iState++){
      ...
      CFreal nodeZ = state->getCoordinates()[ZZ];
	for (CFuint iPeriodicState = 0; iPeriodicState<count; ++iPeriodicState){
	  ...
	  CFreal nodePeriodicZCoordinate = RecvNodeCoordinate[RecvDis[p] + nC*iPeriodicState + 2];
	  CFreal isParUpdate = RecvNodeCoordinate[RecvDis[p] + nC*iPeriodicState + 3];

	  if(MathTools::MathChecks::isNotEqualWithError(nodeX, nodePeriodicXCoordinate, m_threshold)) {
	    if(MathTools::MathChecks::isEqualWithError(nodeY,nodePeriodicYCoordinate, m_threshold)){
	      if(MathTools::MathChecks::isEqualWithError(nodeZ,nodePeriodicZCoordinate, m_threshold)){
		if(isParUpdate == 1.0){
...
void PeriodicBC3DMPIeqX::executeOnTrs()
{
 ...
 for (CFuint is = 0; is < applied_trs_statesIdx->size(); ++is)
  ...
  if ((state->isParUpdatable())){
	...
	rhs(applied_sid,h,_nE) += _rbuf2[_RecvDis2[ProcessPeriodic] +_nE*FacePeriodic+h];
      ...
      isUpdated[applied_sid] = true; // NV: Not so sure that this is enough....maybe it is also necessary to flag the state on the other side
...
void PeriodicBC3DMPIeqX::configure ( Config::ConfigArgs& args )
{
...
void PeriodicBC3DMPIeqX::preProcess()
{
...
void PeriodicBC3DMPIeqX::Barrier()
{
  MPI_Barrier(_comm);
}
