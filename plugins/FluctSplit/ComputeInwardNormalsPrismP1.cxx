...
#include "ComputeInwardNormalsPrismP1.hh"
...
Environment::ObjectProvider<ComputeInwardNormalsPrismP1,
               ComputeNormals,
               FluctSplitModule>
computeInwardNormalsPrismP1Provider("InwardPrismP1");

//////////////////////////////////////////////////////////////////////////////

void ComputeInwardNormalsPrismP1::operator() (const CFuint& iFirstCell,
                                              ...
                                              const CFuint& iType)
{
...
#ifndef NDEBUG
  //  std::string filename("PrismP1normals.dat");
  ...
  for (CFuint iCell = iFirstCell; iCell < iLastCell; ++iCell) {
    ...
    coordinates.setRow(*(*nodes)[5],5);

    // computation of Face normals

    calc.computePrismNormals(coordinates,faceNormals);
  ...
  sum += faceNormals(iFace, iDim) * faceNormals(iFace, iDim);
    ...
    // NN0 = - F3 - F1
    tmp = faceNormals.getRow<RealVector>(3) + faceNormals.getRow<RealVector>(1);
    ...
    // NN3 = - F0 - F3
    tmp = faceNormals.getRow<RealVector>(0) + faceNormals.getRow<RealVector>(3);
    nodalNormals.setRow(tmp,3);

    // NN4 = - F4 - F0
    tmp = faceNormals.getRow<RealVector>(4) + faceNormals.getRow<RealVector>(0);
    nodalNormals.setRow(tmp,4);

    // NN5 = - F2 - F0
    tmp = faceNormals.getRow<RealVector>(2) + faceNormals.getRow<RealVector>(0);
    nodalNormals.setRow(tmp,5);
...
void ComputeInwardNormalsPrismP1::update(const CFuint& iFirstCell,
                                         const CFuint& iLastCell,
                                         const CFuint& iType)
{
  NormalsCalculator calc;
