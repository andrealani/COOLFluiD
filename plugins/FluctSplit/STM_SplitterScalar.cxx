...
STM_SplitterScalar::STM_SplitterScalar(const std::string& name) :
...
STM_SplitterScalar::~STM_SplitterScalar()
{
}

//////////////////////////////////////////////////////////////////////////////

void STM_SplitterScalar::setBlockData()
{
...
void STM_SplitterScalar::computeK(const vector<State*>& states,
					   const InwardNormalsData* const normalsData)
{
  ...
  CFreal onetheirdArea = _cellVolume/(nbDim+1);
  ...
  for (CFuint iState = 0; iState < _nbStatesInCell; ++iState) {
    ...
    for (CFuint iEq = 0; iEq < _nbEquations; iEq++) {
      ...
      mini = min(mini, (Area+PastArea)/_kPlus[iState][iEq]);
    }
    ...
    if (_isOnlySplitter && !getMethodData().getDistributionData().isPerturb) {
      ...
      _updateCoeff[states[iState]->getLocalID()] += (onetheirdArea+((_timeStep/2.)*_kPlus[iState].max()));
    }
