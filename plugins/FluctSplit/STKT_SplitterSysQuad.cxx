...
STKT_SplitterSysQuad::STKT_SplitterSysQuad(const std::string& name) :
...
STKT_SplitterSysQuad::~STKT_SplitterSysQuad()
{
...
void STKT_SplitterSysQuad::setBlockData()
{
...
void STKT_SplitterSysQuad::setup()
{
...
void STKT_SplitterSysQuad::computeK(const vector<State*>& states,
					   const InwardNormalsData* const normalsData)
{
  ...
  for (CFuint iState = 0; iState < _nbStatesInCell; ++iState) {
    ...
    if (!ddata.isPerturb) {
      ...
      if (maxKiplus != 0.0)
        mini = min(mini, (Area+PastArea)/(4.0*maxKiplus));
