...
#include "FluctSplit/StrongSymmetryAxisImpl.hh"
...
MethodCommandProvider<StrongSymmetryAxisImpl,
		      FluctuationSplitData,
		      FluctSplitModule>
strongSymmetryAxisImplProvider("StrongSymmetryAxisImpl");

//////////////////////////////////////////////////////////////////////////////

void StrongSymmetryAxisImpl::defineConfigOptions(Config::OptionList& options)
{
}

//////////////////////////////////////////////////////////////////////////////

StrongSymmetryAxisImpl::StrongSymmetryAxisImpl(const std::string& name) :
  StrongImplBC(name)
{
...
StrongSymmetryAxisImpl::~StrongSymmetryAxisImpl()
{
}

//////////////////////////////////////////////////////////////////////////////

void StrongSymmetryAxisImpl::setup()
{
  CFAUTOTRACE;
...
//   _TWall /= term->getTempRef();
  ...
  for(CFuint iTRS = 0; iTRS < trsList.size(); ++iTRS) {
    ...
    CFLogDebugMax("StrongSymmetryAxisImpl::setup() called for TRS: "
		  << trs->getName() << "\n");
    ...
    for (CFuint iState = 0; iState < statesIdx->size(); ++iState) {
      ...
      for (CFuint iVar = 0; iVar < nbEqs; ++iVar) {
	    if (dim == DIM_3D) {
	      discardTimeJacob[localStateID][iVar] = iVar == nbSpPlus2 ?  true : false;
	    }
	    else {
	      discardTimeJacob[localStateID][iVar] = iVar == nbSpPlus1 ?  true : false;
	    }  
      }
    }
  }
...
void StrongSymmetryAxisImpl::executeOnTrs()
{  
  (PhysicalModelStack::getActive()->getDim() == DIM_3D) ? compute3D() : compute2D();
}
//////////////////////////////////////////////////////////////////////////////

void StrongSymmetryAxisImpl::compute2D()
{ 
  ...
  CFLogDebugMax( "StrongSymmetryAxisImpl::execute() called for TRS: "
		 << trs->getName() << "\n");
...
//                 jacobMatrix->setValue(uID, globalColID, 0.0);//Should I comment out this line ...
                jacobMatrix->setValue(vID, globalColID, 0.0);
//                 jacobMatrix->setValue(eID, globalColID, 0.0);//... and this one?
              }
            }
          }// end - for
          isUpdated[localStateID] = true; // flagging is important!!!!!
        }// end - if (!isUpdated[localStateID])
      }// end - if( currState->isParUpdatable() )
    }// end - if (!_useForInitialization)
    else {
  //    (*currState)[nbSpecies] = 0.0;
      (*currState)[nbSpPlus1] = 0.0;
//       (*currState)[nbSpPlus2] = _TWall;
...
void StrongSymmetryAxisImpl::compute3D()
{
