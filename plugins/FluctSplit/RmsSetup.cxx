...
#include "FluctSplit/RmsSetup.hh"
...
MethodCommandProvider<RmsSetup, FluctuationSplitData, FluctSplitModule> rmsSetupProvider("RmsSetup");

//////////////////////////////////////////////////////////////////////////////

void RmsSetup::defineConfigOptions(Config::OptionList& options)
{
  options.addConfigOption< std::string >
    ("RMSName","name of the input limiter socket");
  options.addConfigOption< std::string >
    ("RMSDimension","Switch RMS calculation in 2D or 3D");
}
//////////////////////////////////////////////////////////////////////////////

RmsSetup::RmsSetup(const std::string& name) :
  ...
  socket_rms("rms")
{
 addConfigOptionsTo(this);
  ...
  _rmsSocketName = "Null";
   setParameter("RMSName",&_rmsSocketName);
   
   _rmsDimension = "2D";
   setParameter("RMSDimension",&_rmsDimension);
}
...
void RmsSetup::configure ( Config::ConfigArgs& args )
{
  CFAUTOTRACE;
  ...
  if (_rmsSocketName != "Null") {
    m_dynamicSockets.createSocketSink<CFreal>(_rmsSocketName, true);
  }
...
void RmsSetup::execute()
{
  ...
  CFuint sizerms = nbStates*10;
 if(_rmsDimension == "2D"){
   cout<<"RMS Setup is set to 2D!!"<<endl;
   sizerms = nbStates*10;
 }
 else{
   cout<<"RMS Setup is set to 3D!!"<<endl;
   sizerms = nbStates*7;
 }
  ...
  if (m_dynamicSockets.sinkSocketExists(_rmsSocketName)) {
  
    DataHandle<CFreal> initrms = m_dynamicSockets.getSocketSink<CFreal>(_rmsSocketName)->getDataHandle();
    cf_assert(initrms.size() == rms.size());
    for (CFuint i = 0; i < sizerms; ++i) {
      rms[i] = initrms[i];
    }
  } 
}
...
void RmsSetup::storeBoundaryNormals()
{
...
void RmsSetup::computeNormalsData()
{
  CFAUTOTRACE;
...
void RmsSetup::flagBoundaryStates()
{
  CFAUTOTRACE;
...
void RmsSetup::setFaceNeighCell()
{
  CFAUTOTRACE;
...
void RmsSetup::computeCellVolumes()
{
