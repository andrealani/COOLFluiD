...
#include "StrongSubOutletNonRefSteadyEuler2DCons.hh"
...
MethodCommandProvider<StrongSubOutletNonRefSteadyEuler2DCons, FluctuationSplitData, FluctSplitNavierStokesModule> strongSubOutletNonRefSteadyEuler2DConsProvider("StrongSubOutletNonRefSteadyEuler2DCons");

//////////////////////////////////////////////////////////////////////////////

StrongSubOutletNonRefSteadyEuler2DCons::StrongSubOutletNonRefSteadyEuler2DCons(const std::string& name) :
...
StrongSubOutletNonRefSteadyEuler2DCons::~StrongSubOutletNonRefSteadyEuler2DCons()
{
  if (isSetup()) unsetup();
}
...
StrongSubOutletNonRefSteadyEuler2DCons::needsSockets()
{
...
void StrongSubOutletNonRefSteadyEuler2DCons::setup()
{
...
void StrongSubOutletNonRefSteadyEuler2DCons::executeOnTrs()
{
...
/********************* Matrix distribution in characteristic ***********************/
	...
	//Editted this function to remove spacetime parameters and change to steady form
	
        computeCharK(*statesInCell, m_kPlus, m_k, m_Statenormals, faceLength, Area);
//         compute the distribution coefficients betas in this cell
	
        m_betas = distributeLDA(*statesInCell, m_kPlus, m_betas, boundaryState);
	
      
        for (CFuint i = 0; i < nbStatesInCell; ++i) {
          resCharElemTwo += (m_k[i])*(acoustics[i]);
          resCharElemThree += (m_k[i])*(omega[i]);
        }
...
void StrongSubOutletNonRefSteadyEuler2DCons::configure ( Config::ConfigArgs& args )
{
...
//removed any spacetime values from this function and inserted steady form of K
...
//void StrongSubOutletNonRefEuler2DCons::computeCharK(std::vector<Framework::State*>& states,   RealVector& _kPlus,   RealVector& _k, RealVector& _kPast, std::vector<RealVector*>& normal, RealVector& faceLength, CFreal& Area)
  void StrongSubOutletNonRefSteadyEuler2DCons::computeCharK(std::vector<Framework::State*>& states,   RealVector& _kPlus,   RealVector& _k, std::vector<RealVector*>& normal, RealVector& faceLength, CFreal& Area)
  {
...
CFreal StrongSubOutletNonRefSteadyEuler2DCons::distributeLDA(std::vector<Framework::State*>& states, RealVector& m_kPlus, CFreal m_betas, CFuint boundarystate){
...
void StrongSubOutletNonRefSteadyEuler2DCons::unsetup()
{
}
