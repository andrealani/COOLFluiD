...
void WeakSlipWall2DHOImpl::defineConfigOptions(Config::OptionList& options)
{
...
WeakSlipWall2DHOImpl::WeakSlipWall2DHOImpl(const std::string& name) :
...
WeakSlipWall2DHOImpl::~WeakSlipWall2DHOImpl()
{
}
...
WeakSlipWall2DHOImpl::needsSockets()
{
...
void WeakSlipWall2DHOImpl::setup()
{
...
void WeakSlipWall2DHOImpl::executeOnTrs()
{
 ...
 for ( CFuint iState = 0; iState < nb_state; ++iState){
    ...
    for (CFuint iEq = 0; iEq < nbEqs; ++iEq) {
      if (!isUpdated[m_state[0]->getLocalID()]){
        rhs(m_state[0]->getLocalID(), iEq, nbEqs) -= m_alpha*onesixth*m_flux[0][iEq] + halfoEminusAlpha*onesixth*m_flux[1][iEq] + halfoEminusAlpha*twothird*m_flux[2][iEq];

        _flagState[m_state[0]->getLocalID()] = true;
      }

      if (!isUpdated[m_state[1]->getLocalID()]){
        rhs(m_state[1]->getLocalID(), iEq, nbEqs) -= m_alpha*onesixth*m_flux[1][iEq] + halfoEminusAlpha*onesixth*m_flux[0][iEq] + halfoEminusAlpha*twothird*m_flux[2][iEq];

        _flagState[m_state[1]->getLocalID()] = true;
      }

      if (!isUpdated[m_state[2]->getLocalID()]){
        rhs(m_state[2]->getLocalID(), iEq, nbEqs) -= m_alpha*twothird*m_flux[2][iEq] + halfoEminusAlpha*onesixth*m_flux[1][iEq] + halfoEminusAlpha*onesixth*m_flux[0][iEq];

        _flagState[m_state[2]->getLocalID()] = true;
      }
...
void WeakSlipWall2DHOImpl::setFaceNormal
