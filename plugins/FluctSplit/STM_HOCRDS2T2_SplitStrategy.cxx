#include "FluctSplit/STM_HOCRDS2T2_SplitStrategy.hh"
...
MethodStrategyProvider<STM_HOCRDS2T2_SplitStrategy,
                       FluctuationSplitData,
                       FluctuationSplitStrategy,
                       FluctSplitSpaceTimeModule>
spaceTimemhocrdS2T2FluctSplitStrategyProvider("STM_HOCRDS2T2");

//////////////////////////////////////////////////////////////////////////////

void STM_HOCRDS2T2_SplitStrategy::defineConfigOptions(Config::OptionList& options)
{
  options.addConfigOption< CFreal>("theta","First coefficient of the time discretization");
  options.addConfigOption< CFreal>("xi"  ,"Second coefficient of the time discretization");
  options.addConfigOption< CFreal>("chi"  ,"Third coefficient of the time discretization");
}

//////////////////////////////////////////////////////////////////////////////

STM_HOCRDS2T2_SplitStrategy::STM_HOCRDS2T2_SplitStrategy(const std::string& name) :
  ...
  temp_vect5(0)
{
  ...
  m_theta = 0.5;
  setParameter("theta",&m_theta);

  m_xi = 0.0;
  setParameter("xi",&m_xi);

  m_chi = 0.0;
  setParameter("chi",&m_chi);
}

//////////////////////////////////////////////////////////////////////////////

STM_HOCRDS2T2_SplitStrategy::~STM_HOCRDS2T2_SplitStrategy()
{
  if (isSetup()) unsetup();
}
...
void STM_HOCRDS2T2_SplitStrategy::computeFluctuation(vector<RealVector>& residual)
{
  CFuint nbIter = SubSystemStatusStack::getActive()->getNbIter();

  if(nbIter == 1) docomputeFirstFluctuation(residual);
  else docomputeFluctuation(residual);
}

//////////////////////////////////////////////////////////////////////////////

void STM_HOCRDS2T2_SplitStrategy::docomputeFirstFluctuation(std::vector<RealVector>& residual){
  ...
  cf_assert(_interStates.size() == 6); // P2 triangles for solution space
  cf_assert(residual.size() >= _interStates.size()); // state residual
  ...
  if (SubSystemStatusStack::getActive()->isFirstStep()){
    ...
    // The residual from the past and the one from the intermediate level are both not changing
    // So, we can store the sum of both in one vector
    ...
    computeHOFluctuation(_interStates);

    /*****         Triangle 0-3-5          *****/

    substates[0] = _interStates[0];
    substates[1] = _interStates[3];
    substates[2] = _interStates[5];
    ...
    for (CFuint iState = 0; iState < nbStatesInCell; ++iState) {
        ddata.cell->setState(iState, _interStates[iState]);
    }
    ...
    *phi = *getMethodData().getSolutionToDistribMatTrans()->transformFromRef(m_phisubT[0]);
    ...
    temp_vect0 = betasInTriag[0]*(*_interStates[0]);
    temp_vect1 = betasInTriag[0]*(*_interStates[1]);
    temp_vect2 = betasInTriag[0]*(*_interStates[2]);
    temp_vect3 = betasInTriag[0]*(*_interStates[3]);
    temp_vect4 = betasInTriag[0]*(*_interStates[4]);
    temp_vect5 = betasInTriag[0]*(*_interStates[5]);

    for (CFuint iEq = 0; iEq < nbEqs; ++iEq) {
      temp_residual[0][iEq] += 0.5*subresidual[0][iEq] -
                                  volume*(kappa1(0,0)*(*_interStates[0])[iEq] + kappab1(0,0)*temp_vect0[iEq]  +
                                          kappa1(0,1)*(*_interStates[1])[iEq] + kappab1(0,1)*temp_vect1[iEq]  +
                                          kappa1(0,2)*(*_interStates[2])[iEq] + kappab1(0,2)*temp_vect2[iEq]  +
                                          kappa1(0,3)*(*_interStates[3])[iEq] + kappab1(0,3)*temp_vect3[iEq]  +
                                          kappa1(0,4)*(*_interStates[4])[iEq] + kappab1(0,4)*temp_vect4[iEq]  +
                                          kappa1(0,5)*(*_interStates[5])[iEq] + kappab1(0,5)*temp_vect5[iEq]) ;


      temp_residual[1][iEq] -= volume*(kappa1(1,0)*(*_interStates[0])[iEq] + kappa1(1,1)*(*_interStates[1])[iEq] +
                                       kappa1(1,2)*(*_interStates[2])[iEq] + kappa1(1,3)*(*_interStates[3])[iEq] +
                                       kappa1(1,4)*(*_interStates[4])[iEq] + kappa1(1,5)*(*_interStates[5])[iEq]) ;

      temp_residual[2][iEq] -= volume*(kappa1(2,0)*(*_interStates[0])[iEq] + kappa1(2,1)*(*_interStates[1])[iEq] +
                                       kappa1(2,2)*(*_interStates[2])[iEq] + kappa1(2,3)*(*_interStates[3])[iEq] +
                                       kappa1(2,4)*(*_interStates[4])[iEq] + kappa1(2,5)*(*_interStates[5])[iEq]) ;
    }

    temp_vect0 = betasInTriag[1]*(*_interStates[0]);
    temp_vect1 = betasInTriag[1]*(*_interStates[1]);
    temp_vect2 = betasInTriag[1]*(*_interStates[2]);
    temp_vect3 = betasInTriag[1]*(*_interStates[3]);
    temp_vect4 = betasInTriag[1]*(*_interStates[4]);
    temp_vect5 = betasInTriag[1]*(*_interStates[5]);

    for (CFuint iEq = 0; iEq < nbEqs; ++iEq) {
      temp_residual[3][iEq] += 0.5*subresidual[1][iEq] -
                                  volume*(kappa1(3,0)*(*_interStates[0])[iEq] +  kappab1(1,0)*temp_vect0[iEq] +
                                          kappa1(3,1)*(*_interStates[1])[iEq] +  kappab1(1,1)*temp_vect1[iEq] +
                                          kappa1(3,2)*(*_interStates[2])[iEq] +  kappab1(1,2)*temp_vect2[iEq] +
                                          kappa1(3,3)*(*_interStates[3])[iEq] +  kappab1(1,3)*temp_vect3[iEq] +
                                          kappa1(3,4)*(*_interStates[4])[iEq] +  kappab1(1,4)*temp_vect4[iEq] +
                                          kappa1(3,5)*(*_interStates[5])[iEq] +  kappab1(1,5)*temp_vect5[iEq] );

      temp_residual[4][iEq] -= volume*(kappa1(4,0)*(*_interStates[0])[iEq] + kappa1(4,1)*(*_interStates[1])[iEq] +
                                       kappa1(4,2)*(*_interStates[2])[iEq] + kappa1(4,3)*(*_interStates[3])[iEq] +
                                       kappa1(4,4)*(*_interStates[4])[iEq] + kappa1(4,5)*(*_interStates[5])[iEq]);
    }

    temp_vect0 = betasInTriag[2]*(*_interStates[0]);
    temp_vect1 = betasInTriag[2]*(*_interStates[1]);
    temp_vect2 = betasInTriag[2]*(*_interStates[2]);
    temp_vect3 = betasInTriag[2]*(*_interStates[3]);
    temp_vect4 = betasInTriag[2]*(*_interStates[4]);
    temp_vect5 = betasInTriag[2]*(*_interStates[5]);

    for (CFuint iEq = 0; iEq < nbEqs; ++iEq) {
      temp_residual[5][iEq] += 0.5*subresidual[2][iEq] -
                                  volume*(kappa1(5,0)*(*_interStates[0])[iEq] + kappab1(2,0)*temp_vect0[iEq] +
                                          kappa1(5,1)*(*_interStates[1])[iEq] + kappab1(2,1)*temp_vect1[iEq] +
                                          kappa1(5,2)*(*_interStates[2])[iEq] + kappab1(2,2)*temp_vect2[iEq] +
                                          kappa1(5,3)*(*_interStates[3])[iEq] + kappab1(2,3)*temp_vect3[iEq] +
                                          kappa1(5,4)*(*_interStates[4])[iEq] + kappab1(2,4)*temp_vect4[iEq] +
                                          kappa1(5,5)*(*_interStates[5])[iEq] + kappab1(2,5)*temp_vect5[iEq]);
    ...
    substates[0] = _interStates[3];
    substates[1] = _interStates[1];
    substates[2] = _interStates[4];
    ...
    *phi = *getMethodData().getSolutionToDistribMatTrans()->transformFromRef(m_phisubT[1]);
    ...
    for (CFuint iEq = 0; iEq < nbEqs; ++iEq) {
      temp_residual[0][iEq] -= volume*(kappa2(0,0)*(*_interStates[0])[iEq] + kappa2(0,1)*(*_interStates[1])[iEq] +
                                       kappa2(0,2)*(*_interStates[2])[iEq] + kappa2(0,3)*(*_interStates[3])[iEq] +
                                       kappa2(0,4)*(*_interStates[4])[iEq] + kappa2(0,5)*(*_interStates[5])[iEq]);
    }
    ...
    for (CFuint iEq = 0; iEq < nbEqs; ++iEq) {
      temp_residual[1][iEq] += 0.5*subresidual[1][iEq] -
                                  volume*(kappa2(1,0)*(*_interStates[0])[iEq] + kappab2(1,0)*temp_vect0[iEq] +
                                          kappa2(1,1)*(*_interStates[1])[iEq] + kappab2(1,1)*temp_vect1[iEq] +
                                          kappa2(1,2)*(*_interStates[2])[iEq] + kappab2(1,2)*temp_vect2[iEq] +
                                          kappa2(1,3)*(*_interStates[3])[iEq] + kappab2(1,3)*temp_vect3[iEq] +
                                          kappa2(1,4)*(*_interStates[4])[iEq] + kappab2(1,4)*temp_vect4[iEq] +
                                          kappa2(1,5)*(*_interStates[5])[iEq] + kappab2(1,5)*temp_vect5[iEq] );

      temp_residual[2][iEq] -= volume*(kappa2(2,0)*(*_interStates[0])[iEq] + kappa2(2,1)*(*_interStates[1])[iEq] +
                                       kappa2(2,2)*(*_interStates[2])[iEq] + kappa2(2,3)*(*_interStates[3])[iEq] +
                                       kappa2(2,4)*(*_interStates[4])[iEq] + kappa2(2,5)*(*_interStates[5])[iEq]) ;
    }
    ...
    for (CFuint iEq = 0; iEq < nbEqs; ++iEq) {
      temp_residual[3][iEq] += 0.5*subresidual[0][iEq] -
                                 volume*(kappa2(3,0)*(*_interStates[0])[iEq] + kappab2(0,0)*temp_vect0[iEq] +
                                         kappa2(3,1)*(*_interStates[1])[iEq] + kappab2(0,1)*temp_vect1[iEq] +
                                         kappa2(3,2)*(*_interStates[2])[iEq] + kappab2(0,2)*temp_vect2[iEq] +
                                         kappa2(3,3)*(*_interStates[3])[iEq] + kappab2(0,3)*temp_vect3[iEq] +
                                         kappa2(3,4)*(*_interStates[4])[iEq] + kappab2(0,4)*temp_vect4[iEq] +
                                         kappa2(3,5)*(*_interStates[5])[iEq] + kappab2(0,5)*temp_vect5[iEq] );
    }
    ...
    for (CFuint iEq = 0; iEq < nbEqs; ++iEq) {
      temp_residual[4][iEq] += 0.5*subresidual[2][iEq] -
                                  volume*(kappa2(4,0)*(*_interStates[0])[iEq] + kappab2(2,0)*temp_vect0[iEq] +
                                          kappa2(4,1)*(*_interStates[1])[iEq] + kappab2(2,1)*temp_vect1[iEq] +
                                          kappa2(4,2)*(*_interStates[2])[iEq] + kappab2(2,2)*temp_vect2[iEq] +
                                          kappa2(4,3)*(*_interStates[3])[iEq] + kappab2(2,3)*temp_vect3[iEq] +
                                          kappa2(4,4)*(*_interStates[4])[iEq] + kappab2(2,4)*temp_vect4[iEq] +
                                          kappa2(4,5)*(*_interStates[5])[iEq] + kappab2(2,5)*temp_vect5[iEq]);

      temp_residual[5][iEq] -= volume*(kappa2(5,0)*(*_interStates[0])[iEq] + kappa2(5,1)*(*_interStates[1])[iEq] +
                                       kappa2(5,2)*(*_interStates[2])[iEq] + kappa2(5,3)*(*_interStates[3])[iEq] +
                                       kappa2(5,4)*(*_interStates[4])[iEq] + kappa2(5,5)*(*_interStates[5])[iEq]) ;
    ...
    substates[0] = _interStates[5];
    substates[1] = _interStates[4];
    substates[2] = _interStates[2];
    ...
    *phi = *getMethodData().getSolutionToDistribMatTrans()->transformFromRef(m_phisubT[2]);
    ...
    for (CFuint iEq = 0; iEq < nbEqs; ++iEq) {
    temp_residual[0][iEq] -= volume*(kappa3(0,0)*(*_interStates[0])[iEq] + kappa3(0,1)*(*_interStates[1])[iEq] +
                                     kappa3(0,2)*(*_interStates[2])[iEq] + kappa3(0,3)*(*_interStates[3])[iEq] +
                                     kappa3(0,4)*(*_interStates[4])[iEq] + kappa3(0,5)*(*_interStates[5])[iEq]);

    temp_residual[1][iEq] -= volume*(kappa3(1,0)*(*_interStates[0])[iEq] + kappa3(1,1)*(*_interStates[1])[iEq] +
                                     kappa3(1,2)*(*_interStates[2])[iEq] + kappa3(1,3)*(*_interStates[3])[iEq] +
                                     kappa3(1,4)*(*_interStates[4])[iEq] + kappa3(1,5)*(*_interStates[5])[iEq]);

    temp_residual[2][iEq] += 0.5*subresidual[2][iEq] -
                                  volume*(kappa3(2,0)*(*_interStates[0])[iEq] + kappab3(2,0)*temp_vect0[iEq] +
                                          kappa3(2,1)*(*_interStates[1])[iEq] + kappab3(2,1)*temp_vect1[iEq] +
                                          kappa3(2,2)*(*_interStates[2])[iEq] + kappab3(2,2)*temp_vect2[iEq] +
                                          kappa3(2,3)*(*_interStates[3])[iEq] + kappab3(2,3)*temp_vect3[iEq] +
                                          kappa3(2,4)*(*_interStates[4])[iEq] + kappab3(2,4)*temp_vect4[iEq] +
                                          kappa3(2,5)*(*_interStates[5])[iEq] + kappab3(2,5)*temp_vect5[iEq]);


    temp_residual[3][iEq] -= volume*(kappa3(3,0)*(*_interStates[0])[iEq] + kappa3(3,1)*(*_interStates[1])[iEq] +
                                     kappa3(3,2)*(*_interStates[2])[iEq] + kappa3(3,3)*(*_interStates[3])[iEq] +
                                     kappa3(3,4)*(*_interStates[4])[iEq] + kappa3(3,5)*(*_interStates[5])[iEq]);
    }
    ...
    for (CFuint iEq = 0; iEq < nbEqs; ++iEq) {
      ...
      temp_residual[4][iEq] += 0.5*subresidual[1][iEq] -
                                  volume*(kappa3(4,0)*(*_interStates[0])[iEq] + kappab3(1,0)*temp_vect0[iEq] +
                                          kappa3(4,1)*(*_interStates[1])[iEq] + kappab3(1,1)*temp_vect1[iEq] +
                                          kappa3(4,2)*(*_interStates[2])[iEq] + kappab3(1,2)*temp_vect2[iEq] +
                                          kappa3(4,3)*(*_interStates[3])[iEq] + kappab3(1,3)*temp_vect3[iEq] +
                                          kappa3(4,4)*(*_interStates[4])[iEq] + kappab3(1,4)*temp_vect4[iEq] +
                                          kappa3(4,5)*(*_interStates[5])[iEq] + kappab3(1,5)*temp_vect5[iEq]) ;
    ...
    for (CFuint iEq = 0; iEq < nbEqs; ++iEq) {
      temp_residual[5][iEq] += 0.5*subresidual[0][iEq] -
                                  volume*(kappa3(5,0)*(*_interStates[0])[iEq] + kappab3(0,0)*temp_vect0[iEq] +
                                          kappa3(5,1)*(*_interStates[1])[iEq] + kappab3(0,1)*temp_vect1[iEq] +
                                          kappa3(5,2)*(*_interStates[2])[iEq] + kappab3(0,2)*temp_vect2[iEq] +
                                          kappa3(5,3)*(*_interStates[3])[iEq] + kappab3(0,3)*temp_vect3[iEq] +
                                          kappa3(5,4)*(*_interStates[4])[iEq] + kappab3(0,4)*temp_vect4[iEq] +
                                          kappa3(5,5)*(*_interStates[5])[iEq] + kappab3(0,5)*temp_vect5[iEq]) ;
   }


   /*****         Triangle 4-5-3          *****/

   substates[0] = _interStates[4];
   substates[1] = _interStates[5];
   substates[2] = _interStates[3];
   ...
   *phi = *getMethodData().getSolutionToDistribMatTrans()->transformFromRef(m_phisubT[3]);
   ...
   for (CFuint iEq = 0; iEq < nbEqs; ++iEq) {
     temp_residual[0][iEq] -= volume*(kappa4(0,0)*(*_interStates[0])[iEq] + kappa4(0,1)*(*_interStates[1])[iEq] +
                                      kappa4(0,2)*(*_interStates[2])[iEq] + kappa4(0,3)*(*_interStates[3])[iEq] +
                                      kappa4(0,4)*(*_interStates[4])[iEq] + kappa4(0,5)*(*_interStates[5])[iEq]);

     temp_residual[1][iEq] -= volume*(kappa4(1,0)*(*_interStates[0])[iEq] + kappa4(1,1)*(*_interStates[1])[iEq] +
                                      kappa4(1,2)*(*_interStates[2])[iEq] + kappa4(1,3)*(*_interStates[3])[iEq] +
                                      kappa4(1,4)*(*_interStates[4])[iEq] + kappa4(1,5)*(*_interStates[5])[iEq]);

     temp_residual[2][iEq] -= volume*(kappa4(2,0)*(*_interStates[0])[iEq] + kappa4(2,1)*(*_interStates[1])[iEq] +
                                      kappa4(2,2)*(*_interStates[2])[iEq] + kappa4(2,3)*(*_interStates[3])[iEq] +
                                      kappa4(2,4)*(*_interStates[4])[iEq] + kappa4(2,5)*(*_interStates[5])[iEq]);

     temp_residual[3][iEq] += 0.5*subresidual[2][iEq] -
                                  volume*(kappa4(3,0)*(*_interStates[0])[iEq] + kappab4(2,0)*temp_vect0[iEq] +
                                          kappa4(3,1)*(*_interStates[1])[iEq] + kappab4(2,1)*temp_vect1[iEq] +
                                          kappa4(3,2)*(*_interStates[2])[iEq] + kappab4(2,2)*temp_vect2[iEq] +
                                          kappa4(3,3)*(*_interStates[3])[iEq] + kappab4(2,3)*temp_vect3[iEq] +
                                          kappa4(3,4)*(*_interStates[4])[iEq] + kappab4(2,4)*temp_vect4[iEq] +
                                          kappa4(3,5)*(*_interStates[5])[iEq] + kappab4(2,5)*temp_vect5[iEq]) ;
   }
   ...
   for (CFuint iEq = 0; iEq < nbEqs; ++iEq) {
     temp_residual[4][iEq] += 0.5*subresidual[0][iEq] -
                                  volume*(kappa4(4,0)*(*_interStates[0])[iEq] + kappab4(0,0)*temp_vect0[iEq] +
                                          kappa4(4,1)*(*_interStates[1])[iEq] + kappab4(0,1)*temp_vect1[iEq] +
                                          kappa4(4,2)*(*_interStates[2])[iEq] + kappab4(0,2)*temp_vect2[iEq] +
                                          kappa4(4,3)*(*_interStates[3])[iEq] + kappab4(0,3)*temp_vect3[iEq] +
                                          kappa4(4,4)*(*_interStates[4])[iEq] + kappab4(0,4)*temp_vect4[iEq] +
                                          kappa4(4,5)*(*_interStates[5])[iEq] + kappab4(0,5)*temp_vect5[iEq]) ;
   }
   ...
   for (CFuint iEq = 0; iEq < nbEqs; ++iEq) {
     temp_residual[5][iEq] += 0.5*subresidual[1][iEq] -
                                 volume*( kappa4(5,0)*(*_interStates[0])[iEq] + kappab4(1,0)*temp_vect0[iEq] +
                                          kappa4(5,1)*(*_interStates[1])[iEq] + kappab4(1,1)*temp_vect1[iEq] +
                                          kappa4(5,2)*(*_interStates[2])[iEq] + kappab4(1,2)*temp_vect2[iEq] +
                                          kappa4(5,3)*(*_interStates[3])[iEq] + kappab4(1,3)*temp_vect3[iEq] +
                                          kappa4(5,4)*(*_interStates[4])[iEq] + kappab4(1,4)*temp_vect4[iEq] +
                                          kappa4(5,5)*(*_interStates[5])[iEq] + kappab4(1,5)*temp_vect5[iEq]) ;
...
//////////////////////////////////////////////////////////////////////////////
 void STM_HOCRDS2T2_SplitStrategy::docomputeFluctuation(std::vector<RealVector>& residual){
///@todo Nv: There is no moving mesh implemented
  DistributionData& ddata = getMethodData().getDistributionData();
  const CFuint nbStatesInCell = ddata.states->size();
  const CFuint nbEqs = PhysicalModelStack::getActive()->getNbEq();
  ...
  for (CFuint i = 0; i < nbStatesInCell; ++i) {
   ...
   computeHOFluctuation(_pastStates);
    ...
    *phi = *getMethodData().getSolutionToDistribMatTrans()->transformFromRef(m_phisubT[0]);
    ...
    for (CFuint iEq = 0; iEq < nbEqs; ++iEq) {
      temp_residual[0][iEq] += (-m_chi)*subresidual[0][iEq] +
                                  (m_xi)*volume*(kappa1(0,0)*(*_pastStates[0])[iEq] + kappab1(0,0)*temp_vect0[iEq]  +
      ...
      temp_residual[1][iEq] += (m_xi)*volume*(kappa1(1,0)*(*_pastStates[0])[iEq] + kappa1(1,1)*(*_pastStates[1])[iEq] +
                                              kappa1(1,2)*(*_pastStates[2])[iEq] + kappa1(1,3)*(*_pastStates[3])[iEq] +
                                              kappa1(1,4)*(*_pastStates[4])[iEq] + kappa1(1,5)*(*_pastStates[5])[iEq]) ;

      temp_residual[2][iEq] += (m_xi)*volume*(kappa1(2,0)*(*_pastStates[0])[iEq] + kappa1(2,1)*(*_pastStates[1])[iEq] +
    ...
    for (CFuint iEq = 0; iEq < nbEqs; ++iEq) {
      temp_residual[3][iEq] += (-m_chi)*subresidual[1][iEq] +
                                  (m_xi)*volume*(kappa1(3,0)*(*_pastStates[0])[iEq] +  kappab1(1,0)*temp_vect0[iEq] +
      ...
      temp_residual[4][iEq] += (m_xi)*volume*(kappa1(4,0)*(*_pastStates[0])[iEq] + kappa1(4,1)*(*_pastStates[1])[iEq] +
    ...
    for (CFuint iEq = 0; iEq < nbEqs; ++iEq) {
      temp_residual[5][iEq] += (-m_chi)*subresidual[2][iEq] +
                                 (m_xi)*volume*(kappa1(5,0)*(*_pastStates[0])[iEq] + kappab1(2,0)*temp_vect0[iEq] +
    ...
    *phi = *getMethodData().getSolutionToDistribMatTrans()->transformFromRef(m_phisubT[1]);
    ...
    for (CFuint iEq = 0; iEq < nbEqs; ++iEq) {
      temp_residual[0][iEq] += (m_xi)*volume*(kappa2(0,0)*(*_pastStates[0])[iEq] + kappa2(0,1)*(*_pastStates[1])[iEq] +
    ...
    for (CFuint iEq = 0; iEq < nbEqs; ++iEq) {
      temp_residual[1][iEq] += (-m_chi)*subresidual[1][iEq] +
                                  (m_xi)*volume*(kappa2(1,0)*(*_pastStates[0])[iEq] + kappab2(1,0)*temp_vect0[iEq] +
      ...
      temp_residual[2][iEq] += (m_xi)*volume*(kappa2(2,0)*(*_pastStates[0])[iEq] + kappa2(2,1)*(*_pastStates[1])[iEq] +
    ...
    for (CFuint iEq = 0; iEq < nbEqs; ++iEq) {
      temp_residual[3][iEq] += (-m_chi)*subresidual[0][iEq] +
                                 (m_xi)*volume*(kappa2(3,0)*(*_pastStates[0])[iEq] + kappab2(0,0)*temp_vect0[iEq] +
    ...
    for (CFuint iEq = 0; iEq < nbEqs; ++iEq) {
      temp_residual[4][iEq] += (-m_chi)*subresidual[2][iEq] +
                                  (m_xi)*volume*(kappa2(4,0)*(*_pastStates[0])[iEq] + kappab2(2,0)*temp_vect0[iEq] +
      ...
      temp_residual[5][iEq] += (m_xi)*volume*(kappa2(5,0)*(*_pastStates[0])[iEq] + kappa2(5,1)*(*_pastStates[1])[iEq] +
    ...
    *phi = *getMethodData().getSolutionToDistribMatTrans()->transformFromRef(m_phisubT[2]);
    ...
    for (CFuint iEq = 0; iEq < nbEqs; ++iEq) {
    temp_residual[0][iEq] += (m_xi)*volume*(kappa3(0,0)*(*_pastStates[0])[iEq] + kappa3(0,1)*(*_pastStates[1])[iEq] +
                                            kappa3(0,2)*(*_pastStates[2])[iEq] + kappa3(0,3)*(*_pastStates[3])[iEq] +
                                            kappa3(0,4)*(*_pastStates[4])[iEq] + kappa3(0,5)*(*_pastStates[5])[iEq]);

    temp_residual[1][iEq] += (m_xi)*volume*(kappa3(1,0)*(*_pastStates[0])[iEq] + kappa3(1,1)*(*_pastStates[1])[iEq] +
                                            kappa3(1,2)*(*_pastStates[2])[iEq] + kappa3(1,3)*(*_pastStates[3])[iEq] +
                                            kappa3(1,4)*(*_pastStates[4])[iEq] + kappa3(1,5)*(*_pastStates[5])[iEq]);

    temp_residual[2][iEq] += (-m_chi)*subresidual[2][iEq] +
                                  (m_xi)*volume*(kappa3(2,0)*(*_pastStates[0])[iEq] + kappab3(2,0)*temp_vect0[iEq] +
    ...
    temp_residual[3][iEq] += (m_xi)*volume*(kappa3(3,0)*(*_pastStates[0])[iEq] + kappa3(3,1)*(*_pastStates[1])[iEq] +
    ...
    for (CFuint iEq = 0; iEq < nbEqs; ++iEq) {
      ...
      temp_residual[4][iEq] += (-m_chi)*subresidual[1][iEq] +
                                  (m_xi)*volume*(kappa3(4,0)*(*_pastStates[0])[iEq] + kappab3(1,0)*temp_vect0[iEq] +
    ...
    for (CFuint iEq = 0; iEq < nbEqs; ++iEq) {
      temp_residual[5][iEq] += (-m_chi)*subresidual[0][iEq] +
                                  (m_xi)*volume*(kappa3(5,0)*(*_pastStates[0])[iEq] + kappab3(0,0)*temp_vect0[iEq] +
   ...
   *phi = *getMethodData().getSolutionToDistribMatTrans()->transformFromRef(m_phisubT[3]);
   ...
   for (CFuint iEq = 0; iEq < nbEqs; ++iEq) {
     temp_residual[0][iEq] += (m_xi)*volume*(kappa4(0,0)*(*_pastStates[0])[iEq] + kappa4(0,1)*(*_pastStates[1])[iEq] +
                                             kappa4(0,2)*(*_pastStates[2])[iEq] + kappa4(0,3)*(*_pastStates[3])[iEq] +
                                             kappa4(0,4)*(*_pastStates[4])[iEq] + kappa4(0,5)*(*_pastStates[5])[iEq]);

     temp_residual[1][iEq] += (m_xi)*volume*(kappa4(1,0)*(*_pastStates[0])[iEq] + kappa4(1,1)*(*_pastStates[1])[iEq] +
                                             kappa4(1,2)*(*_pastStates[2])[iEq] + kappa4(1,3)*(*_pastStates[3])[iEq] +
                                             kappa4(1,4)*(*_pastStates[4])[iEq] + kappa4(1,5)*(*_pastStates[5])[iEq]);

     temp_residual[2][iEq] += (m_xi)*volume*(kappa4(2,0)*(*_pastStates[0])[iEq] + kappa4(2,1)*(*_pastStates[1])[iEq] +
                                             kappa4(2,2)*(*_pastStates[2])[iEq] + kappa4(2,3)*(*_pastStates[3])[iEq] +
                                             kappa4(2,4)*(*_pastStates[4])[iEq] + kappa4(2,5)*(*_pastStates[5])[iEq]);

     temp_residual[3][iEq] += (-m_chi)*subresidual[2][iEq] +
                                  (m_xi)*volume*(kappa4(3,0)*(*_pastStates[0])[iEq] + kappab4(2,0)*temp_vect0[iEq] +
   ...
   for (CFuint iEq = 0; iEq < nbEqs; ++iEq) {
     temp_residual[4][iEq] += (-m_chi)*subresidual[0][iEq] +
                                  (m_xi)*volume*(kappa4(4,0)*(*_pastStates[0])[iEq] + kappab4(0,0)*temp_vect0[iEq] +
   ...
   for (CFuint iEq = 0; iEq < nbEqs; ++iEq) {
     temp_residual[5][iEq] += (-m_chi)*subresidual[1][iEq] +
                                 (m_xi)*volume*( kappa4(5,0)*(*_pastStates[0])[iEq] + kappab4(1,0)*temp_vect0[iEq] +
...
//  The time is the present time (this is used for the source term)
  ddata.time = SubSystemStatusStack::getActive()->getCurrentTime()+dt;
    ...
    *phi = *getMethodData().getSolutionToDistribMatTrans()->transformFromRef(m_phisubT[0]);
    ...
    for (CFuint iEq = 0; iEq < nbEqs; ++iEq) {
      temp_residual[0][iEq] += (1.0 - m_theta + m_chi)*subresidual[0][iEq] -
                                  (1.0 + 2.0*m_xi)*volume*(kappa1(0,0)*(*_interStates[0])[iEq] +
                                                           kappab1(0,0)*temp_vect0[iEq]  +
                                                           kappa1(0,1)*(*_interStates[1])[iEq] +
                                                           kappab1(0,1)*temp_vect1[iEq]  +
                                                           kappa1(0,2)*(*_interStates[2])[iEq] +
                                                           kappab1(0,2)*temp_vect2[iEq]  +
                                                           kappa1(0,3)*(*_interStates[3])[iEq] +
                                                           kappab1(0,3)*temp_vect3[iEq]  +
                                                           kappa1(0,4)*(*_interStates[4])[iEq] +
                                                           kappab1(0,4)*temp_vect4[iEq]  +
                                                           kappa1(0,5)*(*_interStates[5])[iEq] +
                                                           kappab1(0,5)*temp_vect5[iEq]) ;


      temp_residual[1][iEq] -= (1.0 + 2.0*m_xi)*volume*(kappa1(1,0)*(*_interStates[0])[iEq] +
                                                        kappa1(1,1)*(*_interStates[1])[iEq] +
                                                        kappa1(1,2)*(*_interStates[2])[iEq] +
                                                        kappa1(1,3)*(*_interStates[3])[iEq] +
                                                        kappa1(1,4)*(*_interStates[4])[iEq] +
                                                        kappa1(1,5)*(*_interStates[5])[iEq]) ;

      temp_residual[2][iEq] -= (1.0 + 2.0*m_xi)*volume*(kappa1(2,0)*(*_interStates[0])[iEq] +
                                                        kappa1(2,1)*(*_interStates[1])[iEq] +
                                                        kappa1(2,2)*(*_interStates[2])[iEq] +
                                                        kappa1(2,3)*(*_interStates[3])[iEq] +
                                                        kappa1(2,4)*(*_interStates[4])[iEq] +
                                                        kappa1(2,5)*(*_interStates[5])[iEq]) ;
    }
    ...
    for (CFuint iEq = 0; iEq < nbEqs; ++iEq) {
      temp_residual[3][iEq] += (1.0 - m_theta + m_chi)*subresidual[1][iEq] -
                                  (1.0 + 2.0*m_xi)*volume*(kappa1(3,0)*(*_interStates[0])[iEq] +
                                                           kappab1(1,0)*temp_vect0[iEq] +
                                                           kappa1(3,1)*(*_interStates[1])[iEq] +
                                                           kappab1(1,1)*temp_vect1[iEq] +
                                                           kappa1(3,2)*(*_interStates[2])[iEq] +
                                                           kappab1(1,2)*temp_vect2[iEq] +
                                                           kappa1(3,3)*(*_interStates[3])[iEq] +
                                                           kappab1(1,3)*temp_vect3[iEq] +
                                                           kappa1(3,4)*(*_interStates[4])[iEq] +
                                                           kappab1(1,4)*temp_vect4[iEq] +
                                                           kappa1(3,5)*(*_interStates[5])[iEq] +
                                                           kappab1(1,5)*temp_vect5[iEq] );

      temp_residual[4][iEq] -= (1.0 + 2.0*m_xi)*volume*(kappa1(4,0)*(*_interStates[0])[iEq] +
                                                        kappa1(4,1)*(*_interStates[1])[iEq] +
                                                        kappa1(4,2)*(*_interStates[2])[iEq] +
                                                        kappa1(4,3)*(*_interStates[3])[iEq] +
                                                        kappa1(4,4)*(*_interStates[4])[iEq] +
                                                        kappa1(4,5)*(*_interStates[5])[iEq]);
    }
    ...
    for (CFuint iEq = 0; iEq < nbEqs; ++iEq) {
      temp_residual[5][iEq] += (1.0 - m_theta + m_chi)*subresidual[2][iEq] -
                                  (1.0 + 2.0*m_xi)*volume*(kappa1(5,0)*(*_interStates[0])[iEq] +
                                                           kappab1(2,0)*temp_vect0[iEq] +
                                                           kappa1(5,1)*(*_interStates[1])[iEq] +
                                                           kappab1(2,1)*temp_vect1[iEq] +
                                                           kappa1(5,2)*(*_interStates[2])[iEq] +
                                                           kappab1(2,2)*temp_vect2[iEq] +
                                                           kappa1(5,3)*(*_interStates[3])[iEq] +
                                                           kappab1(2,3)*temp_vect3[iEq] +
                                                           kappa1(5,4)*(*_interStates[4])[iEq] +
                                                           kappab1(2,4)*temp_vect4[iEq] +
                                                           kappa1(5,5)*(*_interStates[5])[iEq] +
                                                           kappab1(2,5)*temp_vect5[iEq]);
    ...
    *phi = *getMethodData().getSolutionToDistribMatTrans()->transformFromRef(m_phisubT[1]);
    ...
    for (CFuint iEq = 0; iEq < nbEqs; ++iEq) {
      temp_residual[0][iEq] -= (1.0 + 2.0*m_xi)*volume*(kappa2(0,0)*(*_interStates[0])[iEq] +
                                                        kappa2(0,1)*(*_interStates[1])[iEq] +
                                                        kappa2(0,2)*(*_interStates[2])[iEq] +
                                                        kappa2(0,3)*(*_interStates[3])[iEq] +
                                                        kappa2(0,4)*(*_interStates[4])[iEq] +
                                                        kappa2(0,5)*(*_interStates[5])[iEq]);
    }
    ...
    for (CFuint iEq = 0; iEq < nbEqs; ++iEq) {
      temp_residual[1][iEq] += (1.0 - m_theta + m_chi)*subresidual[1][iEq] -
                                  (1.0 + 2.0*m_xi)*volume*(kappa2(1,0)*(*_interStates[0])[iEq] +
                                                           kappab2(1,0)*temp_vect0[iEq] +
                                                           kappa2(1,1)*(*_interStates[1])[iEq] +
                                                           kappab2(1,1)*temp_vect1[iEq] +
                                                           kappa2(1,2)*(*_interStates[2])[iEq] +
                                                           kappab2(1,2)*temp_vect2[iEq] +
                                                           kappa2(1,3)*(*_interStates[3])[iEq] +
                                                           kappab2(1,3)*temp_vect3[iEq] +
                                                           kappa2(1,4)*(*_interStates[4])[iEq] +
                                                           kappab2(1,4)*temp_vect4[iEq] +
                                                           kappa2(1,5)*(*_interStates[5])[iEq] +
                                                           kappab2(1,5)*temp_vect5[iEq] );

      temp_residual[2][iEq] -= (1.0 + 2.0*m_xi)*volume*(kappa2(2,0)*(*_interStates[0])[iEq] +
                                                        kappa2(2,1)*(*_interStates[1])[iEq] +
                                                        kappa2(2,2)*(*_interStates[2])[iEq] +
                                                        kappa2(2,3)*(*_interStates[3])[iEq] +
                                                        kappa2(2,4)*(*_interStates[4])[iEq] +
                                                        kappa2(2,5)*(*_interStates[5])[iEq]) ;
    }
    ...
    for (CFuint iEq = 0; iEq < nbEqs; ++iEq) {
      temp_residual[3][iEq] += (1.0 - m_theta + m_chi)*subresidual[0][iEq] -
                                 (1.0 + 2.0*m_xi)*volume*(kappa2(3,0)*(*_interStates[0])[iEq] +
                                                          kappab2(0,0)*temp_vect0[iEq] +
                                                          kappa2(3,1)*(*_interStates[1])[iEq] +
                                                          kappab2(0,1)*temp_vect1[iEq] +
                                                          kappa2(3,2)*(*_interStates[2])[iEq] +
                                                          kappab2(0,2)*temp_vect2[iEq] +
                                                          kappa2(3,3)*(*_interStates[3])[iEq] +
                                                          kappab2(0,3)*temp_vect3[iEq] +
                                                          kappa2(3,4)*(*_interStates[4])[iEq] +
                                                          kappab2(0,4)*temp_vect4[iEq] +
                                                          kappa2(3,5)*(*_interStates[5])[iEq] +
                                                          kappab2(0,5)*temp_vect5[iEq] );
    }
    ...
    for (CFuint iEq = 0; iEq < nbEqs; ++iEq) {
      temp_residual[4][iEq] += (1.0 - m_theta + m_chi)*subresidual[2][iEq] -
                                  (1.0 + 2.0*m_xi)*volume*(kappa2(4,0)*(*_interStates[0])[iEq] +
                                                           kappab2(2,0)*temp_vect0[iEq] +
                                                           kappa2(4,1)*(*_interStates[1])[iEq] +
                                                           kappab2(2,1)*temp_vect1[iEq] +
                                                           kappa2(4,2)*(*_interStates[2])[iEq] +
                                                           kappab2(2,2)*temp_vect2[iEq] +
                                                           kappa2(4,3)*(*_interStates[3])[iEq] +
                                                           kappab2(2,3)*temp_vect3[iEq] +
                                                           kappa2(4,4)*(*_interStates[4])[iEq] +
                                                           kappab2(2,4)*temp_vect4[iEq] +
                                                           kappa2(4,5)*(*_interStates[5])[iEq] +
                                                           kappab2(2,5)*temp_vect5[iEq]);

      temp_residual[5][iEq] -= (1.0 + 2.0*m_xi)*volume*(kappa2(5,0)*(*_interStates[0])[iEq] +
                                                        kappa2(5,1)*(*_interStates[1])[iEq] +
                                                        kappa2(5,2)*(*_interStates[2])[iEq] +
                                                        kappa2(5,3)*(*_interStates[3])[iEq] +
                                                        kappa2(5,4)*(*_interStates[4])[iEq] +
                                                        kappa2(5,5)*(*_interStates[5])[iEq]) ;
    ...
    *phi = *getMethodData().getSolutionToDistribMatTrans()->transformFromRef(m_phisubT[2]);
    ...
    for (CFuint iEq = 0; iEq < nbEqs; ++iEq) {
    temp_residual[0][iEq] -= (1.0 + 2.0*m_xi)*volume*(kappa3(0,0)*(*_interStates[0])[iEq] +
                                                      kappa3(0,1)*(*_interStates[1])[iEq] +
                                                      kappa3(0,2)*(*_interStates[2])[iEq] +
                                                      kappa3(0,3)*(*_interStates[3])[iEq] +
                                                      kappa3(0,4)*(*_interStates[4])[iEq] +
                                                      kappa3(0,5)*(*_interStates[5])[iEq]);

    temp_residual[1][iEq] -= (1.0 + 2.0*m_xi)*volume*(kappa3(1,0)*(*_interStates[0])[iEq] +
                                                      kappa3(1,1)*(*_interStates[1])[iEq] +
                                                      kappa3(1,2)*(*_interStates[2])[iEq] +
                                                      kappa3(1,3)*(*_interStates[3])[iEq] +
                                                      kappa3(1,4)*(*_interStates[4])[iEq] +
                                                      kappa3(1,5)*(*_interStates[5])[iEq]);

    temp_residual[2][iEq] += (1.0 - m_theta + m_chi)*subresidual[2][iEq] -
                                  (1.0 + 2.0*m_xi)*volume*(kappa3(2,0)*(*_interStates[0])[iEq] +
                                                           kappab3(2,0)*temp_vect0[iEq] +
                                                           kappa3(2,1)*(*_interStates[1])[iEq] +
                                                           kappab3(2,1)*temp_vect1[iEq] +
                                                           kappa3(2,2)*(*_interStates[2])[iEq] +
                                                           kappab3(2,2)*temp_vect2[iEq] +
                                                           kappa3(2,3)*(*_interStates[3])[iEq] +
                                                           kappab3(2,3)*temp_vect3[iEq] +
                                                           kappa3(2,4)*(*_interStates[4])[iEq] +
                                                           kappab3(2,4)*temp_vect4[iEq] +
                                                           kappa3(2,5)*(*_interStates[5])[iEq] +
                                                           kappab3(2,5)*temp_vect5[iEq]);


    temp_residual[3][iEq] -= (1.0 + 2.0*m_xi)*volume*(kappa3(3,0)*(*_interStates[0])[iEq] +
                                                      kappa3(3,1)*(*_interStates[1])[iEq] +
                                                      kappa3(3,2)*(*_interStates[2])[iEq] +
                                                      kappa3(3,3)*(*_interStates[3])[iEq] +
                                                      kappa3(3,4)*(*_interStates[4])[iEq] +
                                                      kappa3(3,5)*(*_interStates[5])[iEq]);
    }
    ...
    for (CFuint iEq = 0; iEq < nbEqs; ++iEq) {
      ...
      temp_residual[4][iEq] += (1.0 - m_theta + m_chi)*subresidual[1][iEq] -
                                  (1.0 + 2.0*m_xi)*volume*(kappa3(4,0)*(*_interStates[0])[iEq] +
                                                           kappab3(1,0)*temp_vect0[iEq] +
                                                           kappa3(4,1)*(*_interStates[1])[iEq] +
                                                           kappab3(1,1)*temp_vect1[iEq] +
                                                           kappa3(4,2)*(*_interStates[2])[iEq] +
                                                           kappab3(1,2)*temp_vect2[iEq] +
                                                           kappa3(4,3)*(*_interStates[3])[iEq] +
                                                           kappab3(1,3)*temp_vect3[iEq] +
                                                           kappa3(4,4)*(*_interStates[4])[iEq] +
                                                           kappab3(1,4)*temp_vect4[iEq] +
                                                           kappa3(4,5)*(*_interStates[5])[iEq] +
                                                           kappab3(1,5)*temp_vect5[iEq]) ;
    ...
    for (CFuint iEq = 0; iEq < nbEqs; ++iEq) {
      temp_residual[5][iEq] += (1.0 - m_theta + m_chi)*subresidual[0][iEq] -
                                  (1.0 + 2.0*m_xi)*volume*(kappa3(5,0)*(*_interStates[0])[iEq] +
                                                           kappab3(0,0)*temp_vect0[iEq] +
                                                           kappa3(5,1)*(*_interStates[1])[iEq] +
                                                           kappab3(0,1)*temp_vect1[iEq] +
                                                           kappa3(5,2)*(*_interStates[2])[iEq] +
                                                           kappab3(0,2)*temp_vect2[iEq] +
                                                           kappa3(5,3)*(*_interStates[3])[iEq] +
                                                           kappab3(0,3)*temp_vect3[iEq] +
                                                           kappa3(5,4)*(*_interStates[4])[iEq] +
                                                           kappab3(0,4)*temp_vect4[iEq] +
                                                           kappa3(5,5)*(*_interStates[5])[iEq] +
                                                           kappab3(0,5)*temp_vect5[iEq]) ;
    }
   ...
   *phi = *getMethodData().getSolutionToDistribMatTrans()->transformFromRef(m_phisubT[3]);
   ...
   for (CFuint iEq = 0; iEq < nbEqs; ++iEq) {
     temp_residual[0][iEq] -= (1.0 + 2.0*m_xi)*volume*(kappa4(0,0)*(*_interStates[0])[iEq] +
                                                       kappa4(0,1)*(*_interStates[1])[iEq] +
                                                       kappa4(0,2)*(*_interStates[2])[iEq] +
                                                       kappa4(0,3)*(*_interStates[3])[iEq] +
                                                       kappa4(0,4)*(*_interStates[4])[iEq] +
                                                       kappa4(0,5)*(*_interStates[5])[iEq]);

     temp_residual[1][iEq] -= (1.0 + 2.0*m_xi)*volume*(kappa4(1,0)*(*_interStates[0])[iEq] +
                                                       kappa4(1,1)*(*_interStates[1])[iEq] +
                                                       kappa4(1,2)*(*_interStates[2])[iEq] +
                                                       kappa4(1,3)*(*_interStates[3])[iEq] +
                                                       kappa4(1,4)*(*_interStates[4])[iEq] +
                                                       kappa4(1,5)*(*_interStates[5])[iEq]);

     temp_residual[2][iEq] -= (1.0 + 2.0*m_xi)*volume*(kappa4(2,0)*(*_interStates[0])[iEq] +
                                                       kappa4(2,1)*(*_interStates[1])[iEq] +
                                                       kappa4(2,2)*(*_interStates[2])[iEq] +
                                                       kappa4(2,3)*(*_interStates[3])[iEq] +
                                                       kappa4(2,4)*(*_interStates[4])[iEq] +
                                                       kappa4(2,5)*(*_interStates[5])[iEq]);

     temp_residual[3][iEq] += (1.0 - m_theta + m_chi)*subresidual[2][iEq] -
                                  (1.0 + 2.0*m_xi)*volume*(kappa4(3,0)*(*_interStates[0])[iEq] +
                                                           kappab4(2,0)*temp_vect0[iEq] +
                                                           kappa4(3,1)*(*_interStates[1])[iEq] +
                                                           kappab4(2,1)*temp_vect1[iEq] +
                                                           kappa4(3,2)*(*_interStates[2])[iEq] +
                                                           kappab4(2,2)*temp_vect2[iEq] +
                                                           kappa4(3,3)*(*_interStates[3])[iEq] +
                                                           kappab4(2,3)*temp_vect3[iEq] +
                                                           kappa4(3,4)*(*_interStates[4])[iEq] +
                                                           kappab4(2,4)*temp_vect4[iEq] +
                                                           kappa4(3,5)*(*_interStates[5])[iEq] +
                                                           kappab4(2,5)*temp_vect5[iEq]) ;
   }
   ...
   for (CFuint iEq = 0; iEq < nbEqs; ++iEq) {
     temp_residual[4][iEq] += (1.0 - m_theta + m_chi)*subresidual[0][iEq] -
                                  (1.0 + 2.0*m_xi)*volume*(kappa4(4,0)*(*_interStates[0])[iEq] +
                                                           kappab4(0,0)*temp_vect0[iEq] +
                                                           kappa4(4,1)*(*_interStates[1])[iEq] +
                                                           kappab4(0,1)*temp_vect1[iEq] +
                                                           kappa4(4,2)*(*_interStates[2])[iEq] +
                                                           kappab4(0,2)*temp_vect2[iEq] +
                                                           kappa4(4,3)*(*_interStates[3])[iEq] +
                                                           kappab4(0,3)*temp_vect3[iEq] +
                                                           kappa4(4,4)*(*_interStates[4])[iEq] +
                                                           kappab4(0,4)*temp_vect4[iEq] +
                                                           kappa4(4,5)*(*_interStates[5])[iEq] +
                                                           kappab4(0,5)*temp_vect5[iEq]) ;
   }
   ...
   for (CFuint iEq = 0; iEq < nbEqs; ++iEq) {
     temp_residual[5][iEq] += (1.0 - m_theta + m_chi)*subresidual[1][iEq] -
                                 (1.0 + 2.0*m_xi)*volume*( kappa4(5,0)*(*_interStates[0])[iEq] +
                                                           kappab4(1,0)*temp_vect0[iEq] +
                                                           kappa4(5,1)*(*_interStates[1])[iEq] +
                                                           kappab4(1,1)*temp_vect1[iEq] +
                                                           kappa4(5,2)*(*_interStates[2])[iEq] +
                                                           kappab4(1,2)*temp_vect2[iEq] +
                                                           kappa4(5,3)*(*_interStates[3])[iEq] +
                                                           kappab4(1,3)*temp_vect3[iEq] +
                                                           kappa4(5,4)*(*_interStates[4])[iEq] +
                                                           kappab4(1,4)*temp_vect4[iEq] +
                                                           kappa4(5,5)*(*_interStates[5])[iEq] +
                                                           kappab4(1,5)*temp_vect5[iEq]) ;
...
//  The time is the present time (this is used for the source term)
  ...
  betasInTriag = ddata.betaMats[0];
    ...
    for (CFuint iEq = 0; iEq < nbEqs; ++iEq) {
  residual[0][iEq] += m_theta*subresidual[0][iEq] +
                                  (1.0 + m_xi)*volume*(kappa1(0,0)*(*states[0])[iEq] + kappab1(0,0)*temp_vect0[iEq] +
                                                       kappa1(0,1)*(*states[1])[iEq] + kappab1(0,1)*temp_vect1[iEq] +
                                                       kappa1(0,2)*(*states[2])[iEq] + kappab1(0,2)*temp_vect2[iEq] +
                                                       kappa1(0,3)*(*states[3])[iEq] + kappab1(0,3)*temp_vect3[iEq] +
                                                       kappa1(0,4)*(*states[4])[iEq] + kappab1(0,4)*temp_vect4[iEq] +
   ...
   residual[1][iEq] += (1.0 + m_xi)*volume*(kappa1(1,0)*(*states[0])[iEq] + kappa1(1,1)*(*states[1])[iEq] +
                                            kappa1(1,2)*(*states[2])[iEq] + kappa1(1,3)*(*states[3])[iEq] +
                                            kappa1(1,4)*(*states[4])[iEq] + kappa1(1,5)*(*states[5])[iEq]) ;

   residual[2][iEq] += (1.0 + m_xi)*volume*(kappa1(2,0)*(*states[0])[iEq] + kappa1(2,1)*(*states[1])[iEq] +
                                            kappa1(2,2)*(*states[2])[iEq] + kappa1(2,3)*(*states[3])[iEq] +
                                            kappa1(2,4)*(*states[4])[iEq] + kappa1(2,5)*(*states[5])[iEq]) ;
}
 ...
 temp_vect5 = betasInTriag[1]*(*states[5]);
         ...
         for (CFuint iEq = 0; iEq < nbEqs; ++iEq) {
   residual[3][iEq] += m_theta*subresidual[1][iEq] +
                                  (1.0 + m_xi)*volume*(kappa1(3,0)*(*states[0])[iEq] + kappab1(1,0)*temp_vect0[iEq] +
                                                       kappa1(3,1)*(*states[1])[iEq] + kappab1(1,1)*temp_vect1[iEq] +
                                                       kappa1(3,2)*(*states[2])[iEq] + kappab1(1,2)*temp_vect2[iEq] +
                                                       kappa1(3,3)*(*states[3])[iEq] + kappab1(1,3)*temp_vect3[iEq] +
                                                       kappa1(3,4)*(*states[4])[iEq] + kappab1(1,4)*temp_vect4[iEq] +
   ...
   residual[4][iEq] += (1.0 + m_xi)*volume*(kappa1(4,0)*(*states[0])[iEq] + kappa1(4,1)*(*states[1])[iEq] +
                                            kappa1(4,2)*(*states[2])[iEq] + kappa1(4,3)*(*states[3])[iEq] +
                                            kappa1(4,4)*(*states[4])[iEq] + kappa1(4,5)*(*states[5])[iEq]) ;
               }
...
temp_vect5 = betasInTriag[2]*(*states[5]);
 for (CFuint iEq = 0; iEq < nbEqs; ++iEq) {
   residual[5][iEq] += m_theta*subresidual[2][iEq] +
                                  (1.0 + m_xi)*volume*(kappa1(5,0)*(*states[0])[iEq] + kappab1(2,0)*temp_vect0[iEq] +
                                                       kappa1(5,1)*(*states[1])[iEq] + kappab1(2,1)*temp_vect1[iEq] +
                                                       kappa1(5,2)*(*states[2])[iEq] + kappab1(2,2)*temp_vect2[iEq] +
                                                       kappa1(5,3)*(*states[3])[iEq] + kappab1(2,3)*temp_vect3[iEq] +
                                                       kappa1(5,4)*(*states[4])[iEq] + kappab1(2,4)*temp_vect4[iEq] +
...
temp_vect5 = betasInTriag[1]*(*states[5]);
 for (CFuint iEq = 0; iEq < nbEqs; ++iEq) {
   residual[0][iEq] += (1.0 + m_xi)*volume*(kappa2(0,0)*(*states[0])[iEq] + kappa2(0,1)*(*states[1])[iEq] +
                                            kappa2(0,2)*(*states[2])[iEq] + kappa2(0,3)*(*states[3])[iEq] +
                                            kappa2(0,4)*(*states[4])[iEq] + kappa2(0,5)*(*states[5])[iEq]);

  residual[1][iEq]+= m_theta*subresidual[1][iEq] +
                                  (1.0 + m_xi)*volume*(kappa2(1,0)*(*states[0])[iEq] + kappab2(1,0)*temp_vect0[iEq] +
                                                       kappa2(1,1)*(*states[1])[iEq] + kappab2(1,1)*temp_vect1[iEq] +
                                                       kappa2(1,2)*(*states[2])[iEq] + kappab2(1,2)*temp_vect2[iEq] +
                                                       kappa2(1,3)*(*states[3])[iEq] + kappab2(1,3)*temp_vect3[iEq] +
                                                       kappa2(1,4)*(*states[4])[iEq] + kappab2(1,4)*temp_vect4[iEq] +
   ...
   residual[2][iEq] += (1.0 + m_xi)*volume*(kappa2(2,0)*(*states[0])[iEq] + kappa2(2,1)*(*states[1])[iEq] +
                                            kappa2(2,2)*(*states[2])[iEq] + kappa2(2,3)*(*states[3])[iEq] +
                                            kappa2(2,4)*(*states[4])[iEq] + kappa2(2,5)*(*states[5])[iEq]) ;
}
...
temp_vect5 = betasInTriag[0]*(*states[5]);
 for (CFuint iEq = 0; iEq < nbEqs; ++iEq) {
   residual[3][iEq] += m_theta*subresidual[0][iEq] +
                                 (1.0 + m_xi)*volume*(kappa2(3,0)*(*states[0])[iEq] + kappab2(0,0)*temp_vect0[iEq] +
                                                      kappa2(3,1)*(*states[1])[iEq] + kappab2(0,1)*temp_vect1[iEq] +
                                                      kappa2(3,2)*(*states[2])[iEq] + kappab2(0,2)*temp_vect2[iEq] +
                                                      kappa2(3,3)*(*states[3])[iEq] + kappab2(0,3)*temp_vect3[iEq] +
                                                      kappa2(3,4)*(*states[4])[iEq] + kappab2(0,4)*temp_vect4[iEq] +
...
temp_vect5 = betasInTriag[2]*(*states[5]);
 for (CFuint iEq = 0; iEq < nbEqs; ++iEq) {
   residual[4][iEq] += m_theta*subresidual[2][iEq] +
                                  (1.0 + m_xi)*volume*(kappa2(4,0)*(*states[0])[iEq] + kappab2(2,0)*temp_vect0[iEq] +
                                                       kappa2(4,1)*(*states[1])[iEq] + kappab2(2,1)*temp_vect1[iEq] +
                                                       kappa2(4,2)*(*states[2])[iEq] + kappab2(2,2)*temp_vect2[iEq] +
                                                       kappa2(4,3)*(*states[3])[iEq] + kappab2(2,3)*temp_vect3[iEq] +
                                                       kappa2(4,4)*(*states[4])[iEq] + kappab2(2,4)*temp_vect4[iEq] +
   ...
   residual[5][iEq] += (1.0 + m_xi)*volume*(kappa2(5,0)*(*states[0])[iEq] + kappa2(5,1)*(*states[1])[iEq] +
                                            kappa2(5,2)*(*states[2])[iEq] + kappa2(5,3)*(*states[3])[iEq] +
                                            kappa2(5,4)*(*states[4])[iEq] + kappa2(5,5)*(*states[5])[iEq]) ;
}
...
temp_vect5 = betasInTriag[2]*(*states[5]);
 for (CFuint iEq = 0; iEq < nbEqs; ++iEq) {
   residual[0][iEq] += (1.0 + m_xi)*volume*(kappa3(0,0)*(*states[0])[iEq] + kappa3(0,1)*(*states[1])[iEq] +
                                            kappa3(0,2)*(*states[2])[iEq] + kappa3(0,3)*(*states[3])[iEq] +
                                            kappa3(0,4)*(*states[4])[iEq] + kappa3(0,5)*(*states[5])[iEq]);

   residual[1][iEq] += (1.0 + m_xi)*volume*(kappa3(1,0)*(*states[0])[iEq] + kappa3(1,1)*(*states[1])[iEq] +
                                            kappa3(1,2)*(*states[2])[iEq] + kappa3(1,3)*(*states[3])[iEq] +
                                            kappa3(1,4)*(*states[4])[iEq] + kappa3(1,5)*(*states[5])[iEq]);

   residual[2][iEq] += m_theta*subresidual[2][iEq] +
                                  (1.0 + m_xi)*volume*(kappa3(2,0)*(*states[0])[iEq] + kappab3(2,0)*temp_vect0[iEq] +
                                                       kappa3(2,1)*(*states[1])[iEq] + kappab3(2,1)*temp_vect1[iEq] +
                                                       kappa3(2,2)*(*states[2])[iEq] + kappab3(2,2)*temp_vect2[iEq] +
                                                       kappa3(2,3)*(*states[3])[iEq] + kappab3(2,3)*temp_vect3[iEq] +
                                                       kappa3(2,4)*(*states[4])[iEq] + kappab3(2,4)*temp_vect4[iEq] +
   ...
   residual[3][iEq] += (1.0 + m_xi)*volume*(kappa3(3,0)*(*states[0])[iEq] + kappa3(3,1)*(*states[1])[iEq] +
                                            kappa3(3,2)*(*states[2])[iEq] + kappa3(3,3)*(*states[3])[iEq] +
                                            kappa3(3,4)*(*states[4])[iEq] + kappa3(3,5)*(*states[5])[iEq]);
    }
...
temp_vect5 = betasInTriag[1]*(*states[5]);
 ...
 for (CFuint iEq = 0; iEq < nbEqs; ++iEq) {
   residual[4][iEq] += m_theta*subresidual[1][iEq]+
                                  (1.0 + m_xi)*volume*(kappa3(4,0)*(*states[0])[iEq] + kappab3(1,0)*temp_vect0[iEq] +
                                                       kappa3(4,1)*(*states[1])[iEq] + kappab3(1,1)*temp_vect1[iEq] +
                                                       kappa3(4,2)*(*states[2])[iEq] + kappab3(1,2)*temp_vect2[iEq] +
                                                       kappa3(4,3)*(*states[3])[iEq] + kappab3(1,3)*temp_vect3[iEq] +
                                                       kappa3(4,4)*(*states[4])[iEq] + kappab3(1,4)*temp_vect4[iEq] +
...
temp_vect5 = betasInTriag[0]*(*states[5]);
 ...
 for (CFuint iEq = 0; iEq < nbEqs; ++iEq) {
   ...
   residual[5][iEq] += m_theta*subresidual[0][iEq]+
                                  (1.0 + m_xi)*volume*(kappa3(5,0)*(*states[0])[iEq] + kappab3(0,0)*temp_vect0[iEq] +
                                                       kappa3(5,1)*(*states[1])[iEq] + kappab3(0,1)*temp_vect1[iEq] +
                                                       kappa3(5,2)*(*states[2])[iEq] + kappab3(0,2)*temp_vect2[iEq] +
                                                       kappa3(5,3)*(*states[3])[iEq] + kappab3(0,3)*temp_vect3[iEq] +
                                                       kappa3(5,4)*(*states[4])[iEq] + kappab3(0,4)*temp_vect4[iEq] +
...
temp_vect5 = betasInTriag[2]*(*states[5]);
 ...
 for (CFuint iEq = 0; iEq < nbEqs; ++iEq) {
   residual[0][iEq] += (1.0 + m_xi)*volume*(kappa4(0,0)*(*states[0])[iEq] + kappa4(0,1)*(*states[1])[iEq] +
                                            kappa4(0,2)*(*states[2])[iEq] + kappa4(0,3)*(*states[3])[iEq] +
                                            kappa4(0,4)*(*states[4])[iEq] + kappa4(0,5)*(*states[5])[iEq]);

   residual[1][iEq] += (1.0 + m_xi)*volume*(kappa4(1,0)*(*states[0])[iEq] + kappa4(1,1)*(*states[1])[iEq] +
                                            kappa4(1,2)*(*states[2])[iEq] + kappa4(1,3)*(*states[3])[iEq] +
                                            kappa4(1,4)*(*states[4])[iEq] + kappa4(1,5)*(*states[5])[iEq]);

   residual[2][iEq] += (1.0 + m_xi)*volume*(kappa4(2,0)*(*states[0])[iEq] + kappa4(2,1)*(*states[1])[iEq] +
                                            kappa4(2,2)*(*states[2])[iEq] + kappa4(2,3)*(*states[3])[iEq] +
                                            kappa4(2,4)*(*states[4])[iEq] + kappa4(2,5)*(*states[5])[iEq]);

   residual[3][iEq] += m_theta*subresidual[2][iEq] +
                                  (1.0 + m_xi)*volume*(kappa4(3,0)*(*states[0])[iEq] + kappab4(2,0)*temp_vect0[iEq] +
                                                       kappa4(3,1)*(*states[1])[iEq] + kappab4(2,1)*temp_vect1[iEq] +
                                                       kappa4(3,2)*(*states[2])[iEq] + kappab4(2,2)*temp_vect2[iEq] +
                                                       kappa4(3,3)*(*states[3])[iEq] + kappab4(2,3)*temp_vect3[iEq] +
                                                       kappa4(3,4)*(*states[4])[iEq] + kappab4(2,4)*temp_vect4[iEq] +
...
temp_vect5 = betasInTriag[0]*(*states[5]);
 ...
 for (CFuint iEq = 0; iEq < nbEqs; ++iEq) {
   residual[4][iEq] += m_theta*subresidual[0][iEq] +
                                  (1.0 + m_xi)*volume*(kappa4(4,0)*(*states[0])[iEq] + kappab4(0,0)*temp_vect0[iEq]  +
                                                       kappa4(4,1)*(*states[1])[iEq] + kappab4(0,1)*temp_vect1[iEq]  +
                                                       kappa4(4,2)*(*states[2])[iEq] + kappab4(0,2)*temp_vect2[iEq]  +
                                                       kappa4(4,3)*(*states[3])[iEq] + kappab4(0,3)*temp_vect3[iEq]  +
                                                       kappa4(4,4)*(*states[4])[iEq] + kappab4(0,4)*temp_vect4[iEq]  +
...
temp_vect5 = betasInTriag[1]*(*states[5]);
 ...
 for (CFuint iEq = 0; iEq < nbEqs; ++iEq) {
   residual[5][iEq] += m_theta*subresidual[1][iEq] +
                                  (1.0 + m_xi)*volume*(kappa4(5,0)*(*states[0])[iEq] + kappab4(1,0)*temp_vect0[iEq] +
                                                       kappa4(5,1)*(*states[1])[iEq] + kappab4(1,1)*temp_vect1[iEq] +
                                                       kappa4(5,2)*(*states[2])[iEq] + kappab4(1,2)*temp_vect2[iEq] +
                                                       kappa4(5,3)*(*states[3])[iEq] + kappab4(1,3)*temp_vect3[iEq] +
                                                       kappa4(5,4)*(*states[4])[iEq] + kappab4(1,4)*temp_vect4[iEq] +
...
void STM_HOCRDS2T2_SplitStrategy::computeHOFluctuation(vector<State*> & states)
{
DistributionData& ddata = getMethodData().getDistributionData();
...
for (CFuint iFace = 0; iFace < subfacetable.nbRows(); ++iFace)
  {
    ...
    computeStatesData(3, m_updateVar, m_qdstates,  m_pdata, qdExtraVars);
...
void STM_HOCRDS2T2_SplitStrategy::unsetup()
{
...
void STM_HOCRDS2T2_SplitStrategy::setup()
{
  CFAUTOTRACE;
...
STM_HOCRDS2T2_SplitStrategy::needsSockets()
{
...
void STM_HOCRDS2T2_SplitStrategy::setCurrentCell()
{
  DistributionData& ddata = getMethodData().getDistributionData();
