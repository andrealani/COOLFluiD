...
using namespace COOLFluiD::Physics::LinearAdvSys;
...
MethodStrategyProvider<LinearAdvDiffHO3D,
		       FluctuationSplitData,
		       ComputeDiffusiveTerm,
		       FluctSplitAdvectionDiffusionModule>
linearadvDiffusiveTermHO3DProvider("LinearAdvHO3D");

//////////////////////////////////////////////////////////////////////////////

LinearAdvDiffHO3D::LinearAdvDiffHO3D(const std::string& name) :
...
LinearAdvDiffHO3D::~LinearAdvDiffHO3D()
{
...
void LinearAdvDiffHO3D::setDiffusiveVarSet(SafePtr<DiffusiveVarSet> diffVar)
{
  _diffVar = diffVar.d_castTo<Physics::LinearAdvSys::AdvectionDiffusionSysVarSet>();
}

//////////////////////////////////////////////////////////////////////////////

void LinearAdvDiffHO3D::setUpdateVarSet(SafePtr<ConvectiveVarSet> updateVar)
{
  _updateVar = updateVar.d_castTo<Physics::LinearAdvSys::LinearAdvSysVarSet>();
}


//////////////////////////////////////////////////////////////////////////////

void LinearAdvDiffHO3D::computeDiffusiveTerm
(GeometricEntity *const geo, vector<RealVector>& result, bool updateCoeffFlag)
{
 ...
 DistributionData& distdata = getMethodData().getDistributionData();
  ...
  ADSysTerm& model = _diffVar->getModel();
  const CFreal mu = model.getPhysicalData()[ADSysTerm::NU];
...
//////////////////////////////////////////////////////////////////////////////
void LinearAdvDiffHO3D::fluctuation_diff_galerkin(CFuint& i1, CFuint& i2 , CFuint& i3, CFuint & i4, bool triag){
 ...
 // const CFreal one_eighth = 1.0/8.0;
  ...
  for (CFuint iQd = 0; iQd < 4; ++iQd) {
     ...
     grad_state_z.resize(4);


         grad_state_x[0] = (nx0*(4.0*L0 - 1.0)*(*_states[0])[0] +
		            nx1*(4.0*L1 - 1.0)*(*_states[1])[0] +
		            nx2*(4.0*L2 - 1.0)*(*_states[2])[0] +
			    nx3*(4.0*L3 - 1.0)*(*_states[3])[0] +
		            4.0*(nx1*L0 + nx0*L1)*(*_states[4])[0] +
		            4.0*(nx2*L1 + nx1*L2)*(*_states[5])[0] +
		            4.0*(nx0*L2 + nx2*L0)*(*_states[6])[0] +
                            4.0*(nx1*L3 + nx3*L1)*(*_states[7])[0] +
                            4.0*(nx2*L3 + nx3*L2)*(*_states[8])[0] +
                            4.0*(nx0*L3 + nx3*L0)*(*_states[9])[0])*inv_sixvolume;

        grad_state_y[0] = (ny0*(4.0*L0 - 1.0)*(*_states[0])[0] +
                            ny1*(4.0*L1 - 1.0)*(*_states[1])[0] +
                            ny2*(4.0*L2 - 1.0)*(*_states[2])[0] +
                            ny3*(4.0*L3 - 1.0)*(*_states[3])[0] +
                            4.0*(ny1*L0 + ny0*L1)*(*_states[4])[0] +
                            4.0*(ny2*L1 + ny1*L2)*(*_states[5])[0] +
                            4.0*(ny0*L2 + ny2*L0)*(*_states[6])[0] +
                            4.0*(ny1*L3 + ny3*L1)*(*_states[7])[0] +
                            4.0*(ny2*L3 + ny3*L2)*(*_states[8])[0] +
                            4.0*(ny0*L3 + ny3*L0)*(*_states[9])[0])*inv_sixvolume;

  grad_state_z[0] = (nz0*(4.0*L0 - 1.0)*(*_states[0])[0] +
                            nz1*(4.0*L1 - 1.0)*(*_states[1])[0] +
                            nz2*(4.0*L2 - 1.0)*(*_states[2])[0] +
                            nz3*(4.0*L3 - 1.0)*(*_states[3])[0] +
                            4.0*(nz1*L0 + nz0*L1)*(*_states[4])[0] +
                            4.0*(nz2*L1 + nz1*L2)*(*_states[5])[0] +
                            4.0*(nz0*L2 + nz2*L0)*(*_states[6])[0] +
                            4.0*(nz1*L3 + nz3*L1)*(*_states[7])[0] +
                            4.0*(nz2*L3 + nz3*L2)*(*_states[8])[0] +
                            4.0*(nz0*L3 + nz3*L0)*(*_states[9])[0])*inv_sixvolume;
...
//////////////////////////////////////////////////////////////////////////////
void LinearAdvDiffHO3D::fluctuation_diff_bubble(CFuint& i1, CFuint& i2, CFuint& i3, CFuint& i4, bool triag)
{
    ...
    (qdstates) = (L0*( 2.0*L0 - 1.0 ) * (*_states[0])) +
     ...
     CFreal coeff = 1.0/6.0;
       ...
       for (CFuint ieq = 0 ; ieq < 4; ++ieq)
         {
          (*_phi_diff_bub)[ieq] += 256.0*(F0[ieq]*L1bub*L2bub*L3bub + F1[ieq]*L0bub*L2bub*L3bub + F2[ieq]*L0bub*L1bub*L3bub + F3[ieq]*L0bub*L1bub*L2bub)*coeff*wqd[iQd];
...
void LinearAdvDiffHO3D::configure ( Config::ConfigArgs& args )
{
