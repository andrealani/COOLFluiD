#include "FluctSplit/STU_CRD_SplitStrategyhyb.hh"
...
/////////////////////////////////////////////////////////////////////////////
MethodStrategyProvider<STU_CRD_SplitStrategyhyb,
                       FluctuationSplitData,
                       FluctuationSplitStrategy,
                       FluctSplitSpaceTimeModule>
spaceTimeucrdhybFluctSplitStrategyProvider("STU_CRD_hyb");

//////////////////////////////////////////////////////////////////////////////

STU_CRD_SplitStrategyhyb::STU_CRD_SplitStrategyhyb(const std::string& name) :
...
STU_CRD_SplitStrategyhyb::~STU_CRD_SplitStrategyhyb()
{
  if (isSetup()) unsetup();
}
...
void STU_CRD_SplitStrategyhyb::computeFluctuation(vector<RealVector>& residual)
{
  ///@todo Nv: There is no moving mesh implemented
  //   std::cout<<"Starting to compute the residual"<<std::endl;
  DistributionData& ddata = getMethodData().getDistributionData();
  m_splitter->setDT(SubSystemStatusStack::getActive()->getDT());
  DataHandle<InwardNormalsData*> normals = socket_normals.getDataHandle();
  DataHandle<CFreal> volumes = socket_volumes.getDataHandle();
  _nbStatesInCell = ddata.states->size();
  ...
  //  std::cout<<"All volumes are setted"<<std::endl;
  ...
  if (SubSystemStatusStack::getActive()->isFirstStep()){
    ...
    //  std::cout<<"Compute consistent states"<<std::endl;
        tPastStates = computeConsistentStates(&Cell_states);
    // m_splitter->setConsStates(_pastStates);
	//   std::cout<<"Compute Flux"<<std::endl;
    computeSTFluxIntegral(true);
    
    //    std::cout<<"Compute past residual"<<std::endl;
  ...
  //   std::cout<<"Compute present residual"<<std::endl;
  ddata.phi = m_flux;
  m_splitter->computeK(*ddata.states, normals[ddata.cellID]);
  //  std::cout<<"K compued"<<std::endl;
  ...
  // std::cout<<"phi distributed"<<std::endl;
}
    
//////////////////////////////////////////////////////////////////////////////

void STU_CRD_SplitStrategyhyb::unsetup()
{
...
void STU_CRD_SplitStrategyhyb::setup()
{
  CFAUTOTRACE;
...
STU_CRD_SplitStrategyhyb::needsSockets()
{
...
void STU_CRD_SplitStrategyhyb::setCurrentCell()
{
  DistributionData& ddata = getMethodData().getDistributionData();
...
void STU_CRD_SplitStrategyhyb::computeSTFluxIntegral(bool isPast)
{ 

  
  DistributionData& ddata = getMethodData().getDistributionData();
    const CFuint nbCellStates = ddata.states->size();
	const CFreal Area = CellVolume/nbCellStates;
