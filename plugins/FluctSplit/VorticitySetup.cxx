...
#include "FluctSplit/VorticitySetup.hh"
...
MethodCommandProvider<VorticitySetup, FluctuationSplitData, FluctSplitModule> vorticitySetupProvider("VorticitySetup");

//////////////////////////////////////////////////////////////////////////////

void VorticitySetup::defineConfigOptions(Config::OptionList& options)
{
  options.addConfigOption< std::string >
    ("VorticityName","name of the input limiter socket");
}
//////////////////////////////////////////////////////////////////////////////

VorticitySetup::VorticitySetup(const std::string& name) :
  ...
  socket_flagStates("flagStates")
{
 addConfigOptionsTo(this);
  ...
  _vorticitySocketName = "Null";
   setParameter("VorticityName",&_vorticitySocketName);
}

//////////////////////////////////////////////////////////////////////////////

VorticitySetup::~VorticitySetup()
{
}
...
VorticitySetup::providesSockets()
{
...
void VorticitySetup::configure ( Config::ConfigArgs& args )
{
  CFAUTOTRACE;
  ...
  if (_vorticitySocketName != "Null") {
    m_dynamicSockets.createSocketSink<CFreal>(_vorticitySocketName, true);
  }
...
void VorticitySetup::execute()
{
  ...
  computeCellVolumes();
   ...
   const CFuint sizevorticity = nbStates;
   DataHandle<CFreal> vorticity = socket_vorticity.getDataHandle();
     vorticity.resize(sizevorticity);
     
   for(CFuint i = 0; i < sizevorticity; ++i) {
     if (dim==2) {
       vorticity[i].resize(dim);
     }
     if (dim==3) {
       vorticity[i].resize(dim+1);     
   }

  if (m_dynamicSockets.sinkSocketExists(_vorticitySocketName)) {
  
    DataHandle<CFreal> initvorticity = m_dynamicSockets.getSocketSink<CFreal>(_vorticitySocketName)->getDataHandle();
    cf_assert(initvorticity.size() == vorticity.size());
    for (CFuint i = 0; i < sizevorticity; ++i) {
      vorticity[i][0] = initvorticity[i][0];
      vorticity[i][1] = initvorticity[i][1];
      if (dim==3)
      vorticity[i][2] = initvorticity[i][2];
      vorticity[i][2] = initvorticity[i][3];
    }
  } 
...
void VorticitySetup::storeBoundaryNormals()
{
...
void VorticitySetup::computeNormalsData()
{
  CFAUTOTRACE;
...
void VorticitySetup::flagBoundaryStates()
{
  CFAUTOTRACE;
...
void VorticitySetup::setFaceNeighCell()
{
  CFAUTOTRACE;
...
void VorticitySetup::computeCellVolumes()
{
