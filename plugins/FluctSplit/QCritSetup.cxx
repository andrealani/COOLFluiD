...
#include "FluctSplit/QCritSetup.hh"
...
MethodCommandProvider<QCritSetup, FluctuationSplitData, FluctSplitModule> qcritSetupProvider("QCritSetup");

//////////////////////////////////////////////////////////////////////////////

void QCritSetup::defineConfigOptions(Config::OptionList& options)
{
  options.addConfigOption< std::string >
    ("QCritName","name of the input limiter socket");
}
//////////////////////////////////////////////////////////////////////////////

QCritSetup::QCritSetup(const std::string& name) :
  ...
  socket_flagStates("flagStates")
{
 addConfigOptionsTo(this);
  ...
  _qcritSocketName = "Null";
   setParameter("QCritName",&_qcritSocketName);
}

//////////////////////////////////////////////////////////////////////////////

QCritSetup::~QCritSetup()
{
}
...
void QCritSetup::configure ( Config::ConfigArgs& args )
{
  CFAUTOTRACE;
  ...
  if (_qcritSocketName != "Null") {
    m_dynamicSockets.createSocketSink<CFreal>(_qcritSocketName, true);
  }
...
void QCritSetup::execute()
{
 ...
 const CFuint sizeqcrit = nbStates;
   DataHandle<CFreal> qcrit = socket_qcrit.getDataHandle();
     qcrit.resize(sizeqcrit);

  if (m_dynamicSockets.sinkSocketExists(_qcritSocketName)) {
  
    DataHandle<CFreal> initqcrit = m_dynamicSockets.getSocketSink<CFreal>(_qcritSocketName)->getDataHandle();
    cf_assert(initqcrit.size() == qcrit.size());
    for (CFuint i = 0; i < sizeqcrit; ++i) {
      qcrit[i] = initqcrit[i];
    }
  } 
...
void QCritSetup::storeBoundaryNormals()
{
...
void QCritSetup::computeNormalsData()
{
  CFAUTOTRACE;
...
void QCritSetup::flagBoundaryStates()
{
  CFAUTOTRACE;
...
void QCritSetup::setFaceNeighCell()
{
  CFAUTOTRACE;
...
void QCritSetup::computeCellVolumes()
{
