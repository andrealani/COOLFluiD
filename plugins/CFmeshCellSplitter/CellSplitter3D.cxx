...
namespace COOLFluiD {
  ...
  namespace IO {
    ...
    namespace CFmeshCellSplitter {
      ...
      const CFuint statesInTetra = 4;

//////////////////////////////////////////////////////////////////////////////

Environment::ObjectProvider<CellSplitter3D,
               MeshFormatConverter,
               CFmeshCellSplitterModule,
               1>
splitter3DProvider("CellSplitter3D");

//////////////////////////////////////////////////////////////////////////////

void CellSplitter3D::defineConfigOptions(Config::OptionList& options)
{
}

//////////////////////////////////////////////////////////////////////////////

CellSplitter3D::CellSplitter3D(const std::string& name)
...
CellSplitter3D::~CellSplitter3D()
{
}

//////////////////////////////////////////////////////////////////////////////

void CellSplitter3D::configure ( Config::ConfigArgs& args )
{
...
void CellSplitter3D::checkFormat(const boost::filesystem::path& filepath)
{
...
void CellSplitter3D::convert(const boost::filesystem::path& fromFilepath,
...
void CellSplitter3D::convertBack(const boost::filesystem::path& filepath)
{
...
void CellSplitter3D::calculateNbNewElements()
{
  CFAUTOTRACE;
...
void CellSplitter3D::convertElementsToTetra()
{
  CFAUTOTRACE;
  ...
  // working only for same order of elem and solution
      ...
      switch(currShape) {
        ...
        case CFGeoShape::TETRA:
          ...
          for(CFuint localID = 0; localID < 4; ++localID) {
            ...
            (*elementState)(elemID,localID) = _oldElemState( elemID, localID );
          }
          ++elemID;
          break;
        ...
        case CFGeoShape::PYRAM:
          ...
          newID.resize(_newTetrasSize);

          newID[0] = elemID;
          newID[1] = oldNbElements + shiftID;
        ...
        case CFGeoShape::PRISM:
          ...
          newID[0] = elemID;
          newID[1] = oldNbElements + shiftID;
          newID[2] = oldNbElements + shiftID + 1;
        ...
        case CFGeoShape::HEXA:
          ...
          for(CFuint iTetra = 1; iTetra < _newTetrasSize; ++iTetra) {
            newID[iTetra] = oldNbElements + shiftID + iTetra - 1;
          }
...
void CellSplitter3D::migrateElementTypes()
{
  CFAUTOTRACE;
  ...
  (*elementType)[0].setNbStates(nodesInTetra);
...
void CellSplitter3D::splitPyram(vector<CFuint> pyram)
{
  _newTetrasSize = 2;
...
void CellSplitter3D::splitPrism(vector<CFuint> prism)
{
  _newTetrasSize = 3;
...
void CellSplitter3D::splitHexa(vector<CFuint> hexa)
{
...
CFuint CellSplitter3D::nbNewTetras(CFuint iHexa)
{
  // Hexa in the article notation (that's why '+1')
  vector<CFuint> NodeGlobalID( nodesInHexa + 1 );
  // Indirections
...
void CellSplitter3D::getSmallestID(CFuint nbNodes, vector<CFuint> OneElement)
{
...
void CellSplitter3D::updateTRSData()
{
  CFAUTOTRACE;
  ...
  for(CFuint iTRS = 0; iTRS < data.getNbTRSs(); ++iTRS ) {
    // reads connectivity of iTRS's boundary
...
//         std::valarray<CFuint>& oldStateCon  = (oldConn[iGeo].second);
        ...
        if ( oldNodeCon.size()==4 ) {
          ...
          for(CFuint nodeID = 0; nodeID < nodesInQuad; ++nodeID){
            tempQuad[nodeID] = oldNodeCon[nodeID];
          }
          splitQuads(tempQuad);
          ...
          for(CFuint nodeID = 0; nodeID < 3; ++nodeID) {
            stateCon1[nodeID] = _newTriags[0][nodeID];
            stateCon2[nodeID] = _newTriags[1][nodeID];
          }
          shift++;
        }
...
void CellSplitter3D::splitQuads(vector<CFuint> quad)
{
