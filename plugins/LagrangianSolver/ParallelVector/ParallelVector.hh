...
void ParallelVector<T>::sincronizeAdd(){
  ...
  vector<CFuint> sendBuffer( m_sendVectorOverlapLocalIDs.size() );
  vector<CFuint> recvBuffer( m_recvVectorOverlapLocalIDs.size() );
...
void ParallelVector<T>::getSharedEntries(){
  ...
  map<CFuint, CFuint> globalLocalGhostMap;
  map<CFuint, CFuint> globalLocalOwnerMap;
  ...
  for(CFuint i=0; i<nbCells; ++i){
    ...
    if (cell->getState(0)->isParUpdatable() ){
      globalLocalGhostMap.insert( pair<CFuint,CFuint>( cell->getState(0)->getGlobalID(),
                                                       cell->getState(0)->getLocalID()    )  );
    }
    else{
      globalLocalOwnerMap.insert( pair<CFuint,CFuint>( cell->getState(0)->getGlobalID(),
                                                       cell->getState(0)->getLocalID()    )  );
