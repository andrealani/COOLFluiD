...
#include "NewtonMethod/StdUpdateSolPP2.hh"
...
MethodCommandProvider<StdUpdateSolPP2, NewtonIteratorData, NewtonMethodMHDModule> 
stdUpdateSolPP2Provider("StdUpdateSolPP2");

//////////////////////////////////////////////////////////////////////////////

void StdUpdateSolPP2::defineConfigOptions(Config::OptionList& options)
{
  ...
  options.addConfigOption< bool >("fullMHDorNot", "Determine whether define nonhomgenious density within 1.2 Rs");
...
StdUpdateSolPP2::StdUpdateSolPP2(const std::string& name) : 
  ...
  socket_invalidStates("invalidStates")
{
  ...
  rho_fullMHDorNot = false;
  setParameter("fullMHDorNot", &rho_fullMHDorNot);
...
void StdUpdateSolPP2::setup()
{
  ...
  if (m_alpha.size() != nbEqs) {
    throw BadValueException (FromHere(),"StdUpdateSolPP2::setup() : m_alpha.size() != nbEqs");
  }
}

//////////////////////////////////////////////////////////////////////////////

void StdUpdateSolPP2::execute()
{
  CFAUTOTRACE;
  ...
  //>>---- Mark 20260215------------------
  //CFreal maxvA = 2.0*Bmagmax*Bref / pow((1.0 * rhoref * mu0), 0.5);
  //<<---- Mark 20260215------------------
  //--------------- Mark 2025.03.11<<---------------------
  for (CFuint iState = 0; iState < states_size; ++iState)
  {
	...
	//	cur_state[7] -= m_alpha[7] * dU(iState, 7, nbEqs);
	 // }
	  ...
	  // ---- >> Mark 2025.12.06-------------------------------------------- -
	  /*
	  if ((cur_state[7] < (_pBC*1.e-9)) || (cur_state[7] > (_pBC*1.e3))){
		  cur_state[7] -= m_alpha[7] * dU(iState, 7, nbEqs);
	  }
	  if ((cur_state[0] < (_rhoBC*1.e-9)) || (cur_state[0] > (_rhoBC*1.e2))){
	  ...
	  */
	  ...
	  if (SubSystemStatusStack::getActive()->getNbIter()>=0){
		  ...
		  //---->> Mark 2025.12.06---------------------------------------------
		 /*
		  //if (r < 1.2 && (SubSystemStatusStack::getActive()->getNbIter() < 10) && (rho_fullMHDorNot == false)){
		  if (r < 1.01){
			  if (Bmag*Bref < 2.1e-3){
			  ...
			  cur_state[0] = densityBoundary_dimless;
		  }		  
		  */
		  //------ Mark 2025.12.06<<-------------------------------------------
		  ...
		  densityBoundary_dimless = cur_state[0];
		  CFreal vA = Bmag*Bref / pow((densityBoundary_dimless * rhoref * mu0), 0.5);
		  dist = (vA - maxvA) / 2.0e3;
		  smooth_factor = 0.5 + 0.5 * std::tanh(dist * 3.141592654);
		  //if (r < 1.1){
		  cur_state[0] = (smooth_factor * (Bmag*Bref)*(Bmag*Bref) / (maxvA*maxvA) / mu0 + densityBoundary_dimless * rhoref * (1.0 - smooth_factor)) / rhoref;
		  //}
		  ...
		  CFreal minT = 1.0e4;
		  pth = p_temp*pref;
		  CFreal T = cur_state[7] * pref / (2.0 * cur_state[0] * rhoref*kB / (mu_cor*mH)); //temperature in K
		  dist = (minT - T) / 2.0e1;
		  smooth_factor = 0.5 + 0.5 * std::tanh(dist * 3.141592654);
		  cur_state[7] = (smooth_factor * minT * (2.0 * cur_state[0] * rhoref*kB / (mu_cor*mH)) + pth * (1.0 - smooth_factor)) / pref;
		...
		//	  cur_state[7] -= m_alpha[7] * dU(iState, 7, nbEqs);
		  ...
		  */
		  ...
		  if (r < 1.1 + 0.002){
			...
			//  CFreal vr = x/r*cur_state[1] + y/r*cur_state[2] + z/r*cur_state[3];
			 // CFreal VrRain = -0.3*std::sqrt(-2.0*g*(1.1 - min(r, 1.1)));
			 // if (vr < VrRain){
			//	  smooth_factor = VrRain / vr;
			//	  cur_state[1] = cur_state[1] * smooth_factor;
			//	  cur_state[2] = cur_state[2] * smooth_factor;
			//	  cur_state[3] = cur_state[3] * smooth_factor;
			//  }
...
//<< mark 2024.06.17	  
  ...
  if (badStatesIDs.size() > 0)
  {
    ...
    CFLog(VERBOSE, "StdUpdateSolPP2::execute() => [" << badStatesIDs.size() << "] invalid states detected\n");
...
vector<SafePtr<BaseDataSocketSink> > StdUpdateSolPP2::needsSockets()
{
...
vector<SafePtr<BaseDataSocketSource> > StdUpdateSolPP2::providesSockets()
{
